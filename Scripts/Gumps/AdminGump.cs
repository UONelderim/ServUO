...
namespace Server.Gumps
{
	...
	public class AdminGump : Gump
	{
		...
		public AdminGump(Mobile from, AdminGumpPage pageType, int listPage, List<object> list, string notice, object state)
			: base(50, 40)
		{
			...
			switch (pageType)
			{
				...
				case AdminGumpPage.Clients:
				{
					if (m_List == null)
					{
						m_List = new List<object>(NetState.Instances);
						m_List.Sort();
					}
					...
					for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
					{
						...
						Mobile m = ns.Mobile;
						IAccount a = ns.Account as IAccount;
						int offset = 140 + (i * 20);
				...
				case AdminGumpPage.ClientInfo:
				{
					...
					IAccount a = m.Account as IAccount;
				...
				case AdminGumpPage.Accounts_Shared:
				{
					...
					for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
					{
						...
						for (int j = 0; j < accts.Count; ++j)
						{
							...
							if (j < 4)
							{
								IAccount acct = (IAccount)accts[j];
				...
				case AdminGumpPage.Accounts:
				{
					if (m_List == null)
					{
						m_List = new List<object>(Accounts.GetAccounts());
						m_List.Sort();
					}

					List<object> rads = (state as List<object>);
					...
					for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
					{
						IAccount a = m_List[index] as IAccount;
				...
				case AdminGumpPage.AccountDetails_Access_ClientIPs:
				{
					...
					if (m_List == null)
						m_List = new List<object>(a.LoginIPs);
				...
				case AdminGumpPage.AccountDetails_Access_Restrictions:
				{
					...
					if (m_List == null)
						m_List = new List<object>(a.IPRestrictions);
				...
				case AdminGumpPage.AccountDetails_Comments:
				{
					...
					for (int i = 0; i < a.Comments.Count; ++i)
					{
						...
						IAccountComment c = a.Comments[i];
				...
				case AdminGumpPage.Firewall:
				{
					...
					if (m_List == null)
						m_List = new List<object>(Firewall.List);
				...
				case AdminGumpPage.FirewallInfo:
				{
					...
					if (!(state is Firewall.IFirewallEntry fwe))
						break;
		...
		private static List<object> GetSharedAccounts(IPAddress[] ipAddresses)
		{
			List<object> list = new List<object>();
		...
		public static void BanShared_Callback(Mobile from, bool okay, object state)
		{
			...
			if (okay)
			{
				IAccount a = (IAccount)state;
				list = GetSharedAccounts(a.LoginIPs);

				for (int i = 0; i < list.Count; ++i)
				{
					IAccount acc = (IAccount)list[i];

					acc.SetUnspecifiedBan(from);
					acc.Banned = true;
				}
		...
		public static void ResendGump_Callback(Mobile from, object state)
		{
			...
			List<object> list = (List<object>)states[0];
			List<object> rads = (List<object>)states[1];
		...
		public static void Marked_Callback(Mobile from, bool okay, object state)
		{
			...
			List<object> list = (List<object>)states[1];
			List<object> rads = (List<object>)states[2];
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (m_PageType == AdminGumpPage.Accounts)
			{
				...
				List<object> rads = m_State as List<object>;
			...
			switch (type)
			{
				...
				case 5:
				{
					switch (index)
					{
						...
						case 15:
							...
							break;
						case 4:
							from.Prompt = new AddCommentPrompt(m_State as IAccount);
							from.SendMessage("Enter the new account comment.");
							break;
						case 5:
							from.Prompt = new AddTagNamePrompt(m_State as IAccount);
						...
						case 6:
						{
							...
							else
							{
								...
								{
									dispAccount = Accounts.Create(un, pw);
						...
						case 7:
						{
							...
							if (string.IsNullOrEmpty(match))
							{
								results = new List<object>(Accounts.GetAccounts());
								results.Sort();
							...
							else
							{
								...
								foreach (IAccount check in Accounts.GetAccounts())
								{
							...
							if (results.Count == 1)
								...
								from.SendGump(new AdminGump(from, AdminGumpPage.Accounts, 0, results, notice == null ? (results.Count == 0 ? "Nothing matched your search terms." : null) : notice, new List<object>()));
						...
						case 11:
						{
							IAccount a = m_State as IAccount;
						...
						case 16: // view shared
						{
							...
							List<object> list = GetSharedAccounts(a.LoginIPs);

							if (list.Count > 1 || (list.Count == 1 && !list.Contains(a)))
							{
								from.SendGump(new AdminGump(from, AdminGumpPage.Accounts, 0, list, null, new List<object>()));
							}
						...
						case 17: // ban shared
						{
							...
							if (list.Count > 0)
							{
								...
								for (int i = 0; i < list.Count; ++i)
									sb.AppendFormat("<br>- {0}", ((IAccount)list[i]).Username);
						...
						case 29: // Mark all
						{
							...
							from.SendGump(new AdminGump(from, AdminGumpPage.Accounts, m_ListPage, m_List, null, new List<object>(list)));
						...
						case 30: // View all accounts less than 3 minutes of total online time.
						{
							...
							if (results.Count == 1)
								...
								from.SendGump(new AdminGump(from, AdminGumpPage.Accounts, 0, results, (results.Count == 0 ? "Nothing matched your search terms." : null), new List<object>()));
						...
						case 35: // Unmark house owners
						{
							...
							List<object> newRads = new List<object>();

							foreach (IAccount acct in rads)
							{
								bool hasHouse = false;
							...
							else
							{
								...
								if (m_List != null && index >= 0 && index < m_List.Count)
								{
									if (m_List[index] is Account)
										...
										from.SendGump(new AdminGump(from, AdminGumpPage.Accounts, 0, (List<object>)(((DictionaryEntry)m_List[index]).Value), null, new List<object>()));
								}
							}
				...
				case 6:
				{
					switch (index)
					{
						case 0:
						{
							...
							{
								foreach (var fwe in Firewall.List)
								{
									string check = fwe.ToString();
				...
				case 8:
				{
					if (m_List != null && index < m_List.Count)
					{
						...
						{
							List<string> list = new List<string>(a.IPRestrictions);

							list.Remove(m_List[index].ToString());
		...
		{
			Account.GetAccountInfo(a, out accessLevel, out online);
		}
		...
		private class AddCommentPrompt : Prompt
		{
			...
			public AddCommentPrompt(IAccount acct)
			{
				m_Account = acct;
			}
			...
			public override void OnResponse(Mobile from, string text)
			{
				if (m_Account != null)
				{
					m_Account.AddComment(from.RawName, text);
		...
		private class AddTagNamePrompt : Prompt
		{
			...
			public AddTagNamePrompt(IAccount acct)
			{
				m_Account = acct;
			}
		...
		private class AddTagValuePrompt : Prompt
		{
			...
			public AddTagValuePrompt(IAccount acct, string name)
			{
				m_Account = acct;
				m_Name = name;
			}
