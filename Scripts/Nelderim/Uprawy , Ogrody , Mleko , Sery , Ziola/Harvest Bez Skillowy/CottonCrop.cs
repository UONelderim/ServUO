...
namespace Server.Items.Crops
{
	...
	public class CottonSeedling : BaseCrop
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if ((Utility.RandomDouble() <= .25) && !(Sower.AccessLevel > AccessLevel.Counselor))
			{
				//25% Chance
	...
	public class CottonCrop : BaseCrop
	{
			...
			if (DateTime.Now > lastpicked.AddSeconds(3)) // 3 seconds between picking
			{
				...
				if (from.InRange(this.GetWorldLocation(), 1))
				{
					...
					else //check sower
					{
						...
						Yield -= pick;
						from.SendMessage("Zebrales {0} bawelny{1}!", pick, (pick == 1 ? "" : ""));
						...
						if (pick == 5)
						{
							...
							from.AddToBackpack(new CottonSeed());
							return;
						}
		...
		{
			private readonly CottonCrop i_plant;
