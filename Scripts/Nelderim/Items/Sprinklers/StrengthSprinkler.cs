...
namespace Server.Items
{
    ...
    public class StrengthSprinkler : Item, ISecurable, IUsesRemaining
    {
		...
		public StrengthSprinkler() : base( 0xE7A )
        {
            Movable = true;
            Weight = 1.0;
            Name = "Urzadzenie od opryskow mikstura sily";
            Hue = 1153;
            StorageLimit = 1000;
        }
            ...
            else
            {
                if (this.IsAccessibleTo(from))
                {
                    if (m_UsesRemaining < 1)
                        from.SendMessage("Musisz wlozyc wiecej mikstur sily, by z tego korzystac!");
                    else
                    {
                        ...
                        foreach (Item item in eable)
                        {
                            if (house.IsInside(item) && item is PlantItem && item.IsLockedDown)
                            {
                                ...
                                if (CanBeStr(plant))
                                {
									if (plant.PlantSystem.Disease == 1 && plant.PlantSystem.StrengthPotion == 0 && m_UsesRemaining > 0)
									{
									...
									if (plant.PlantSystem.Disease == 2 && plant.PlantSystem.StrengthPotion == 0 && m_UsesRemaining > 1)
									{
									...
									if (plant.PlantSystem.Disease == 2 && plant.PlantSystem.StrengthPotion == 1 && m_UsesRemaining > 0)
									{
									plant.PlantSystem.StrengthPotion = 2;
                                    found = true;
									m_UsesRemaining--;
                        ...
                        if (found)
                        {
                            from.SendMessage("Choroba zostaly zabita.");
                            from.PlaySound(0x12);
                        }
                        else
                            from.SendMessage("Wokol brak roslin, ktore mozna wzmocnic!");
                    }
                }
                else
		...
		public override bool OnDragDrop(Mobile from, Item item)
        {
            ...
            if (item is GreaterStrengthPotion && m_UsesRemaining < m_StorageLimit)
            {
						m_UsesRemaining++;
                ...
                if (keg.Type == PotionEffect.StrengthGreater && keg.Held <= freespace )
					 {
