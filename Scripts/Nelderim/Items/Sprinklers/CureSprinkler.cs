...
namespace Server.Items
{
	public class CureSprinkler : Item, ISecurable, IUsesRemaining
	{
		...
		public CureSprinkler() : base(0xE7A)
		{
			Movable = true;
			Weight = 1.0;
			Name = "Urzadzenie od opryskow mikstura odtrucia";
			Hue = 43;
			StorageLimit = 1000;
		}
			...
			else
			{
				if (this.IsAccessibleTo(from))
				{
					if (m_UsesRemaining < 1)
						from.SendMessage("Musisz wlozyc wiecej mikstur odtrucia, by z tego korzystac!");
					else
					{
						...
						if (found)
						{
							from.SendMessage("Grzyb na twych roslinach zostal potraktowany mikstura.");
							from.PlaySound(0x12);
						}
						else
