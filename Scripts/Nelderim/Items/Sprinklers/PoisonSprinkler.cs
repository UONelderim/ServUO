...
namespace Server.Items
{
    ...
    public class PoisonSprinkler : Item, ISecurable, IUsesRemaining
    {
        ...
        [Constructable]
		public PoisonSprinkler() : base( 0xE7A )
        {
            Movable = true;
            Weight = 1.0;
            Name = "Urzadzenie od opryskow trucizna";
            Hue = 68;
            StorageLimit = 1000;
        }
        public PoisonSprinkler(int uses, int storageLimit)
            : base(0xE7A)
        {
            Weight = 1.0;
            Name = "Urzadzenie od opryskow trucizna";
            ...
            else
            {
                if (this.IsAccessibleTo(from))
                {
                    if (m_UsesRemaining < 1)
                        from.SendMessage("Musisz wlozyc wiecej mikstur trucizny, by z tego korzystac!");
                    else
                    {
                        ...
                        foreach (Item item in eable)
                        {
                            if (house.IsInside(item) && item is PlantItem && item.IsLockedDown)
                            {
                                ...
                                {
									if (plant.PlantSystem.Infestation == 1 && plant.PlantSystem.PoisonPotion == 0 && m_UsesRemaining > 0)
									{
									...
									if (plant.PlantSystem.Infestation == 2 && plant.PlantSystem.PoisonPotion == 0 && m_UsesRemaining > 1)
									{
									...
									if (plant.PlantSystem.Infestation == 2 && plant.PlantSystem.PoisonPotion == 1 && m_UsesRemaining > 0)
									{
									plant.PlantSystem.PoisonPotion = 2;
                                    found = true;
									m_UsesRemaining--;
                        ...
                        if (found)
                        {
                            from.SendMessage("Insekty zostaly zabite.");
                            from.PlaySound(0x12);
                        }
                        else
