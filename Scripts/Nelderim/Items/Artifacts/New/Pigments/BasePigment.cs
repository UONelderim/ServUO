    ...
    public class BasePigment : Item, IUsesRemaining
    {
            ...
            switch (m_Target)
            {
                case PigmentTarget.Metal:
                    ...
                    else if (level == 1)
                    {
                        ...
                        Hue = Utility.RandomList(0x07A1, 0x079E, 0x054D, 0x0622, 0x08A4, 0x0552, 1158, 1109, 1644, 33, 1166, 1366);
                    }
                    break;
                case PigmentTarget.Leather:
                    if (level == 0)
                    ...
                    {
                        m_UsesRemaining = uses + 1;
                        Hue = Utility.RandomList(0x07A1, 0x079E, 0x054D, 0x055A, 0x08A4, 0x0552, 1158, 1420, 1270, 1364, 1165);
                    }
                    break;
                case PigmentTarget.Wood:
                    if (level == 0)
                ...
                case PigmentTarget.Spellbook:
                    ...
                    {
                        m_UsesRemaining = 2;
                        Hue = Utility.RandomList(0x07A1, 0x079E, 0x054D, 0x0622, 0x08A4, 0x0552, 1158, 1266, 1359, 1161, 2460);
                    }
                    break;
                default:
                    Hue = 0;
        ...
        private static bool IsInResourceList(List<CraftResource> resourcesAccepted, Item i)
        {
            ...
            if (IsInType(t, typeof(BaseWeapon)))
                return (resourcesAccepted.Contains(((BaseWeapon)i).Resource) || resourcesAccepted.Contains(((BaseWeapon)i).Resource2));
            else if (IsInType(t, typeof(BaseArmor)))
                return (resourcesAccepted.Contains(((BaseArmor)i).Resource) || resourcesAccepted.Contains(((BaseArmor)i).Resource2));
