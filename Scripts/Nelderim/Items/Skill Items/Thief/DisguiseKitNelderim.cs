...
namespace Server.Items
{
	public class DisguiseKitNelderim : Item
	{
		...
		{
			Race race = from.Race;
			if (from is PlayerMobile && ((PlayerMobile)from).RaceMod != null &&
			    ((PlayerMobile)from).RaceMod != Race.DefaultRace)
	...
	{
		private readonly DisguiseKitNelderim m_Kit;
		private string m_Name;

		private static readonly Race[] RacesList =
		{
			Race.NTamael, Race.NJarling, Race.NNaur, Race.NElf, Race.NDrow, Race.NKrasnolud
		};
		...
		private RaceDisguiseGump(Mobile from, Race race, DisguiseKitNelderim kit, string name, int skinHue, int hairHue,
			int hairItemID, int facialHairItemID)
		...
		public override void ExtendedResponse(RelayInfo info)
		{
			...
			m_From.SendGump(new RaceDisguiseGump(m_From, m_Race, m_Kit, m_Name, m_SkinHue, m_HairHue, m_HairItemID,
				m_FacialHairItemID));
		}
		...
		public override void ApplyAppearance(Race race, int skinHue, int hairHue, int hairID, int facialHairID)
		{
			...
			// start timer 2h
			m_Timers[m_From] =
				Timer.DelayCall(TimeSpan.FromHours(2.0), new TimerStateCallback(OnDisguiseExpire), m_From);
		}
