...
namespace Server.Items
{
	...
	{
		private readonly SkinHue m_SkinHue;
		...
		private static readonly SkinHueEntry[] m_Entries =
		{
			//The first number is the starting hue, The second is how
			//many hues after the starting hue will show up on the page.
			new SkinHueEntry("*****", 1602, 26), new SkinHueEntry("*****", 1628, 27),
			new SkinHueEntry("*****", 1502, 32), new SkinHueEntry("*****", 1302, 32),
			new SkinHueEntry("*****", 1402, 32), new SkinHueEntry("*****", 1202, 24),
			new SkinHueEntry("*****", 2402, 29), new SkinHueEntry("*****", 2213, 6), new SkinHueEntry("*****", 1102, 8),
			new SkinHueEntry("*****", 1110, 8), new SkinHueEntry("*****", 1118, 16), new SkinHueEntry("*****", 1134, 16)
		};
		...
		public override void OnResponse(NetState from, RelayInfo info)
		{
			...
			if (info.ButtonID != 0 && switches.Length > 0)
			{
				...
				{
					m.SendMessage(
						"You must have a backpack to use this!"); // Had to add this so the script would work properly.
