...
namespace Server.Items
{
	public class DecietBrazier : Item
	{
		...
		private static readonly Type[] m_tMonsters =
		{
			typeof(HeadlessOne), typeof(Wraith), typeof(Ogre), typeof(Troll), typeof(Wyvern), typeof(GiantSpider),
			typeof(Slime), typeof(Mongbat), typeof(DireWolf), typeof(Lizardman), typeof(Orc), typeof(Skeleton),
			typeof(EarthElemental), typeof(Ettin), typeof(Ratman), typeof(Gazer), typeof(Gargoyle), typeof(Harpy),
			typeof(Lich), typeof(Nightmare), typeof(FireSteed), typeof(Drake), typeof(Dragon), typeof(Zombie)
		};
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			if (DateTime.Now > m_TimeToCanSpawn)
			{
				BaseCreature creature =
					(BaseCreature)Activator.CreateInstance(m_tMonsters[Utility.Random(m_tMonsters.Length)]);
				Point3D loc = GetSpawnPosition();
				creature.MoveToWorld(loc, Map);
				creature.Home = loc;

				Effects.SendLocationParticles(
					EffectItem.Create(creature.Location, creature.Map, EffectItem.DefaultDuration), 0x3709, 10, 30,
					5052);
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			if (m.Player)
			{
				...
				{
					this.PublicOverheadMessage(MessageType.Regular, 905,
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			writer.Write(m_bShowWarning);
			writer.Write(m_TimeDelay);
			writer.Write(m_iSpawnRange);
			writer.Write(m_TimeToCanSpawn);
		}
