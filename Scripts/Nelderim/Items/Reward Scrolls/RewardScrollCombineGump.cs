...
namespace Server.Gumps
{
	public class RewardScrollCombineGump : Gump
	{
		...
		private readonly ArrayList m_AddedScrolls;
		private readonly int m_AfterJoinClass;
		private readonly int m_AfterJoinUses;
			...
			: base(30, 30)
		{
			...
			{
				string tmp = "Zwoj Nagrody Klasy " + sc.Class;
			...
			cl = (sum == 0) ? 0 : (16 - (int)Math.Log(sum / 250.0, 2.0));
			if (uses > 5)
				m_AfterJoinUses = 1;
			else
			{
			...
			string value = "Aktualna wartość: " + sum + " centarów.";
			string cla = "Klasa zwoju po połączeniu: " + cl;
			string us = "Ilość użyć po połączeniu: " + m_AfterJoinUses;
