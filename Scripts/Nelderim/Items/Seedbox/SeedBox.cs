...
/* 
 ...
 Modyfikacje: tlumaczenia, zmiana ilosci przechowywanych nasion
*/

namespace Server.Items
{
	[Aliases("Server.Items.NSeedBox")]
	[FlipableAttribute(0xE41, 0xE40)]
	public class SeedBox : BaseContainer
	{
		private static readonly int m_maxSeeds = 1000; //maximum amount of seeds a Seed Box can hold

		public int[,] m_counts = new int[17, 19];

		private static readonly int[] m_Hues =
		{
			0x455, 0x481, 0x66D, 0x21, 0x59B, 0x42, 0x53D, 0x5, 0x8A5, 0x38, 0x46F, 0x2B, 0xD, 0x10, 0x486, 0x48E,
			0x489, 0x495, 0x0
		};
...
//			return;
			...
			if (Movable)
			{
				from.SendMessage("Skrzynia musi byc zablokowana aby z niej kozystac!");
				return;
			}
			...
			if (!from.InRange(GetWorldLocation(), 2))
				...
				from.SendGump(new NSeedBoxGump((PlayerMobile)from, this));
		}
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			list.Add(1060662, "{0}\t{1}", "Nasion", SeedCount());
		}
		...
		public override bool OnDragDrop(Mobile from, Item dropped)
		{
			...
			if (!(dropped is Seed))
			{
				from.SendMessage("W tym mozna przechowywac tylko nasiona.");
				return false;
			}

			if (SeedCount() > m_maxSeeds)
			{
				from.SendMessage("Skrzynia jest pelna.");
				return false;
			}
			...
			if (!seed.ShowType)
			{
				from.SendMessage("Nie mozesz przechowac nasion z ktorych nie wiesz co wyrosnie.");
				return false;
			}
