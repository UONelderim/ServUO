...
namespace Server.ACC.CSS.Systems.Druid
{
	public class DruidFamiliarSpell : DruidSpell
	{
		...
		public static DruidFamiliarEntry[] Entries { get; } =
		{
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (index >= 0 && index < m_Entries.Length)
			{
				...
				else if (lore < entry.ReqAnimalLore || taming < entry.ReqAnimalTaming)
				{
					...
					m_From.SendMessage(String.Format("Musisz mieć {0:F1} Druidyzmu i {1:F1} Magii.",
						entry.ReqAnimalLore, entry.ReqAnimalTaming));
