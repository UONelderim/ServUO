		...
		public void Target(Mobile m)
		{
			if (CheckBSequence(m) && CheckFizzle())
			{
				...
				if (chance > Utility.Random(100))
				{
					...
					Effects.SendMovingParticles(from, to, 0x2255, 1, 0, false, false, 13, 3, 9501, 1, 0,
		...
		public void TargetItem(Item o, Mobile caster)
		{
			if (caster.CheckSkill(SkillName.Chivalry, 0, 100) && (caster.Karma * -1) > 0)
			{
				if (o is WoodenChest)
				{
					...
					caster.PrivateOverheadMessage(MessageType.Regular, 1153, false,
						"The curse has been lifted from the books.", caster.NetState);
					o.Delete();
				}
				else if (o is MetalChest)
				{
					...
					caster.PrivateOverheadMessage(MessageType.Regular, 1153, false,
						"The curse has been lifted from the " + curseName + ".", caster.NetState);
					o.Delete();
				}
