...
namespace Server.ACC.CSS
{
	public class IconPlacementGump : Gump
	{
		...
		public IconPlacementGump(CSpellbook book, Mobile from, int x, int y, int i, int icon, Type spellType,
			int background, School school) : base(0, 0)
		{
			m_Book = book;
			m_From = from;
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 1:
				{
					...
					CentralMemory.AppendModule(from.Serial, im, false);
				...
				case 2:
				{
					...
					{
						from.SendMessage(
							"Please enter a X coordinate in the top box and a Y coordinate in the bottom box");
					}
					else
					{
						int x = m_X;
					...
					from.SendGump(new IconPlacementGump(m_Book, from, m_X, m_Y, m_I, m_Icon, m_Spell, m_Background,
						m_School));
					break;
				}
				...
				case 3:
				{
					...
					{
						from.SendGump(new IconPlacementGump(m_Book, from, m_X, (m_Y - m_I), m_I, m_Icon, m_Spell,
							m_Background, m_School));
					}
				...
				case 4:
				{
					MakeI(info);
					from.CloseGump(typeof(IconPlacementGump));
					from.SendGump(new IconPlacementGump(m_Book, from, (m_X + m_I), m_Y, m_I, m_Icon, m_Spell,
						m_Background, m_School));
					break;
				}
