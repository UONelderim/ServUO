...
namespace Server.ACC.CSS
{
	...
	{
		private readonly IconInfo m_Info;

		public SpellIconGump(IconInfo info)
			: base(info.Location.X, info.Location.Y)
		{
			m_Info = info;
			...
			AddPage(0);
			AddBackground(0, 0, 54, 54, info.Location.Z);
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				case 1:
				{
					...
					CentralMemory.AppendModule(state.Mobile.Serial, mod, true);
				...
				case 2:
				{
					if (!DesignContext.Check(state.Mobile))
					{
					...
					if (SpellRestrictions.UseRestrictions &&
					    !SpellRestrictions.CheckRestrictions(state.Mobile, m_Info.SpellType))
					{
