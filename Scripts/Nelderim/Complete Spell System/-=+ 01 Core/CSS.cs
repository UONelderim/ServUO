...
namespace Server.ACC.CSS
{
	public class CSS : ACCSystem
	{
		...
		{
			CommandSystem.Register("CSSCast", AccessLevel.Player, CSSCast_OnCommand);
		}
		...
		private static void CSSCast_OnCommand(CommandEventArgs e)
		{
			if (e.Length == 1)
			{
				if (!CentralMemory.Running || !Running)
				{
				...
				CastCommandsModule module =
					(CastCommandsModule)CentralMemory.GetModule(e.Mobile.Serial, typeof(CastCommandsModule));
				if (module == null)
				{
		...
		public override void Save(GenericWriter idx, GenericWriter tdb, GenericWriter writer)
		{
			...
			foreach (DictionaryEntry de in SpellInfoRegistry.Registry)
			{
				...
				writer.Write(((ArrayList)de.Value).Count);
