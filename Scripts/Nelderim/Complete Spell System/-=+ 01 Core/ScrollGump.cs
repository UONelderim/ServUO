...
namespace Server.ACC.CSS
{
	...
	{
		private readonly CSpellInfo m_Info;
		private readonly string m_TextHue;
		private readonly CSpellbook m_Book;
		private readonly CastInfo m_CastInfo;
		private readonly CastCommandsModule m_CastCommandModule;
			...
			: base(485, 175)
		{
			...
			if (info.Name != null)
				AddHtml(30, 3, 140, 20,
					String.Format("<basefont color=#{0}><center>{1}</center></font>", textHue, info.Name), false,
					false);
			...
			if (info.Regs != null)
			{
				...
				InfoString += String.Format("<basefont color=black>Reagents :</font><br><basefont color=#{0}>",
					textHue);
			...
			{
				m_CastCommandModule =
					(CastCommandsModule)CentralMemory.GetModule(sender.Serial, typeof(CastCommandsModule));
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (info.ButtonID == 1)
			{
				if (SpellRestrictions.UseRestrictions &&
				    !SpellRestrictions.CheckRestrictions(state.Mobile, m_Info.School))
				{
			...
			else if (info.ButtonID == 3)
			{
				...
				state.Mobile.SendGump(new IconPlacementGump(m_Book, state.Mobile, 100, 100, 10, m_Info.Icon,
					m_Info.Type, m_Info.Back, m_Book.School));
			}
