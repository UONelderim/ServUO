...
namespace Server.ACC.CSS
{
	...
	{
		private readonly CSpellbook m_Book;
		private readonly ArrayList m_Spells;
			...
			*/
			...
			for (int i = 0; i < Pages; i++, CurrentPage++)
			{
				...
				for (int j = (CurrentPage - 1) * 16, C = 0; j < CurrentPage * 16 && j < m_Spells.Count; j++, C++)
				{
					...
					{
						AddButton((C > 7 ? 305 : 145), 142 + (C > 7 ? (C - 8) * 20 : C * 20), 2482, 2482, j + 1000,
							GumpButtonType.Reply, 0);
					}
				}

				AddImage(100, 100, BGImage);
				AddHtml(165, 107, 100, 20, String.Format("<basefont color=#{0}><Center>{1}</Center>", TextHue, Label1),
					false, false);
				AddHtml(285, 107, 100, 20, String.Format("<basefont color=#{0}><Center>{1}</Center>", TextHue, Label2),
					false, false);
				//End Hidden Buttons
				...
				for (int j = (CurrentPage - 1) * 16, C = 0; j < CurrentPage * 16 && j < m_Spells.Count; j++, C++)
				{
					if (HasSpell((Type)m_Spells[j]))
					{
						...
						AddHtml((C > 7 ? 305 : 145), 140 + (C > 7 ? (C - 8) * 20 : C * 20), 120, 20,
							String.Format("<basefont color=#{0}>{1}</basefont>", TextHue, info.Name), false, false);
						AddButton((C > 7 ? 285 : 125), 143 + (C > 7 ? (C - 8) * 20 : C * 20), SpellBtn, SpellBtnP,
							j + 2000, GumpButtonType.Reply, 0);
						AddButton((C > 7 ? 410 : 250), 142 + (C > 7 ? (C - 8) * 20 : C * 20), 5411, 5411, j + 1000,
							GumpButtonType.Reply, 0);
					}
				}
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (info.ButtonID >= 1000 && info.ButtonID < (1000 + m_Spells.Count))
			{
				if (SpellRestrictions.UseRestrictions &&
				    !SpellRestrictions.CheckRestrictions(state.Mobile, m_Book.School))
				{
			...
			else if (info.ButtonID >= 2000 && info.ButtonID < (2000 + m_Spells.Count))
			{
				...
				Spell spell = SpellInfoRegistry.NewSpell((Type)m_Spells[info.ButtonID - 2000], m_Book.School,
					state.Mobile, null);
				if (spell == null)
					state.Mobile.SendMessage("That spell is disabled.");
				else
