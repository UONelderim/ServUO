...
namespace Server.Gumps
{
	...
	public class GeneralItemGump : Gump
	{
		...
		///     <para>
		///         Jezeli uzytkownik wybierze przedmiot, to wykona sie akcja onItemChoose(sender, info) gdzie info.ButtonID
		///         zawiera indeks wybranego przedmiotu.
		///     </para>
		///     <para>
		///         Jezeli uzytkownik zdecyduje sie zamknac gumpa np. prawym przyciskiem myszy, to opcjonalnie zostanie wykonana
		///         akcja _onGumpClosed.
		///     </para>
		/// </summary>
		...
		public GeneralItemGump(IEnumerable<Item> items, Action<NetState, RelayInfo> onItemChoose,
			Action onGumpClosed = null)
			: this
			(
				SelectItem(items),
				onItemChoose,
		...
		/// <param name="onItemChoose">Delegat lub lambda expresion</param>
		/// <param name="onGumpClosed">Opcjonalnie delegat lub lambda expresion</param>
		public GeneralItemGump(IEnumerable<Type> types, Action<NetState, RelayInfo> onItemChoose,
			Action onGumpClosed = null)
			: this
			(
				SelectType(types),
		...
		/// <param name="onItemChoose">Delegat lub lambda expresion</param>
		/// <param name="onGumpClosed">Opcjonalnie delegat lub lambda expresion</param>
		public GeneralItemGump(IEnumerable<ItemGumpInfo> gumpInfo, Action<NetState, RelayInfo> onItemChoose,
			Action onGumpClosed = null) : this(gumpInfo)
		{
		...
		private void CreateGump()
		{
			...
			this.Y = (Count(_items) / ColumnsCount + (Count(_items) % ColumnsCount == 0 ? 0 : 1)) * _buttonHeight +
			         _horizontalMargin * 2;
			...
			foreach (var e in entries)
				AddImageTiledButton(e.x, e.y, _buttonArt, _buttonArt, e.index, GumpButtonType.Reply, 0, e.info.ItemID,
					0x0, ItemPosition.X, ItemPosition.Y, e.info.LocalizedTooltip);
		}
		...
		private static List<GumpHelper> SelectGump(IEnumerable<ItemGumpInfo> items, int columns)
		{
			...
			foreach (ItemGumpInfo it in items)
			{
				list.Add(new GumpHelper
				{
					index = i + 1,
					x = _verticalMargin + _buttonWidth * (i % columns),
					y = _horizontalMargin + (i / columns) * _buttonHeight,
					info = it
				});
				i++;
			}
