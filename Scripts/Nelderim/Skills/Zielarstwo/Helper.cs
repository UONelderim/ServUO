...
namespace Server.Items.Crops
{
	...
	class WeedHelper
	{
		...
		public static int GardenTileID { get { return 13001; } } // 13001 == 0x32C9
		...
		public static bool CheckSoil(BaseSeedling crop, Map map, Point3D p)
		{
			var floors = GetTopSurface(map, p);

			if (CanGrowOnStaticTile(crop, floors))
				return true;

			if (crop.CanGrowGarden && ValidateGardenPlot(map, p))
			{
				crop.BumpZ = true;
				return true;
			}

			if (floors.Count == 0 && CanGrowOnLandTile(crop, map, p))
				return true;

			return false;
		}

		private static List<StaticTile> GetTopSurface(Map map, Point3D p)
		{
			var floors = new List<StaticTile>();
		...
		private static bool CanGrowOnStaticTile(BaseSeedling crop, List<StaticTile> zTiles)
		{
			if (zTiles.Count == 0)
				return false;
