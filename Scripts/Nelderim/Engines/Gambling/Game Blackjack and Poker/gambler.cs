...
namespace Server.Mobiles
{
       ...
       public override bool HandlesOnSpeech( Mobile from )
       {
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( from.InRange( this, 4 ))
			{
				...
				if (e.Speech.ToLower() == "witaj" || e.Speech.ToLower() == "powitać" || e.Speech.ToLower() == "hazard")
				{
                    			message = "Powitac. Grasz w karty? Jesli tak, to powiedz, ze chcesz zagrac.";
                    			this.Say( message );
				}
				else if (e.Speech.ToLower() == "gra")
				{
                    			message = "Chcesz zagrać w czarnego Jacka czy partyjke pokerka?";
                    			this.Say( message );
				}
				else if (e.Speech.ToLower() == "od nowa")
				{
                    			if ( from.AccessLevel >= AccessLevel.Seer )
                    			{
                    				busy = false;
                    				message = "Juz nie jestem zajety.";
                    				this.Say( message );
                    			}
				}
				else if  (e.Speech.ToLower() == "czarny Jacek" || e.Speech.ToLower() == "zagraj w czarnego Jacka")
				{
					if (!busy)
					{
					playerbet = 100;
					...
					message = "Czy chcesz sprobowac szczescia?";
					this.Say( message );
					...
					{
					message = "Juz gramy w karty, Ty mosku.";
					this.Say( message );
					}
					else
					{
					message = "Jestem zajety.";
					this.Say( message );
					}
				}
			
				else if  (e.Speech.ToLower() == "poker" || e.Speech.ToLower() == "zagraj w pokera")
				{
					if (!busy)
					{
					playerbet = 100;
					...
					{
					message = "Juz gramy w karty, Ty pacanie.";
					this.Say( message );
					}
					else
					{
	...
	private Gambler m_From;
		...
		public GamblerStatsGump( Gambler gambler ) : base( 10, 10 )
		{
			...
			AddLabel( 45, 100, 70, "Czarny Jacek" );
			AddLabel( 45, 115, 600, "Wygrane: "+m_From.gamestats[0] );
			AddLabel( 45, 130, 600, "Przegrane: "+m_From.gamestats[1] );
			AddLabel( 45, 145, 600, "Remisy: "+m_From.gamestats[2] );

			AddLabel( 45, 165, 70, "Poker" );
			AddLabel( 45, 180, 600, "Wygrane: "+m_From.gamestats[3] );
			AddLabel( 45, 195, 600, "Przegrane: "+m_From.gamestats[4] );
			AddLabel( 45, 210, 600, "Remisy: "+m_From.gamestats[5] );

			AddLabel(  45, 230, 1500, "Od nowa" );
					...
					else
					{
						if (m_From.dwin==0)
						{
						   if (m_From.paydealer( from, m_From.playerbet))
						   {
							...
							m_From.pokermsg = "(Klikaj na karty by rozdac je ponownie).";
						   }
						   else
						   {
						   	m_From.pokermsg = "Chcesz grac bez zlota?!";
						   }
						}
	...
	public void finishpokergame(Mobile from)
	{
		...
		if(isFlush && isStrt)
		{
  			...
  			{
  				m_From.pokermsg = "Poker krolewski";
  				m_From.pwin = m_From.playerbet * 12;
  			}
  			else
  			{
  				m_From.pokermsg = "Strit Kolor";
	...
	private Gambler m_From;
		...
		public BlackjackGump( Mobile mobile, Gambler gambler ) : base( 10, 10 )
		{
			...
			AddLabel( 35, 109, 1500, "Rozdajacy:" );
			AddLabel( 35, 229, 600, "Gracz:" );
			...
			switch ( info.ButtonID )
			{
				...
				{ // deal
					...
					for ( i = 2; i <= 4; ++i  )
					{
						...
						m_From.pokermsg = "Musisz mieć pieniadze!";
	...
	public void finishgame(Mobile from)
	{
		...
		if (m_From.dbj && m_From.pbj)
		{
			...
			m_From.pokermsg = "Remis.";

		}
		else if (m_From.dbj)
		{
			m_From.gamestats[0] += 1;
			m_From.pokermsg = "Wyglada na to, ze wygrywam.";
		...
		{ // count players card
		   ...
		   { // player won
			...
			m_From.payplayer(from,m_From.pwin);
			m_From.gamestats[1] += 1;
			m_From.pokermsg = "Wygrywasz kolejna runde .";
		   }
		   else
		   { // player won
			m_From.dwin = 0;
			...
			m_From.payplayer(from,m_From.pwin);
			m_From.gamestats[1] += 1;
			m_From.pokermsg = "Ten jest dla Ciebie...";
		   }
		}
		} // end of else
		}
	...
	{
	    string message = "CZY TY KURWA PROBUJESZ MNIE WCISKAC LAPOWKE, TY LACHMANIE!";
	    this.Say( message );
	    return false;
	}
