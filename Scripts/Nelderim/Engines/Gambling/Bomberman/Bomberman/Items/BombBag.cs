...
namespace Solaris.BoardGames
{
	...
	public class BombBag : Item
	{
		...
		public override void OnDoubleClick(Mobile from)
		{
			if (Owner == null)
			{
				from.SendMessage("Jesteś teraz właścicielem tego plecaka z bombami!");
				Owner = from;
			}
			...
			if (from.Followers > 0)
			{
				from.SendMessage("Nie możesz używać tego, gdy masz zwierzęta!");
				return;
			}
			...
			if (BombAtFeet(from))
			{
				from.SendMessage("U twoich stóp już znajduje się bomba!");
				return;
			}
			...
			if (Bombs.Count < MaxBombs)
			{
				...
				from.SendMessage("Wstawiam bombę!");
			}
			else
			{
				from.SendMessage("Masz już za dużo bomb na polu!");
			}
		}
