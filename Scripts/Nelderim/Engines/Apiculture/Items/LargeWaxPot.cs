...
namespace Server.Items
{
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			else
			{
				from.PrivateOverheadMessage(0, 1154, false,
					"Wybierz czysty wosk pszczeli by go stopic lub ten garnek by wyjac z niego wosk.", from.NetState);
				BeginAdd(from);
			}
		}
		...
		public void EndAdd(Mobile from, object o)
		{
			if (o is Item && ((Item)o).IsChildOf(from.Backpack))
			{
				if (o is PureRawBeeswax)
				{
					...
					{
						from.PrivateOverheadMessage(0, 1154, false,
							"Ten garnek jest zbyt uszkodzony, by moc go nadal uzywac.", from.NetState);
						return;
					}

					if (m_Beeswax >= MaxWax)
					{
						from.PrivateOverheadMessage(0, 1154, false, "Ten garnek nie zmiesci wiecej wosku.",
							from.NetState);
						return;
					}
					...
					{
						@from.PrivateOverheadMessage(0, 1154, false,
							"Musisz byc blisko zrodla ciepla, by moc topic wosk.", @from.NetState);
						return;
					}
				...
				else if (o == this)
				{
					...
					else
					{
						...
						if (!from.PlaceInBackpack(wax))
						{
							wax.Delete();
							from.PrivateOverheadMessage(0, 1154, false,
								"Nie masz miejsca w plecaku, by wyciagnac wosk!", from.NetState);
							return;
						}
						...
						from.PrivateOverheadMessage(0, 1154, false,
							"Oprozniles garnek i umiesciles czysty wosk pszczeli w plecaku.", from.NetState);
					}
				}
				else
					from.PrivateOverheadMessage(0, 1154, false,
						"Mozesz topic jedynie czysty wosk pszczeli w tym garnku.", from.NetState);
			}
			else
			{
				from.PrivateOverheadMessage(0, 1154, false, "Wosk musi znajdowac sie w twoim plecaku by moc go stopic.",
					from.NetState);
			}
		}
	...
	{
		private readonly apiLargeWaxPot m_pot;
