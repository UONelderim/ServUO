...
namespace Server.Items
{
	public class apiSmallWaxPot : Item
	{
		...
		public void BeginAdd(Mobile from)
		{
			if (m_RawBeeswax < MaxWax)
				...
				from.PrivateOverheadMessage(0, 1154, false, "Garnek nie moze zmiescic wiecej nieprzetworzonego wosku.",
					from.NetState);
		}
		...
		public void EndAdd(Mobile from, object o)
		{
			if (o is Item && ((Item)o).IsChildOf(from.Backpack))
			{
				if (o is RawBeeswax)
				{
					...
					from.PrivateOverheadMessage(0, 1154, false, "Umiesciles nieprzetworzony pszczeli wosk w garnku.",
						from.NetState);
					...
					from.PrivateOverheadMessage(0, 1154, false,
						"Mozesz umiescic jedynie nieprzetworzony wosk pszczeli w tym garnku.", from.NetState);
			}
			else
			{
				from.PrivateOverheadMessage(0, 1154, false,
					"Wosk musi znajdowac sie w twoim plecaku, by moc go umiescic w garnku.", from.NetState);
			}
		}
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			writer.Write(m_RawBeeswax);
			writer.Write(m_PureBeeswax);
		}
	...
	{
		private readonly apiSmallWaxPot m_pot;
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case (int)Buttons.cmdAddRaw: //Add Raw Honey
				{
					...
					{
						from.PrivateOverheadMessage(0, 1154, false,
							"Nie mozesz mieszczac czystego wosku z nieprzetworzonym. Oproznij wpierw garnek.",
							from.NetState);
						return;
					}

					from.PrivateOverheadMessage(0, 1154, false,
						"Wybierz nieprzetworzony wosk pszczeli, ktory chcesz umiescic w tym garnku.", from.NetState);
					m_pot.BeginAdd(from);
				...
				case (int)Buttons.cmdRenderWax: //render the wax
				{
					...
					{
						//no uses remaining
						from.PrivateOverheadMessage(0, 1154, false, "Garnek jest zbyt zniszczony by go uzywac.",
							from.NetState);
						from.SendGump(new apiBeeHiveSmallPotGump(from, m_pot));
						return;
					}

					if (m_pot.PureBeeswax > 1)
					{
						//already rendered
						from.PrivateOverheadMessage(0, 1154, false, "Garnek jest pelen czystego wosku pszczelego.",
							from.NetState);
						from.SendGump(new apiBeeHiveSmallPotGump(from, m_pot));
						return;
					}

					if (m_pot.RawBeeswax < 10)
					{
						//not enough raw beeswax
						from.PrivateOverheadMessage(0, 1154, false,
							"Nie ma wystarczajaco duzo nieprzetworzonego wosku w garnku.", from.NetState);
						from.SendGump(new apiBeeHiveSmallPotGump(from, m_pot));
						return;
					}

					if (!BeeHiveHelper.Find(@from, BeeHiveHelper.m_HeatSources))
					{
						//need a heat source to melt the wax
						@from.PrivateOverheadMessage(0, 1154, false,
							"Musisz byc blisko zrodla ciepla, by wyczyscic wosk.", @from.NetState);
						@from.SendGump(new apiBeeHiveSmallPotGump(@from, m_pot));
						return;
					}
					...
					from.PlaySound(0x21);
					from.PrivateOverheadMessage(0, 1154, false,
						"Powoli topisz wosk i usuwasz z niego zanieczyszczenia.", from.NetState);
