...
{	
	public class apiBeeHiveMainGump : Gump
	{
		...
		public apiBeeHiveMainGump( Mobile from, apiBeeHive hive ) : base( 20, 20 )
		{
			...
			if( hive.HiveStage >= HiveStatus.Producing )
				AddLabel(90, 42, 92, "Kolonia : "+hive.Population.ToString()+"0k" );
			else if( hive.HiveStage >= HiveStatus.Brooding )
				AddLabel(90, 42, 92, "   Rozmnazanie");
			else
				AddLabel(90, 42, 92, "  Kolonizowanie");
			switch( hive.OverallHealth ) //overall health
			{
				case HiveHealth.Dying: AddLabel(116, 146, 37, "Umierajace"); break;
				case HiveHealth.Sickly: AddLabel(116, 146, 52, "Chore"); break;
				case HiveHealth.Healthy: AddLabel(116, 146, 67, "Zdrowe"); break;
				case HiveHealth.Thriving: AddLabel(116, 146, 92, "Prosperujace"); break;
			}  
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			{
				m_hive.LabelTo( from, "Nie mozesz tego uzyc." );
				return;
			}
	...
	public class apiBeeHiveProductionGump : Gump
	{
		...
		public apiBeeHiveProductionGump( Mobile from, apiBeeHive hive ) : base( 20, 20 )
		{
			...
			AddLabel(110, 43, 92, "Produkcja");  //title

            AddLabel(47, 60, 92, "Powiekszony wplyw kwiatow: " + m_hive.FlowersPower.ToString());
            ...
            AddLabel(90, 150, 70 + 10*(int)m_hive.FlowersQuarter, "Miod: " + m_hive.FlowersQuarter.ToString());  //rodzaj miodu
		}
			...
			switch ( info.ButtonID )
			{
				...
				case (int)Buttons.butHoney: //Honey
				{
					...
					if( NeedHiveTool )
					{
						...
						{
							m_hive.LabelTo( from, "Potrzebujesz narzedzia bartniczego zeby wydobyc miod!" );
							from.SendGump( new apiBeeHiveProductionGump( from, m_hive ) );
							return;
						}
					}
					...
					if( m_hive.Honey < 3 )
					{
						m_hive.LabelTo( from, "Nie ma wystarczajaco duzo miodu, by zapelnic sloj!" );
						from.SendGump( new apiBeeHiveProductionGump( from, m_hive ) );
						break;
					}
					...
					if ( pack != null && pack.ConsumeTotal( typeof( Bottle ), 1 ) )
					{
                        ...
                        switch (m_hive.FlowersQuarter)
                        {
                            case HoneyType.Gryczany:
                                honey = new JarHoneyGryczany();
                                break;
                            case HoneyType.Lesny:
                                honey = new JarHoneyLesny();
                                break;
                            case HoneyType.Spadziowy:
                                honey = new JarHoneySpadziowy();
                                break;
                            case HoneyType.Nostrzykowy:
                                honey = new JarHoneyNostrzykowy();
                                break;
                            default:
                                break;
                        }						
						...
						if ( !from.PlaceInBackpack( honey ) )
						{
							...
							m_hive.LabelTo( from, "Nie masz miejsca w plecaku!" );
							from.SendGump( new apiBeeHiveProductionGump( from, m_hive ) );
							break;
						}
						...
						if( NeedHiveTool )
						{
							...
							{
								from.SendMessage("Skonczyly ci sie narzedzia bartnicze.");
								hivetool.Delete();
							}
						}

						m_hive.Honey -= 3;
						m_hive.LabelTo( from, "Wypelniles butelke miodem i wlozyles do plecaka." );
						from.SendGump( new apiBeeHiveProductionGump(from,m_hive) );
						break;
					}
					else
					{
						m_hive.LabelTo( from, "Potrzebujesz butelki, by napelnic ja miodem!" );
						from.SendGump( new apiBeeHiveProductionGump( from, m_hive ) );
						break;
					}
				}
				case (int)Buttons.butWax: //Wax
				{
					...
					if( NeedHiveTool )
					{
						if( hivetool == null || !(hivetool is HiveTool) )
						{
							m_hive.LabelTo( from, "Potrzebujesz narzedzia bartnieczego, by wydobyc wosk!" );
							from.SendGump( new apiBeeHiveProductionGump( from, m_hive ) );
							return;
						}
					}
					...
					if( m_hive.Wax < 1 )
					{
						m_hive.LabelTo( from, "Nie ma wystarczajaco duzo wosku, by rozpoczac jego wydobycie!" );
						return;
					}
					...
					if( PureWax )
					{
						wax = new PureRawBeeswax(m_hive.Wax);
					}
					else
					{
					...
					m_hive.Wax = 0;
                    m_hive.LabelTo(from, "Zebrales wosk i wlozyles go do plecaka.");
					from.SendGump( new apiBeeHiveProductionGump(from,m_hive) );
					break;
				}
			}
	...
	public class apiBeeHiveDestroyGump : Gump
	{
		...
		public apiBeeHiveDestroyGump( Mobile from, apiBeeHive hive ) : base( 20, 20 )
		{
			...
			AddLabel(84, 43, 92, "Zniszczyc ten ul?");
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if( !m_hive.IsAccessibleTo( from ) )
			{
				m_hive.LabelTo( from, "Nie mozesz tego zrobic." );
				return;
			}
			...
			switch ( info.ButtonID )
			{
				...
				case (int)Buttons.butOkay: //okay
				{
					...
					if ( !from.PlaceInBackpack( deed ) )
					{
						...
						m_hive.LabelTo( from, "Nie mozesz zniszczyc ula, kiedy nie masz miejsca w plecaku!" );
						from.SendGump( new apiBeeHiveMainGump( from, m_hive ) );
