...
namespace Server.Mobiles
{
	public class ArenaTrainer : BaseVendor
	{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				try
				{
					if ( targeted is PlayerMobile && !( targeted as PlayerMobile ).Young )
					{
						...
						{
							string args = String.Format( "{0}\t{1}", m_Atacker.Name, ( m_FightType == FightType.ShortDuel ) ? "krotki" : "dlugi" );
							
							op.SendLocalizedMessage( 00505178, args ); // ~1_NAME~ wyzywa Cie na ~2_TYPE~ pojedynek. Wskaz wyzywajacego, jesli akceptujesz wyzwanie. 
		...
		private class AcceptTarget : Target
		{
			...
			public AcceptTarget( ArenaTrainer owner, Mobile atacker, Mobile opponent, FightType ft ) :  base ( 4, false, TargetFlags.None )
			{
				m_Owner = owner;
				m_FightType = ft;
				m_Atacker = atacker;
				...
				try
				{
					if ( targeted is Mobile && ( targeted as Mobile) == m_Atacker )
					{
						...
						if ( abox != null && obox != null 
						    && !( ( ArenaTrainer.GetBalance( m_Atacker ) < price ) 
						         || ( ArenaTrainer.GetBalance( m_Opponent ) < price ) ) )
						{
							ArenaTrainer.Withdraw( m_Atacker, price );
							ArenaTrainer.Withdraw( m_Opponent, price );
					...
					case 0:
						{
							...
							catch
							{
								...
								m_Owner.Say( "Nie rozumiem daty. Ustalam termin turnieju na " 
								    + NelderimDateTime.TransformToString( m_Owner.TournamentStart, NelderimDateTimeFormat.LongIs ) );
					...
					case 3:
						{
							...
							if ( box == null || !ArenaTrainer.Withdraw( from, cost ) )
		...
		public class TrainingEntry : ContextMenuEntry
		{
			...
			public TrainingEntry( Mobile m, Mobile from, bool longTraining ) : base(  50006 + ( ( longTraining ) ? 1 : 0 ), 6 ) // Krotki & Dlugi trening 
			{
				m_Mobile = m;
				m_From = from;
				m_Long = longTraining;
		...
		public class DuelEntry : ContextMenuEntry
		{
			...
			public DuelEntry( Mobile m, Mobile from, bool longDuel ) : base( 50004 + ( ( longDuel ) ? 1 : 0 ), 6 ) // Krotki & Dlugi pojedynek
			{
				m_Mobile = m;
				m_From = from;
				m_Long = longDuel;
		...
		public ArrayList Competitors { get { return m_Competitors; } }
		...
		{ 
			...
			set 
			{
				...
				if ( m_TournamentState == TournamentState.Configuration )
				{
					m_TournamentMinNumberOfCompetitors = ( value <= 4 ) ? 4 : ( value <= 8 ) ? 8 : ( value <= 16 ) ? 16 : ( value <= 32 ) ? 32 : ( value <= 64 ) ? 64 : 128;
...
//				    && ( m_TournamentStart < DateTime.Now + TimeSpan.FromSeconds( ( int ) TournamentIntervals.Day ) ) )
				...
				{
					Say( ( m_TournamentAutoStart ) ? 00505175 : 00505176 ); // "Sam zaczne! Nie ruszaj!" : "Jest za wczesnie. Nie rozpoczne turnieju."
					return;
				}
		...
		{ 
			get 
			{ 
				return ( m_Tournament != null ) ? m_Tournament.Progress : TournamentProgress.None; 
			} 
		}
		
		[CommandProperty( AccessLevel.Counselor )]
		public int TrnCompCount { get { return m_Competitors.Count; } }
		...
		public NArenaRegion Arena { get { return m_Arena; } }
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( m_Active )
			{
				m_Arena = new NArenaRegion( "arena", this.Map, null, this );
				m_Arena.Owner = this;
				ActivateArena();
			}
		...
		public override void AddCustomContextEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			try
			{
				if ( from.Alive )
				{
					...
					if ( m_TournamentState == TournamentState.Finished || ( int ) TrnState < ( int ) TournamentState.Initialization )
					...
					if ( ( int ) m_TournamentState > ( int ) TournamentState.Configuration )
					{
						...
						if ( m_TournamentState == TournamentState.InProgress && ( int ) TrnStateProgress < ( int ) TournamentProgress.Fight )
						{
				...
				if ( m_Active && !e.Handled && e.Mobile.InRange( this, 3 ) )
				{
					...
					if ( Regex.IsMatch( e.Speech, "zasad", RegexOptions.IgnoreCase ) )
					{
						if (CheckVendorAccess(from)) {
							e.Handled = true;
							if (e.Speech.ToLower() == "zasad" || Regex.IsMatch(e.Speech, "^zasad.$", RegexOptions.IgnoreCase)) OnLazySpeech();
							else {
								try {
					...
					else if ( Regex.IsMatch( e.Speech, "turniej", RegexOptions.IgnoreCase ) )
					{
						if ( CheckVendorAccess( from ) )
						{
							           ...
							           ( int ) TrnStateProgress < ( int ) TournamentProgress.Fight ) ) )
							{
								...
								else if ( box != null )
								{
									if ( m_TournamentFee > 0 )
										Say( 00505192, m_TournamentFee.ToString() ); // Zgloszenie do turnieju kosztuje Cie ~1_GOLD~ centarow. Do zobaczenia w dniu zawodow!
									else 
							...
							else if ( m_TournamentState == TournamentState.InProgress && ( !Regex.IsMatch( e.Speech, "potwierdz", RegexOptions.IgnoreCase ) ) )
							{
								Say( 00505172 ); // Turniej wlasnie trwa!
							}
							else if ( Regex.IsMatch( e.Speech, "wycof", RegexOptions.IgnoreCase ) && m_TournamentState == TournamentState.Recrutation )
							{
						...
						if ( CheckVendorAccess( from ) )
						{
							...
							else if ( Regex.IsMatch( e.Speech, "krotki", RegexOptions.IgnoreCase ) )
							{
								...
								else if ( box != null )
								{
									...
									this.Say( 505208, args ); // ~1_TYPE~ trening kosztuje Cie ~2_GOLD~ centarow. Mozesz wkroczyc na arene.
		...
		private void RefundCompetitors()
		{
			try
			{
				for ( int i = 0; m_Competitors != null && i < m_Competitors.Count; i++ )
					Banker.Deposit( ( m_Competitors[ i ] as TournamentCompetitor ).Competitor, this.m_TournamentFee );
		...
		public string GetTournamentStatus()
		{
			...
			try
			{
				...
				if ( TrnReward1st> 0 && TrnRewardName == null )
					status += " o nagrode w wysokosci " + this.TrnReward1st.ToString() + " sztuk zlota";
				else if ( TrnReward1st> 0 )
					status += " oraz " + this.TrnReward1st.ToString() + " sztuk zlota";
				...
				if ( TrnFee > 0 )
				{
					...
					status += " po osmej czesci zebranego wpisowego, ktore wynosi jedyne " + TrnFee.ToString() + ".\n";
				}
				
				if ( this.TrnReward2nd > 0 )
				{
					status += " Drugi zawodnik turnieju otrzyma nagrode w wysokosci " + TrnReward2nd.ToString() + " centarow.\n";
				}
				
				if ( this.TrnReward3rd > 0 )
				{
					status += " Trzeci zawodnicy otrzymaja po " + TrnReward3rd.ToString() + " centarow.\n";
				}
				...
				{
					status += "swoj udzial zglosilo " + m_Competitors.Count + " zawodnikow (w kolejnosci zgloszen): \n\n";
		...
		private bool ActivateArena()
		{
			try
			{
				...
				else 
				{
					...
					m_Arena = new NArenaRegion( "arena", this.Map, new Rectangle3D[]{ rect }, this );

                    if ( m_Arena.Contains( m_EndOfFightPoint ) || m_Arena.Contains( m_ExtortPoint )
                       || !m_Arena.Contains( m_BlueCorner ) || !m_Arena.Contains( m_RedCorner ) )
                    {
