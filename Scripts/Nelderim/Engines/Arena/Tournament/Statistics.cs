...
namespace Server.Engines.Tournament
{
	public class RankComparer : IComparer
	{
		...
		public int Compare( object left, object right )
		{
			...
			if ( l.Place1 > r.Place1 )
				return -1;
			else if ( l.Place1 < r.Place1 ) 
				return 1;
				
			if ( l.Place2 > r.Place2 )
				return -1;
			else if ( l.Place2 < r.Place2 ) 
				return 1;
				
			if ( l.Place3 > r.Place3 )
				return -1;
			else if ( l.Place3 < r.Place3 ) 
				return 1;
	...
	public class TournamentRecord
	{
		...
		public TournamentRecord( Tournament t )
		{
			...
			for( int i = 1; i <= 4; i ++ )
			{
				...
				switch ( i )
				{
					case 1: m_1stName = name; break;
					case 2: m_2ndName = name; break;
					case 3: m_3rdName = name; break;
					case 4: m_4thName = name; break;
				}
			}
		}
		...
		public override string ToString()
		{
			...
			switch ( m_Class )
			{
				case TournamentClass.Normal: output += "Turniej klasyczny\n";break;
				case TournamentClass.Masters: output += "Turniej mistrzow\n";break;
				case TournamentClass.Open: output += "Turniej otwarty\n";break;
				case TournamentClass.Private: output += "Turniej prywatny\n";break;
			}
			
			output += "Poczatek turnieju: " + ( new NelderimDateTime( m_StartDate ) ).ToString( NelderimDateTimeFormat.LongIs ) + "\n";
	...
	public class TournamentCompetitorRecord
	{
		...
		public int Place1 { get { return m_1sts; } }
		public int Place2 { get { return m_2nds; } }
		public int Place3 { get { return m_3rds; } }
		public Serial Serial { get { return m_Serial; } } 
		
		public TournamentCompetitorRecord()
		{
			m_Serial = new Serial();
			m_Status = 0;
			m_1sts = 0;
	...
	public class TournamentStatistics
	{
		...
		private static void Clean()
		{
			try
			{
				...
				{
					TournamentCompetitorRecord tcr =  m_Competitors[ i ] as TournamentCompetitorRecord;
		...
		public static string PrintRecords( int count )
		{
			...
			{
				TournamentRecord tcr = m_Records[ i ] as TournamentRecord;
		...
		public static void WriteHTML()
		{
			try
			{
				using ( StreamWriter op = new StreamWriter( "Logs/Turnieje/ranks.html" ) )
				{
					...
					op.WriteLine( "      Calkowita liczba uczestnikow wszystkich turniejow Nelderim: {0}<br>", m_Competitors.Count );
					op.WriteLine( "      <table width=\"100%\">" );
					op.WriteLine( "         <tr>" );
					op.WriteLine( "            <td bgcolor=\"black\"><font color=\"white\">nr</font></td><td bgcolor=\"black\"><font color=\"white\">Imie</font></td><td bgcolor=\"black\"><font color=\"white\">Punkty</font></td><td bgcolor=\"black\"><font color=\"white\">1 miejsc</font></td><td bgcolor=\"black\"><font color=\"white\">2 miejsc</font></td><td bgcolor=\"black\"><font color=\"white\">3 miejsc</font></td>" );
					op.WriteLine( "         </tr>" );

					for ( int i = 0; i < m_Competitors.Count; i++ )
					{
						TournamentCompetitorRecord rec = m_Competitors[i] as TournamentCompetitorRecord;
						rec.ToHTML( op, i+1 );
					}
				...
				using ( StreamWriter op = new StreamWriter( "Logs/Turnieje/history.html" ) )
				{
					...
					{
						TournamentRecord rec = m_Records[i] as TournamentRecord;
						rec.ToHTML( op );
					}
