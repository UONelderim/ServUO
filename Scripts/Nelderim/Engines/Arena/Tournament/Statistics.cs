...
namespace Server.Engines.Tournament
{
	...
	public class TournamentRecord
	{
		...
		DateTime m_StopDate;
		readonly string m_ArenaName;
		readonly string m_TournamentName;
		readonly string m_RegionName;
		readonly string m_RewardName;
		readonly string m_1stName;
		readonly string m_2ndName;
		readonly string m_3rdName;
		readonly string m_4thName;
		readonly TournamentClass m_Class;
	...
	public class TournamentCompetitorRecord
	{
		...
		public int Place1 { get; private set; }
		public int Place2 { get; private set; }
		public int Place3 { get; private set; }
		...
		public TournamentCompetitorRecord(XmlElement doc)
		{
			try
			{
				...
				Status = XmlConvert.ToInt32(xml.GetAttribute("value"));

				xml = doc.GetElementsByTagName("place1").Item(0) as XmlElement;
				Place1 = XmlConvert.ToInt32(xml.GetAttribute("value"));

				xml = doc.GetElementsByTagName("place2").Item(0) as XmlElement;
				Place2 = XmlConvert.ToInt32(xml.GetAttribute("value"));

				xml = doc.GetElementsByTagName("place3").Item(0) as XmlElement;
				Place3 = XmlConvert.ToInt32(xml.GetAttribute("value"));
			}
			catch (Exception exc)
			{
				Console.WriteLine(exc.ToString());
		...
		public override string ToString()
		{
			return (Competitor != null)
				? Competitor.Name + " [" + Status + "] (" + Place1 + "-" + Place2 + "-" + Place3 + ")"
				: "brak danych";
		}
		...
		public bool Write(XmlTextWriter xml)
		{
			try
			{
				...
				xml.WriteAttributeString("value", Status.ToString());
				xml.WriteEndElement();

				xml.WriteStartElement("place1");
				xml.WriteAttributeString("value", Place1.ToString());
				xml.WriteEndElement();

				xml.WriteStartElement("place2");
				xml.WriteAttributeString("value", Place2.ToString());
				xml.WriteEndElement();

				xml.WriteStartElement("place3");
				xml.WriteAttributeString("value", Place3.ToString());
				xml.WriteEndElement();
	...
	{
		private static readonly ArrayList m_Records;
		private static readonly ArrayList m_Competitors;
		...
		private static bool Save()
		{
			try
			{
				...
				XmlTextWriter xml = new XmlTextWriter("Data/Tournaments.xml", Encoding.UTF8);
				xml.Indentation = 1;
				xml.IndentChar = '\t';
		...
		public static bool UpdateRank(Mobile mob, int round, int fights, bool won, TournamentClass tclass)
		{
			...
			int rounds = (int)Math.Round(Math.Log(fights + 1, 2));
			int place = (won) ? 1 : (round == rounds) ? 2 : (round == rounds - 1) ? 3 : 0;
			int rank = (int)(Math.Pow(2, round - 1) * ((won) ? 2 : 1));
