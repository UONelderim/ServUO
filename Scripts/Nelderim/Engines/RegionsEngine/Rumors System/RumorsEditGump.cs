					...
					case PageName.List:
						{
							...
							{
								AddHtml( 10, 80, 530, 20, Center( Color("W regionie brak jakichkolwiek plotek" ) ), false, false ); //<CENTER>W regionie brak jakichkolwiek plotek</CENTER>
							}
							else
							{
								if ( listPage > 0 )
								...
								for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_RumorsList.Count; ++i, ++index )
								{
									RumorRecord rum = (RumorRecord) m_RumorsList[ index ];
									...
									AddHtml( 320, y + ( i * 20 ) ,80, 20, Color( rum.StartRumor.ToString() , color ), false, false );
									AddHtml( 415, y + ( i * 20 ) ,80, 20, Color( rum.EndRumor.ToString(),  color ), false, false );
									AddButton( 510, y + ( i * 20 ), 0xFA5, 0xFA7, GetButtonID( 1, index + 2 ), GumpButtonType.Reply, 0 );
								}
							}
							break;
					}
					...
					case PageName.Paste:
						{
							...
							if( m_RumorsList.Count <= 0 || m_RumorsList == null )
							{
								AddHtml( 10, 80, 530, 20, Color( "W regionie brak jakichkolwiek plotek" ), false, false ); //<CENTER>W regionie brak jakichkolwiek plotek</CENTER>
							}
							else
							{
								if ( listPage > 0 )
								...
								for ( int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_RumorsList.Count; ++i, ++index )
								{
									...
									{
										string regname = (string)rum.Regions[j];
									...
									AddButton( 510, y + ( i * 20 ), 0xFA5, 0xFA7, GetButtonID( 3, index + 2 ), GumpButtonType.Reply, 0 );
								}
							}
							break;
					}
					...
					case PageName.Edit:
						{
							...
							string[] phrases = ( rumor != null ) ? rumor.Text.Split( Separator ) : new string[] { "" };
							...
							AddHtml( 485, y + 125, 20, 20, Center( Color( ( m_ListPage + 1 ).ToString(), LabelColor32 ) ), false, false ); // numer strony
							...
							AddTextEntry( x + 2, y + 2, 140 - 4, 20 - 4, 0, 3, rumor != null ? rumor.StartRumor.ToString() : DateTime.Now.ToString() );
							
							x += 300;
							// Koniec
							AddBackground( x - 2, y - 2, 140 + 4, 20 + 4, 0x2486 );
							AddTextEntry( x + 2, y + 2, 140 - 4, 20 - 4, 0, 4, rumor != null ? rumor.EndRumor.ToString() : DateTime.Now.ToString() );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			try
			{
				...
				if( info.ButtonID == 0 )
				{
					switch ( m_Page )
					{
						case PageName.New: case PageName.Paste:
							state.Mobile.SendGump( new RumorsEditGump( m_From, m_Region, "", PageName.List, null, 0 ) ); break;
						default:
							state.Mobile.SendGump( new RegionsGump( state.Mobile ) ); break;
					}
					...
					case 0:
						{
							...
							switch( index )
							{
								case 1 : page = PageName.List; rumor = false; break;
								case 2 : page = PageName.New; rumor = false; break;
								case 3 : page = PageName.Edit; rumor = true; break;
								case 5 : page = PageName.Paste; rumor = false; break;
								case 4 : 
									{
										if( m_Rumor != null )
					...
					case 1:
					{
						switch( index )
						{
							...
							default:
							{
								...
								{
									RumorRecord record = (RumorRecord)m_RumorsList[index];
							...
							default:
							{
								...
								if( m_RumorsList != null && index >= 0 && index < m_RumorsList.Count )
								{
									...
									{	
										...
										information = "Wstawiles plotke do nowego regionu"; // Wstawiles plotke do nowego regionu
									}
									catch( Exception e )
									{
                                        information = "Wystapil wyjatkowy blad skryptu"; // Wystapil wyjatkowy blad skryptu
					...
					case 2:
						{
							if ( index == 7 )
							{
								...
								from.SendGump( new RumorsEditGump( from, m_Region, "Pozycja zostala skopiowana i dolaczona do regionu glownego", PageName.List ) ); // Pozycja zostala skopiowana i dolaczona do regionu glownego
							}
							else if ( index == 6 )
							{
								m_Rumor.AddRegionExclude( m_Region.Name );
								from.SendGump( new RumorsEditGump( from, m_Region, "Wylaczyles pozycje dla wskazanego regionu", PageName.List ) ); // Wylaczyles pozycje dla wskazanego regionu
							...
							else
							{
								...
								string[] phrases = ( !newRecord ) ? m_Rumor.Text.Split( Separator ) : new string[] { "" };
								...
								{
									case 0: 
										from.SendGump( new RumorsEditGump( from, m_Region, "Pierwsza strona", PageName.Edit, m_Rumor, m_ListPage - 1 ) ); break;
									case 1: 
										from.SendGump( new RumorsEditGump( from, m_Region, "Druga strona", PageName.Edit, m_Rumor, m_ListPage + 1 ) ); break;
									case 8:
										{
									...
									default:
										from.SendGump( new RumorsEditGump( from, m_Region, infromation, page, m_Rumor, 0 ) ); TownCrier.UpdateNews(); break;
								}
							}
						break;
					}
