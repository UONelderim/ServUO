...
namespace Server.Nelderim
{
    public class RegionsEngine
    {
                ...
                foreach ( XmlElement reg in root.GetElementsByTagName( "region" ) )
                {
                    ...
                    try
                    {
                        ...
                        if ( nodes.Count > 0 )
                        {
                            ...
                            if ( sum != 100 )
                                Console.WriteLine( "Suma prawdopodobienstw dla regionu {0} jest niepoprawna i wynosi {1}", name, sum );

                            newRegion.RegionPopulation = new Population( population );
                        }
                    } catch ( Exception e )
                    {
                        Console.WriteLine( e.ToString() );
                    ...
                    try
                    {
                        ...
                        foreach ( XmlElement guard in g.GetElementsByTagName( "guard" ) )
                        {
                            ...
                            if ( sum != 100 )
                                Console.WriteLine( "[Guards] Suma prawdopodobienstw dla regionu {0} jest niepoprawna i wynosi {1}", name, sum );
        ...
        public static bool Save()
        {
            ...
            try
            {
                XmlTextWriter xml = new XmlTextWriter( "Data/NelderimRegions/NelderimRegions.xml", System.Text.Encoding.UTF8 );
                xml.Indentation = 1;
                xml.IndentChar = '\t';
                    ...
                    if ( reg.Guards != null )
                    {
                        ...
                        for ( int i = 0; i < reg.Guards.Length; i++ )
                        {
                            ...
                            if ( g != null )
                            {
                                ...
                                xml.WriteAttributeString( "file", g.FileName.ToString() );
        ...
        public static bool PetIsBanned( string regionName, BaseCreature pet )
        {
            try
            {
                ...
                if ( (pet is SummonedAirElemental || pet is SummonedDaemon || pet is SummonedEarthElemental || pet is SummonedFireElemental || pet is SummonedWaterElemental) && reg.SummonsAreBanned )
                    return true;
            } catch ( Exception e )
            {
                Console.WriteLine( e.ToString() );
        ...
        public static void RandomWarningMsg( Mobile source, Mobile target )
        {
            switch ( Utility.Random( 7 ) )
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    source.Yell( 00505143, target.Race.GetName( Cases.Wolacz ) ); // Lepiej opusc ta okolice ~1_NAME~! Moze Cie spotkac krzywda!
                    break;
                case 3:
