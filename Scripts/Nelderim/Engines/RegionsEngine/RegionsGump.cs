...
namespace Server.Nelderim
{
	public class RegionsGump : Gump
	{
		...
		{
			CommandSystem.Register("Regions", AccessLevel.Counselor, RegionsList_OnCommand);
			CommandSystem.Register("RegionsList", AccessLevel.Counselor, RegionsList_OnCommand);
		}
		...
		private static readonly int ReturnWidth = 24;
		private static readonly int ReturnButtonID1 = 2223; // 2224
		private static readonly int ReturnButtonID2 = 2223;

		private static readonly int NextRegWidth = 24;
		private static readonly int NextRegButtonID1 = 2224; // 2224
		private static readonly int NextRegButtonID2 = 2224;
		...
		private static readonly int TotalWidth = OffsetSize + EntryWidth + OffsetSize + SetWidth + OffsetSize +
		                                         NextWidth + OffsetSize + PrevWidth + OffsetSize;

		private static readonly int TotalHeight =
			OffsetSize + EntryHeight + OffsetSize + ((EntryHeight + OffsetSize) * (EntryCount + 1));
		...
		public void Initialize(string parent, int page)
		{
			...
			AddLabel(x + TextOffsetX, y, TextHue,
				String.Format("Page {0} of {1} ({2})", page + 1, (m_Regions_List.Count + EntryCount - 1) / EntryCount,
					m_Regions_List.Count));
			...
			for (int i = 0, index = page * EntryCount; i < EntryCount && index < m_Regions_List.Count; ++i, ++index)
			{
				...
				if (CheckSecondaryRegion(reg.Name))
					AddButton(x + SetOffsetX, y + SetOffsetY, NextRegButtonID1, NextRegButtonID2,
						i + 6 + (EntryCount * 2), GumpButtonType.Reply, 0);
			}
		}
