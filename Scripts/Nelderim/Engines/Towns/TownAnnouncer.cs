    ...
    class TownAnnouncer
    {
        ...
        static void CalculateResourcePricesToBuyAndSell()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                do
                {
                    resource = new TownResource(resources.Resources[Utility.RandomMinMax(0, resources.Resources.Count - 1)].ResourceType, 0, 0, 0);
                    resource.Amount = resources.Resources.Find(obj => obj.ResourceType == resource.ResourceType).Amount;
                } while (m_foreignResourcesToBuy.Find(obj => obj.ResourceType == resource.ResourceType) != null);
                m_foreignResourcesToBuy.Add(resource);
                do
                {
                    resource = new TownResource(resources.Resources[Utility.RandomMinMax(0, resources.Resources.Count - 1)].ResourceType, 0, 0, 0);
                    resource.Amount = resources.Resources.Find(obj => obj.ResourceType == resource.ResourceType).MaxAmount;
                } while (m_foreignResourcesToSell.Find(obj => obj.ResourceType == resource.ResourceType) != null);
                    ...
                    foreach (TownBuilding tb in TownDatabase.GetBuildingsList(tm))
                    {
                        if (tb.Status == TownBuildingStatus.Dziala)
                        {
                            foreach (TownResource tr in tb.Resources.Resources)
                            {
                                ...
                                {
                                    tmpTown.Resources.ResourceDailyChangeIncrease(tr.ResourceType, (int)(-1 * tr.Amount * 0.001)); // Jesli charge ustawiony, oplata za budynek bedzie rowna 1 promil podstawowej oplaty
                                }
                            }
                        }
                    }
                            ...
                            foreach (TownBuilding tb in TownDatabase.GetBuildingsList(tm))
                            {
                                if (tb.Status == TownBuildingStatus.Dziala)
                                {
                                    if (tb.Charge)
                                    {
                                        ...
                                        foreach (TownResource tr in tb.Resources.Resources)
                                        {
                                            if (tmpTown.Resources.ResourceAmount(tr.ResourceType) < (int)(1 * tr.Amount * 0.001))
                                            {
                                        ...
                                        else
                                        {
                                            foreach (TownResource tr in tb.Resources.Resources)
                                            {
                                                ...
                                                tmpTown.Resources.ResourceDecreaseAmount(tr.ResourceType, (int)(1 * tr.Amount * 0.001));
                                            }
                                        }
                                    }
                                }
