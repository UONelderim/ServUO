...
namespace Server.Gumps
{
    public class TownTownsExchangeGump : Gump
    {
        ...
        private Towns m_town, m_townToSend;
            ...
            : base(50, 40)
        {
            ...
            if (main)
            {
                AddLabelCropped(10, 30, 300, 40, SelectedColor, string.Format("Wyslanie kazdej karawny kosztuje 1000 zlota."));
                AddLabel(10, 70, SelectedColor, string.Format("Wybierz miasto ktore ma otrzymac surowce"));
                int yLab = 90;
            ...
            {
                AddLabel(10, 30, SelectedColor, string.Format("Wyslanie surowcow do miasta {0}", m_townToSend.ToString()));
                int yLab = 80;
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            else
            {
                ...
                if (val == 300)
                {
                    from.SendGump(new TownTownsExchangeGump(m_town, from, true));
                    return;
                }
                else
                {
                    ...
                    if (twojeZasoby.HasResourceAmount(TownResourceType.Zloto, 1000))
                    {
                        ...
                        if (res == TownResourceType.Zloto && !twojeZasoby.HasResourceAmount(TownResourceType.Zloto, 1000 + amount))
                        {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if ((res == TownResourceType.Zloto && zasobyMiastaDocelowego.IsResourceAmountProper(TownResourceType.Zloto, amount + 1000)) ||
                                    zasobyMiastaDocelowego.IsResourceAmountProper(res, amount))
                                {
                                    ...
                                    TownDatabase.AddTownLog(m_town, TownLogTypes.SUROWCE_WYSLANO, "", amount, (int)(res), (int)m_townToSend);
                                    TownDatabase.AddTownLog(m_townToSend, TownLogTypes.SUROWCE_OTRZYMANO, "", amount, (int)(res), (int)m_town);
