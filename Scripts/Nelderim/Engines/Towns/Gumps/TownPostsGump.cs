    ...
    public class TownPostsGump : Gump
    {
            ...
            : base(50, 40)
        {
            ...
            else if (page == 2)
            {
                ...
                AddHtmlLocalized(10, 120, 250, 250, 1063913, LabelColor, false, false); // Lacznie aktywnych dni straznika
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
                ...
                if (from.Region.ToString() == m_fromTown.ToString())
                {
                    ...
                    if (TownDatabase.GetTown(m_fromTown).Resources.HasResourceAmount(TownResourceType.Zloto, 1000))
                    {
                        if (TownDatabase.GetTown(m_fromTown).TownPosts[val - 1600].IsGuardAlive())
                        {
                            ...
                            TownDatabase.GetTown(m_fromTown).Resources.ResourceDecreaseAmount(TownResourceType.Zloto, 1000);
            ...
            else if (val >= 700) // Wznow posterunek
            {
                ...
                if(tm.HasResourcesForGuard(tm.TownPosts[val - 700].TownGuard, TownBuildingStatus.Zawieszony))
                {
                    ...
                    if (tm.GetActiveGuards() < tm.MaxGuards)
                    {
                        ...
                        TownDatabase.AddTownLog(m_fromTown, TownLogTypes.POSTERUNEK_WZNOWIONO, tm.TownPosts[val - 700].PostName, 0, 0, 0);
                    }
                    else
                    {
                        from.SendLocalizedMessage(1063932); // Ilosc aktywnych straznikow jest zbyt duza, by aktywowac kolejnego straznika.
                    }
                }
                else
	            {
                    from.SendLocalizedMessage(1063933); // Skarbiec miasta nie posiada surowcow, by reaktywowac posterunek.
	            }
            }
            else if (val >= 600) // Zawies posterunek
            {
                ...
                TownDatabase.AddTownLog(m_fromTown, TownLogTypes.POSTERUNEK_ZAWIESZONO, tm.TownPosts[val - 600].PostName, 0, 0, 0);
            }
            else if (val >= 500) // Uruchom posterunek
            {
                ...
                if (tm.HasResourcesForGuard(tm.TownPosts[val - 500].TownGuard, TownBuildingStatus.Dostepny))
                {
                    ...
                    if (tm.GetActiveGuards() < tm.MaxGuards)
                    {
                        ...
                        TownDatabase.AddTownLog(m_fromTown, TownLogTypes.POSTERUNEK_WYBUDOWANO, tm.TownPosts[val - 500].PostName, 0, 0, 0);
                ...
                {
                    from.SendLocalizedMessage(1063934); // Skarbiec miasta nie posiada surowcow, by aktywowac posterunek.
                }
            }
            else if (val >= 400) //Usun posterunek
            {
                TownDatabase.GetTown(m_fromTown).TownPosts[val - 400].PostStatus = TownBuildingStatus.Dostepny;
                TownDatabase.GetTown(m_fromTown).TownPosts.Remove(TownDatabase.GetTown(m_fromTown).TownPosts[val - 400]);
            ...
            else if (val >= 200) // Zmiana nazwy posterunku
            {
                ...
                from.SendGump(new TownPostsGump(m_fromTown, from, 0));
            }
                ...
                switch (val)
                {
                    ...
                    case 2:
                        ...
                        if (from.Region.ToString() == m_fromTown.ToString())
                        {
                            ...
                            {
                                // Sprawdz czy skarbiec posiada zloto                          
                                if (TownDatabase.GetTown(m_fromTown).Resources.HasResourceAmount(TownResourceType.Zloto, 5000))
                                {
                                    // Warunki spelnione, pobierz pieniadze ze skarbca i utworz posterunek
                                    TownDatabase.GetTown(m_fromTown).Resources.ResourceDecreaseAmount(TownResourceType.Zloto, 5000);
...
namespace Server.Prompts
{
    ...
    {
        private int m_whichPost;
        private int m_howMuch;
        private Towns m_whichTown;
