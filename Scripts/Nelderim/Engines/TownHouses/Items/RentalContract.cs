	...
	public class RentalContract : TownHouseSign
	{
		...
		public bool HasContractedArea(Rectangle2D rect, int z)
		{
			foreach (Item item in AllSigns)
				if (item is RentalContract && item != this && item.Map == Map &&
				    ParentHouse == ((RentalContract)item).ParentHouse)
		...
		protected override void DemolishAlert()
		{
			...
			RentalMaster.SendMessage(
				"You have begun to use lockdowns reserved for {0}, and their rental unit will collapse in {1}.",
				c_RentalClient.Name, Math.Round((DemolishTime - DateTime.Now).TotalHours, 2));
			c_RentalClient.SendMessage(
				...
				RentalMaster.Name, Math.Round((DemolishTime - DateTime.Now).TotalHours, 2));
		}
		...
		public bool ValidateLocSec()
		{
			...
			if (General.RemainingLocks(ParentHouse) + Locks < Locks)
				return false;
