...
namespace Knives.TownHouses
{
	...
	public class TownHouseSign : Item
	{
		...
		private bool c_YoungOnly, c_RecurRent, c_Relock, c_KeepItems, c_LeaveItems, c_RentToOwn, c_Free, c_ForcePrivate, c_ForcePublic, c_NoTrade;
		...
		public void UpdateRegion() {
			...
			for (int i = 0; i < House.Region.Area.Length; ++i) {
				...
				rect = new Rectangle3D(new Point3D(rect.Start.X - House.X, rect.Start.Y - House.Y, MinZ), new Point3D(rect.End.X - House.X, rect.End.Y - House.Y, MaxZ));
		...
		{
			c_House.Owner.SendMessage( "Juz nie spelniasz wymagan na ten dom, zostaniesz eksmitowany za {0}h:{1}m:{2}s.", (c_DemolishTime-DateTime.Now).Hours, (c_DemolishTime-DateTime.Now).Minutes, (c_DemolishTime-DateTime.Now).Seconds );
		}
		...
		public void CheckRentTimer()
		{
			...
			c_House.Owner.SendMessage( "Cykl wynajmu konczy sie za {0} dni, {1}h:{2}m:{3}s.", (c_RentTime-DateTime.Now).Days, (c_RentTime-DateTime.Now).Hours, (c_RentTime-DateTime.Now).Minutes, (c_RentTime-DateTime.Now).Seconds );
		}
		...
		public override void Delete()
		{
			if ( c_House == null || c_House.Deleted )
				...
				PublicOverheadMessage( Server.Network.MessageType.Regular, 0x0, true, "Nie mozesz tego usunac, dopoki dom ma wlasciciela." );
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( c_Free )
				...
				list.Add( 1060658, "{0}\t{1}\r{2}", PriceType + (c_RentToOwn ? " Wynajem na wlasnosc" : " Cykliczny"), c_Price, c_KeepItems ? " (+" + c_ItemsPrice + " za przedmioty)" : "" );
			else
				list.Add( 1060658, "Jeden {0}\t{1}{2}", PriceTypeShort, c_Price, c_KeepItems ? " (+" + c_ItemsPrice + " za przedmioty)" : "" );
