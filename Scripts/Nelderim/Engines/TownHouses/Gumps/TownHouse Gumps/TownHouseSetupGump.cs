...
namespace Knives.TownHouses
{
	public class TownHouseSetupGump : GumpPlusLight
	{
		...
		private void SignPage(int width, ref int y)
		{
			...
			AddHtml(0, y += 25, width, "<CENTER>Flip Sign");
			AddButton(width / 2 - 60, y + 3, 0x2716, "Flip Sign", SignFlip);
			AddButton(width / 2 + 50, y + 3, 0x2716, "Flip Sign", SignFlip);
		...
		private void OtherPage(int width, ref int y)
		{
			...
			if (!c_Sign.YoungOnly)
			{
				...
				AddButton(width / 2 - 80, y, c_Sign.Murderers == Intu.No ? 0xD3 : 0xD2, "No Murderers", Murderers,
					Intu.No);
				AddButton(width / 2 + 60, y, c_Sign.Murderers == Intu.No ? 0xD3 : 0xD2, "No Murderers", Murderers,
					Intu.No);
		...
		{
			if (c_Sign.ItemID == (int)SignIDs.TownHouseSignNS)
			{
				c_Sign.ItemID = (int)SignIDs.TownHouseSignWE;
			}
			else
			{
				c_Sign.ItemID = (int)SignIDs.TownHouseSignNS;
			}
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget(TownHouseSetupGump gump, TownHouseSign sign, TargetType type) : this(gump, sign, type,
				Point3D.Zero)
			{
			}

			public InternalTarget(TownHouseSetupGump gump, TownHouseSign sign, TargetType type, Point3D point) : base(
				20, true, TargetFlags.None)
			{
				c_Gump = gump;
				c_Sign = sign;
				c_Type = type;
			...
			protected override void OnTarget(Mobile m, object o)
			{
				...
				switch (c_Type)
				{
					...
					case TargetType.BlockTwo:
						c_Sign.Blocks.Add(FixRect(new Rectangle2D(c_BoundOne,
