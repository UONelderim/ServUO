...
namespace Arya.Jail
{
	...
	public class JailViewGump : Gump
	{
		...
		private readonly JailEntry m_Jail;
		...
		private void MakeGump()
		{
			...
			this.AddHtml(20, 40, 560, 50, String.Format("<basefont color=#CCCCCC>{0}", m_Jail.HistoryRecord), false,
				false);
			...
			if (active)
			{
				...
				this.AddLabel(160, 150, m_Jail.AutoRelease ? GreenHue : RedHue,
					m_Jail.AutoRelease ? "Enabled" : "Disabled");

				// Toggle Auto Release: Button 3
				this.AddButton(290, 150, 4005, 4006, 3, GumpButtonType.Reply, 0);
				this.AddLabel(330, 150, LabelHue,
					String.Format("Turn {0} auto release", m_Jail.AutoRelease ? "off" : "on"));
				...
				{
					this.AddLabel(330, 180, LabelHue,
						String.Format("Turn {0} full account jail", m_Jail.FullJail ? "off" : "on"));
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			switch (info.ButtonID)
			{
				...
				case 2: // Display account admin
					...
					if (m_Jail.Account != null)
					{
						m_User.SendGump(new AdminGump(m_User, AdminGumpPage.AccountDetails_Information, 0, null,
							"Jailed account information", m_Jail.Account));
					}
				...
				case 7: // Update duration
					...
					try
					{
						days = UInt32.Parse(info.TextEntries[0].Text);
					}
					catch { }

					try
					{
						hours = UInt32.Parse(info.TextEntries[1].Text);
					}
					catch { }
					...
					{
						m_User.SendMessage(
							"Invalid duration specified. If you wish to release the player, use the unjail button.");
					}
					else
					{
