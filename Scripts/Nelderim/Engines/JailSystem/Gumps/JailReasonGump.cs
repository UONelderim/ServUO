...
namespace Arya.Jail
{
	...
	public class JailReasonGump : Gump
	{
		...
		/// </summary>
		private readonly int m_JailChoice;
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			try
			{
				...
				switch (info.ButtonID)
				{
					...
					{
						if (m_Offender != null)
							sender.Mobile.SendGump(new JailReasonGump(m_Offender, info.ButtonID,
								info.TextEntries[0].Text));
						else
							sender.Mobile.SendGump(new JailReasonGump(m_Account, info.ButtonID,
								info.TextEntries[0].Text));
						break;
					}
					case 6: // Other, check for text
						...
						else
						{
							...
							if (m_Offender != null)
							{
								sender.Mobile.SendGump(new JailReasonGump(m_Offender, m_JailChoice,
									info.TextEntries[0].Text));
							}
							else
							{
								sender.Mobile.SendGump(new JailReasonGump(m_Account, m_JailChoice,
									info.TextEntries[0].Text));
							}
						}
