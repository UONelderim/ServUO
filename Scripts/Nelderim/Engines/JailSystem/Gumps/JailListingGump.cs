...
namespace Arya.Jail
{
	...
	public class JailListingGump : Gump
	{
		...
		private readonly ArrayList m_MasterList;
		...
		private void MakeGump()
		{
			...
			for (int i = m_Page * 10; i < m_Page * 10 + 10 && i < m_MasterList.Count; i++)
			{
				...
				JailEntry jail = m_MasterList[i] as JailEntry;
				m_Jailings[index] = jail;
					...
					if (jail != null)
					{
						switch (type)
						{
							case 1: // View Information
								...
								m_User.SendGump(new JailViewGump(m_User, jail, JailListingCallback));
								break;
							...
							case 2: // View Account details
								...
								if (jail.Account != null)
								{
									m_User.SendGump(new AdminGump(m_User, AdminGumpPage.AccountDetails_Information, 0,
										null, "Requested by the Jail System", jail.Account));
								}
