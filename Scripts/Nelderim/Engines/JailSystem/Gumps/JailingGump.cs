...
namespace Arya.Jail
{
	...
	public class JailingGump : Gump
	{
		...
		/// </summary>
		private readonly Mobile m_Offender;
		...
		public JailingGump(Mobile offender, Account account, string reason, JailType jail) : this(offender, account,
			reason, jail, true, 1, 0, false, null)
		{
		}

		public JailingGump(Mobile offender, Account account, string reason, JailType jail, bool autorelease, int days,
			int hours, bool fulljail, string comment) : base(100, 100)
		{
			m_Offender = offender;
			m_Account = account;
			m_Reason = reason;
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			try
			{
				...
				try
				{
					...
					{
						m_Days = (int)UInt32.Parse(info.TextEntries[0].Text);
					}
				}
				catch { }
				...
				try
				{
					...
					{
						m_Hours = (int)UInt32.Parse(info.TextEntries[1].Text);
					}
				}
				catch { }
				...
				switch (info.ButtonID)
				{
					case 0: // Cancel jailing
						...
						sender.Mobile.SendMessage(
							"You have canceled the jailing of {0} and brought them to your location", m_Offender.Name);
						return;
					...
					case 1: // Commit jailing
						...
						{
							sender.Mobile.SendMessage(
								"Zaznaczajac opcje autowypuszczenie musisz podac prawidlowy czas trawania kary!");
							sender.Mobile.SendGump(new JailingGump(m_Offender, m_Account, m_Reason, m_Jail,
								m_AutoRelease, m_Days, m_Hours, m_FullJail, m_Comment));
							return;
						}
						...
						{
							sender.Mobile.SendMessage(
								"Musisz dodac komentarz do tego zamkniecia , czyli za co dokladnie i szczegolowy opis!");
							sender.Mobile.SendGump(new JailingGump(m_Offender, m_Account, m_Reason, m_Jail,
								m_AutoRelease, m_Days, m_Hours, m_FullJail, m_Comment));
							return;
						}
						...
						JailSystem.CommitJailing(m_Offender, m_Account, sender.Mobile, m_Reason, m_AutoRelease,
							duration, m_FullJail, m_Comment, m_Jail);
						break;
				}
			}
			catch (Exception exc)
