	...
	public class PlayerJailGump : Gump
	{
		...
		private void MakeGump()
		{
			try
			{
				...
				{
					this.AddLabel(120, 55, GreenHue,
						String.Format("{0} {1} i {2} godzin{3}", m_Jail.Duration.Days,
							m_Jail.Duration.Days == 1 ? "dzien" : "dni ", m_Jail.Duration.Hours,
							m_Jail.Duration.Hours <= 1 ? "a" : "y"));
				}
				else
				{
				...
				if (m_Jail.AutoRelease)
				{
					...
					{
						AddLabel(20, 75, LabelHue,
							String.Format("Do konca kary pozostalo {0} {1}, {2} godzin{3} i {4} minut{5}.", left.Days,
								left.Days == 1 ? "dzien" : "dni", left.Hours, left.Hours <= 1 ? "a" : "y", left.Minutes,
								left.Minutes == 1 ? "a" : "y"));
					}
					else
					{
