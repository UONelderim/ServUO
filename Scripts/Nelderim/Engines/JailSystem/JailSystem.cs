...
namespace Arya.Jail
{
	...
	public class JailSystem
	{
		...
		private static readonly string m_SaveFolder = "Logs/Jail";

		private static readonly string m_BackupFolder = @"Logs/Jail/Backup";

		private static readonly string m_PurgeFolder = @"Logs/Jail/Purged";

		public static AccessLevel m_MacroAccessLevel = AccessLevel.Counselor;

		public static bool m_AllowMacroCheck = true;

		public static ArrayList JailComments { get; set; }

		public static JailType[] m_Jail =
		{
			new JailType(true, "Wiezienie glowne", new[]
				{
					new Point3D(2033, 1572, 0), /// Cela 1 
				...
				new Point3D(1994, 1673, 0), new Point3D(2038, 1603, 0),
				Map.Felucca.Regions.ContainsKey("Wiezienie")
					? Map.Felucca.Regions["Wiezienie"].Area
					: new Rectangle3D[] { }
			)
		};
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register("wina", AccessLevel.Player, OnJailInfoCommand);
			CommandSystem.Register("QuickJail", m_MinLevel, OnQuickJail);
			CommandSystem.Register("QJ", m_MinLevel, OnQuickJail);
		...
		*/
		...
		[Usage("Jail [release/view/info/history/admin/help/player <name>/account <name>]"),
		 Description(
			 "This command is used in conjunction with a number of parameters to administer the jail system. Type \"[Jail Help\" in game for a detailed description of all the parameters and their usage.")]
		/// <summary>
				...
				switch (e.Arguments[0].ToLower())
				{
					...
					case "comment":
						...
						if (e.Mobile.AccessLevel >= m_MinLevel)
						{
							...
							e.Mobile.Target = new JailTarget(OnJailCommentTarget);
						}
						else
						{
							e.Mobile.SendMessage("You don't have access to that command");
					...
					default:
						...
						e.Mobile.SendMessage(
							"Invalid command parameters. Use Jail Help to view information on the jail system.");
						break;
				}
			}
			else if (e.Arguments.Length > 1)
		...
		private static void OnQuickJail(CommandEventArgs e)
		{
			...
			try
			{
				minutes = Int32.Parse(e.Arguments[0]);
			}
			catch
			{
			...
			try
			{
				jailtyp = m_Jail[Int32.Parse(e.Arguments[1])];
			}
			catch
			{
		...
		/// </summary>
		private static readonly ArrayList m_PendingJailings;
		...
		/// </summary>
		private static readonly ArrayList m_Jailings;
		...
		/// </summary>
		private static readonly ArrayList m_ExpiredJailings;

		/// <summary>
		///     Lists the QuickJailEntry objects currently active. This list isn't saved
		/// </summary>
		private static readonly ArrayList m_QuickJailings;
		...
		public static bool InitJail(Mobile from, Mobile offender, bool forceJail)
		{
			...
			else
			{
				...
				{
					from.SendMessage(
						"You can't jail that player because they're either already jailed or a staff member.");
				}
		...
		public static void CancelJail(Mobile m, Mobile from)
		{
			...
			if (m_PendingJailings
			    .Contains(m)) // This will prevent moving to the exit players jailed offline who aren't put in the pending list
			{
				if (m.NetState != null)
				{
				...
				try
				{
					...
					else if (m.NetState != null)
					{
						m.SendLocalizedMessage(
							505614); // Wystapil blad systemu wieziennictwa! Zglos go Ekipie Nelderim!
					}
				}
				catch (Exception exc)
				{
		...
		public static void CommitJailing(Mobile m, Account account, Mobile from, string reason, bool autorelease,
			TimeSpan duration, bool fulljail, string comment, JailType jailtype)
		{
		...
		private static void Save()
		{
			...
			SaveCommentFile(jailcommentFile, JailComments);
		}
		...
		private static void Backup()
		{
			...
			string[] folders = { "Third Backup", "Second Backup", "Most Recent" };
		...
		private static void Load()
		{
			...
			{
				JailComments.AddRange(LoadJailCommentFile(jailcommentFile));
			}
		}
		...
		public static string CheckComments(Mobile player)
		{
			...
			foreach (PlayerCommentEntry comment in JailComments)
			{
				if (comment.Player == player)
				{
					html += String.Format("({0})(Attention){1}<br>", comment.CreatedTime.ToShortDateString(),
						comment.Comment);
				}
			}
		...
		private static JailEntry GetCurrentJail(Mobile m)
		{
			lock (m_Jailings)
			{
				foreach (JailEntry jail in m_Jailings)
				{
					...
					if (jail.Account == (m.Account as Account) && jail.FullJail)
					{
						return jail;
					}
				}
