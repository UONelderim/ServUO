...
namespace Arya.Jail
{
	...
	public class JailEntry
	{
		...
		public bool FullJail
		{
			...
			set
			{
				...
				foreach (NetState ns in NetState.Instances)
				{
					...
					{
						if ((ns.Account as Account) == m_Account && ns.Mobile != m_Mobile &&
						    JailSystem.IsInJail(ns.Mobile))
						{
		...
		public JailEntry(Mobile offender, Account account, Mobile from, TimeSpan duration, string reason,
			string comment, bool autorelease, bool fulljail, JailType jailtype)
		{
			...
			{
				m_Comments.Add(String.Format("{0}{1}(Jailing)({2}): {3}", from.Name, AccessLevelPlayer(from),
					DateTime.Now.ToShortTimeString(), comment));
			}
		...
		{
			m_Comments.Add(
				String.Format("[{0}] {1}(Jailed): {2}", DateTime.Now.ToShortDateString(), from.Name, comment));
		}
		...
		public static JailEntry Load(XmlNode xNode)
		{
			...
			try
			{
				autorelease = Boolean.Parse(xNode.Attributes["AutoRelease"].Value);
			}
			catch { }
			...
			try
			{
				fulljail = Boolean.Parse(xNode.Attributes["FullJail"].Value);
			}
			catch { }
