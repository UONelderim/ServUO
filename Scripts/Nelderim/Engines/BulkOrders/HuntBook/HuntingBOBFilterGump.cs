...
*/
		...
		private static readonly int[,] m_ClassFilters =
		{
			{ 1062229, 0 }, // All
			{ 1, 1 }, // 1 Class
			{ 2, 2 }, // 2 Class
		...
		private static readonly int[][,] m_Filters = { m_TypeFilters, m_AmountFilters, m_ClassFilters };

		private static readonly int[] m_XOffsets_Type = { 0, 100, 195 };
		private static readonly int[] m_XOffsets_Amount = { 0, 100, 195, 300 };
		private static readonly int[] m_XOffsets_Class = { 0, 100, 195, 300 };

		private static readonly int[] m_XWidths_Small = { 50, 50, 70, 50 };
		private static int[] m_XWidths_Large = { 80, 50, 50, 50, 50, 50 };

		private void AddFilterList(int x, int y, int[] xOffsets, int yOffset, int[,] filters, int[] xWidths,
			int filterValue, int filterIndex)
		{
			for (int i = 0; i < filters.GetLength(0); ++i)
			{
				...
				else
				{
					...
					if (isSelected)
						sel = "#0000FF";
					AddHtml(x + 35 + xOffsets[i % xOffsets.Length], y + ((i / xOffsets.Length) * yOffset),
						xWidths[i % xOffsets.Length], 32, "<basefont color=" + sel + ">Klasa " + number + "</basefont>",
						false, false);
				}
