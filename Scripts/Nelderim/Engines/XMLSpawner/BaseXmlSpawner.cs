...
namespace Server.Mobiles
{
    ...
    public class BaseXmlSpawner
    {
                ...
                if (pack != null)
                {
                    ...
                    if (IsSpecialItemKeyword(baseitemtype))
                    {
                        ...
                        switch (kw)
                        {
                            ...
                            case itemKeyword.RANGED:
                                {
                                    // syntax is RANGED,min,max
                                    //get the min,max
                                    if (itemkeywordargs.Length == 3)
                                    {
                                        ...
                                        catch { status_str = "Invalid RANGED args : " + itemtypestr; converterror = true; }

                                        try { max = int.Parse(itemkeywordargs[2]); }
                                        catch { status_str = "Invalid RANGED args : " + itemtypestr; converterror = true; }

                                        if (converterror) return false;
                                        Item item = MagicRangedWeapon(min, max, false);
                                        if (item != null)
                                        {
                                            if (equip && m != null)
                                            {
                                    ...
                                    {
                                        status_str = "RANGED takes 2 args : " + itemtypestr;
                                        return false;
                                    }
                                    break;
                                }
                            case itemKeyword.JRANGED:
                                {
                                    ...
                                    if (itemkeywordargs.Length == 3)
                                    {
                                        ...
                                        bool converterror = false;
                                        try { min = int.Parse(itemkeywordargs[1]); }
                                        catch { status_str = "Invalid JRANGED args : " + itemtypestr; converterror = true; }

                                        try { max = int.Parse(itemkeywordargs[2]); }
                                        catch { status_str = "Invalid JRANGED args : " + itemtypestr; converterror = true; }

                                        if (converterror) return false;
                                        Item item = MagicRangedWeapon(min, max, true);
                                        if (item != null)
                                        {
                                            if (equip && m != null)
                                            {
                                    ...
                                    {
                                        status_str = "JRANGED takes 2 args : " + itemtypestr;
                                        return false;
                                    }
                                    break;
                                }
        ...
        public static Item MagicRangedWeapon(int minLevel, int maxLevel, bool jewel)
        {
