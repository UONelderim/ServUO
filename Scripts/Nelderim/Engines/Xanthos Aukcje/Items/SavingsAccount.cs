...
namespace Arya.Savings
{
	...
	public class SavingsAccount : Item
	{
		...
		public static void Initialize()
		{
			...
			CommandHandlers.Register("InstallSavings", AccessLevel.GameMaster, InstallSavings_OnCommand);
			CommandHandlers.Register("ScaleSavings", AccessLevel.GameMaster, ScaleSavings_OnCommand);

			EventSink.WorldSave += World_Save;
		}
		...
		private static void World_Save(WorldSaveEventArgs args)
		{
			...
			if (AuctionConfig.InterestHour <= now.Hour && DateLastPaid != now.Date)
			{
				...
				foreach (Item item in accounts)
				{
					...
					if (null != box && null != box.Owner && box.Owner.Map != Map.Internal)
						box.Owner.SendMessage(Misc.kRedHue,
							"Interest has been paid on the {0} in your savings account!",
							AuctionConfig.EnableTokens ? "gold and tokens" : "gold");
				}

				DateLastPaid = now.Date;
			}
		}
		...
		public override bool OnDragDrop(Mobile from, Item dropped)
		{
			...
			if (!success)
			{
				string text = EnableTokens ? ", tokens " : " ";
		...
		private bool DepositItem(Item forDeposit, out bool isContainer)
		{
			...
			else if (forDeposit is Container)
			{
				...
				if (null != container.Items)
				{
					...
					{
						if (list[i] is Item && !DepositItem(list[i], out isContainer))
							result = false;
					}
				}
		...
		private static void ScaleSavings_OnCommand(CommandEventArgs e)
		{
			...
			if (1 <= e.Length)
			{
				...
				try
				{
					percent = Math.Abs(Int32.Parse(e.GetString(e.Length - 1)));
				}
				catch (Exception exc)
				{
			...
			if (percent >= 0)
				ScaleSavingsAccounts(e.Mobile, option, percent / 100.00);
		}
		...
		private static void ScaleSavingsAccounts(Mobile from, SavingsOption option, double percent)
		{
			...
			string optionText = option == SavingsOption.Both ? "gold & tokens" :
				option == SavingsOption.Gold ? "gold" : "tokens";

			foreach (Item i in items)
			{
				SavingsAccount sa = i as SavingsAccount;
				...
				if (null != sa)
				{
					...
					if (option == SavingsOption.Both || option == SavingsOption.Gold)
						sa.Gold = (int)(sa.Gold * percent);
					if (option == SavingsOption.Both || option == SavingsOption.Tokens)
						sa.Tokens = (int)(sa.Tokens * percent);
				}
			}

			World.Broadcast(59, true, "Scaling complete, {0} accounts ({1}) scaled by {2}%", accountsScaled, optionText,
				percent * 100);
		}
	...
	{
		private readonly SavingsAccount m_Account;
		private readonly BankBox m_Bank;
		private readonly Container m_Pack;
		private readonly int m_AmountID = 0;
		private readonly string m_MaximumCheckValue;

		private static readonly Type[] s_ArgTypes = { typeof(int), };
		...
		public SavingsGump(Mobile from) : base(400, 300)
		{
			...
			if (SavingsAccount.EnableTokens)
			...
			if (null != check)
			{
				if (m_Account.Withdraw(currency, amount))
				{
					if (m_Pack == null || !m_Pack.TryDropItem(from, check, false))
					{
						...
						{
							from.SendMessage("You have withdrawn " + amount.ToString("#,0") + " {0} from savings.",
								currencyString);
						}
					}
					else
					...
					from.SendMessage(
						"You have insufficient funds to withdraw " + amount.ToString("#,0") + " {0} from savings!",
						currencyString);
			}
		}
	}
