...
namespace Xanthos.Utilities
{
	public class FindMobsGump : Gump
	{
		...
		private static readonly bool s_PrevLabel = false;
		private static readonly bool s_NextLabel = false;

		private readonly string m_SearchValue;
		private readonly int m_Options;
		private Mobile m_Owner;
		private readonly ArrayList m_Names;
		private int m_Page;
		private string m_Args;
		private readonly bool m_MiniGump;

		public static void Initialize()
		{
			CommandSystem.Register(kCommandName, AccessLevel.GameMaster, Find_OnCommand);
		}

		[Usage(kCommandName + " [-i] [-p|-t] <name>")]
		[Description(
			"Finds all mobiles by name, -p for players, -t searches by mobiles type name, -i specifies the internal map")]
		...
		public FindMobsGump(Mobile owner, string searchValue, int options) : this(owner,
			BuildList(owner, searchValue, options), 0, searchValue, options, false)
		{
		...
		public FindMobsGump(Mobile owner, ArrayList list, int page, string searchValue, int options, bool miniGump) :
			base(kGumpOffsetX, kGumpOffsetY)
		{
			m_Owner = owner;
			m_Names = list;
			m_SearchValue = searchValue;
		...
		public static ArrayList BuildList(Mobile owner, string searchValue, int options)
		{
			...
			foreach (Mobile mob in World.Mobiles.Values)
			{
				if (searchInternal != (Map.Internal == mob.Map))
					continue;
				if (findPlayers && !(mob is PlayerMobile))
					continue;
		...
		public void Initialize(int page)
		{
			...
			else
			{
				...
				AddLabel(x + kTextOffsetX, y, kTextHue,
					String.Format("Page {0} of {1} ({2}) - Matches for: {3}", page + 1,
						(m_Names.Count + kEntryCount - 1) / kEntryCount, m_Names.Count, m_Args));
				...
				if (page > 0)
				{
					AddButton(x + kPrevOffsetX, y + kPrevOffsetY, kPrevButtonID1, kPrevButtonID2, 1,
				...
				{
					AddButton(x + kNextOffsetX, y + kNextOffsetY, kNextButtonID1, kNextButtonID2, 2,
						GumpButtonType.Reply, 1);
				...
				for (int i = 0, index = page * kEntryCount; i < kEntryCount && index < m_Names.Count; ++i, ++index)
				{
					...
					if (!item.Deleted)
					{
						...
						AddButton(x + kBringOffsetX, y + kBringOffsetY, kBringButtonID1, kBringButtonID2, i + 4,
							GumpButtonType.Reply, 0);

						x += kButtonWidth + kOffsetSize;
						AddImageTiled(x, y, kButtonWidth, kEntryHeight, kButtonGumpID);
						AddButton(x + kGoOffsetX, y + kGoOffsetY, kGoButtonID1, kGoButtonID2, i + 1004,
							GumpButtonType.Reply, 0);
					}
				}
