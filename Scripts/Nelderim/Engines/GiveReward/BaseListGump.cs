...
namespace Server.Gumps
{
	public class BaseListGump : Gump
	{
		...
		public virtual void Init()
		{
			...
			this.AddHtml(81, 43, 409, 19, String.Format("<center>{0} V{1}</center>", GumpName, VerNum), false, false);
			this.AddHtml(111, 87, 413, 409, PopulateStringList(m_iLoc), true, false);
		...
		public virtual void CreateButtons(int iTotalEnteries)
		{
			...
			{
				if (UseButton1)
					this.AddButton(64, 91 + (18 * i), Button1NormalID, Button1PressedID,
						(i + m_iLoc) + (iButtonShift * 2), GumpButtonType.Reply, 0);
				if (UseButton2)
					this.AddButton(88, 91 + (18 * i), Button2NormalID, Button2PressedID, (i + m_iLoc) + iButtonShift,
		...
		protected bool CheckArrayAtLoc(int loc)
		{
			if (loc < m_alList.Count)
			{
				...
				if (m_alList[loc] is Mobile && (((Mobile)m_alList[loc]) == null || ((Mobile)m_alList[loc]).Deleted))
				{
					m_alList.RemoveAt(loc);
					return false;
				}

				if (m_alList[loc] == null)
				{
					m_alList.RemoveAt(loc);
					return false;
				}
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			switch (info.ButtonID)
			{
				case (int)Buttons.Find:
					m_sFilter = info.GetTextEntry((int)Buttons.FilterEntry).Text;
