...
namespace Server.Engines
{
	public class CheckPlayer
	{
		...
		public void PlayerRequest(bool isTrue)
		{
			...
			if (isTrue)
				m_GM.SendMessage(0x40, "Gracz {0} poprawnie odpowiedzial na wezwanie w czasie {1} sekund.",
					m_Player.Name, TimeSpanFormat(clickbutton));
			else
				m_GM.SendMessage(0x20, "Gracz {0} blednie odpowiedzial na wezwanie w czasie {1} sekund.", m_Player.Name,
...
//				JailSystem.FinalizeJail( m_Player );
			...
			m_GM.SendMessage("Gracz {0}, nie odpowiedzial w czasie 1 minuty i zostal zamkniety w wiezieniu.",
				m_Player.Name);
		}
	}
}
