...
namespace Nelderim
{
    class Commands
    {
        public static void Initialize() {
            CommandSystem.Register("kartapostaci", AccessLevel.Player, new CommandEventHandler(KartaPostaciGracz));
            CommandSystem.Register("kp", AccessLevel.Player, new CommandEventHandler(KartaPostaciGracz));
            CommandSystem.Register("kartapostaciinnych", AccessLevel.Counselor, new CommandEventHandler(KartaPostaciGraczInny));
            CommandSystem.Register("kpi", AccessLevel.Counselor, new CommandEventHandler(KartaPostaciGraczInny));
        ...
        private static void KartaPostaciGraczInny(CommandEventArgs e) {
            e.Mobile.BeginTarget(8, false, TargetFlags.None, new TargetCallback(CharacterSheetOfOtherPlayer_OnTarget));
        }
        ...
        private static void Qs_OnCommand(CommandEventArgs args) {
            ...
            try {
                foreach (NetState ns in NetState.Instances) {
                    ...
                    if (m != null && from != m && m.AccessLevel == AccessLevel.Player && !from.InRange(m.Location, 5) && m.Map != Map.Internal && CharacterSheet.Get( m ).AttendenceInEvents )
                        players.Add(m);
                }
