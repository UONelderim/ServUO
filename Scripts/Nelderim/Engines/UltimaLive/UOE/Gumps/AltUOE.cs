			...
			switch (info.ButtonID)
			{
				...
				case 1:
				{
					...
					if (dd.StcT)
					{
						if (dd.MultiT)
							CommandSystem.Handle(pm,
								String.Format("{0}m incStaticAlt {1}", CommandSystem.Prefix, dd.A_Val));
						else
							CommandSystem.Handle(pm,
								String.Format("{0}incStaticAlt {1}", CommandSystem.Prefix, dd.A_Val));
					...
					if (dd.LndT)
					{
						if (dd.MultiT)
							CommandSystem.Handle(pm,
								String.Format("{0}m incLandAlt {1}", CommandSystem.Prefix, dd.A_Val));
						else
							CommandSystem.Handle(pm,
								String.Format("{0}incLandAlt {1}", CommandSystem.Prefix, dd.A_Val));
