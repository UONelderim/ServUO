...
namespace Server.Gumps
{
	public class SynthGump : Gump
	{
		...
		private int m_Page;
		private readonly bool m_Music;
		private readonly bool m_Keys;
		private readonly bool m_Recording;
		private readonly bool m_FileOptions;
		private bool m_Chords;
		private readonly bool[] m_Mixer;
		private string m_Name;
		private string m_Speed;
		private readonly List<XmlMusic> m_MusicTracks;
		private readonly Mobile m_From;

		public static void Initialize()
		{
			CommandSystem.Register("Synth", AccessLevel.Administrator, SynthGump_OnCommand);
		}
		...
		public SynthGump(Mobile from, int page)
			: this(from, page, 0, false, false, false, false, false,
				new[] { false, false, false, false, false, false, false, false, false },
				new List<string>(), "Untitled", new List<XmlMusic>())
		{
		}

		public SynthGump(Mobile from, int page, int instrument, bool music, bool keys, bool recording, bool fileoptions,
			bool chords, bool[] mixer,
						...
						if (instruments.Count > 0)
						{
							...
							for (int x = 0; x < Math.Min(4, instruments.Count); x++)
							{
								...
								if (findxm != null)
								{
									...
									{
										AddLabel(258, 470 + (x * 29), labelhue, "Nie moge odczytac tej piosenki.");
									}
								}
							}
							...
							if (instruments.Count > 4)
							{
								for (int x = 4; x < Math.Min(8, instruments.Count); x++)
								{
									...
									if (findxm != null)
									{
										...
										{
											AddLabel(540, 470 + ((x - 4) * 29), labelhue,
												"Nie moge odczytac tej piosenki.");
										}
									}
								}
							}
		...
		public enum Buttons
		{
			Quit, n01_c_low, n02_c_sharp_low, n03_d, n04_d_sharp, n05_e, n06_f, n07_f_sharp, n08_g, n09_g_sharp, n10_a,
			n11_a_sharp, n12_b,
			n13_c, n14_c_sharp, n15_d_high, n16_d_sharp_high, n17_e_high, n18_f_high, n19_f_sharp_high, n20_g_high,
			n21_g_sharp_high,
			n22_a_high, n23_a_sharp_high, n24_b_high, n25_c_high, n26_Pause, RB1_Harp, RB2_LapHarp, RB3_Lute, RB4_Drums,
			ShowMusic, ShowChords,
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			if (info.ButtonID > 100)
			{
				...
				from.SendGump(new SynthGump(m_From, m_Page, (int)inst, m_Music, m_Keys, m_Recording, m_FileOptions,
					m_Chords, m_Mixer, m_Song, m_Name, m_MusicTracks));
			}
				...
				else
				{
					...
					if (instruments.Count > 0)
					{
						...
						for (int x = 0; x < instruments.Count; x++)
						{
							if (!m_Mixer[x + 1])
							{
								from.SendMessage("Pomijam piosenke {0}", x + 1);
								continue;
							}
			...
			else
			{
				switch (info.ButtonID)
				{
					...
					case (int)Buttons.ShowKeys:
						from.SendGump(new SynthGump(m_From, m_Page, (int)inst, m_Music, !m_Keys, m_Recording,
							m_FileOptions, m_Chords, m_Mixer, m_Song, m_Name, m_MusicTracks));
						break;
					...
					case (int)Buttons.StartRecording:
					{
						...
						else
						{
							...
							m_Song.Add("normalne");
							from.SendGump(new SynthGump(m_From, m_Page, (int)inst, m_Music, m_Keys, true, m_FileOptions,
								m_Chords, m_Mixer, m_Song, m_Name, m_MusicTracks));
						}
							...
							if (m_Chords && info.ButtonID < (int)Buttons.n13_c)
							{
								switch (inst)
								{
									case InstrumentType.Harp:
									{
										...
										Harp inst2 = new Harp();
										Harp inst3 = new Harp();
										if (Play.PlayChord(from,
											    new[] { GetNoteString((NoteValue)(info.ButtonID - 1)) }, true, inst1,
											    inst2, inst3))
										{
											inst1.Delete();
											inst2.Delete();
		...
		private void LoadTrack(Mobile from)
		{
			...
			BaseInstrument.PickInstrument(from, OnPickedInstrument);
		}

		private void PlayTracks(Mobile from, List<XmlMusic> mxm)
		{
			if (mxm != null && mxm.Count > 0)
				for (int x = 0; x < mxm.Count; x++)
					PlayTrack(from, mxm[x]);
		}
		...
		private Queue LoadPlayList(Mobile from, string speed, List<string> notelist)
		{
			...
			string[] Notes =
			{
				"cl", "csl", "d", "ds", "e", "f", "fs", "g", "gs", "a", "as", "b", "c", "cs", "dh", "dsh", "eh",
				"fh", "fsh", "gh", "gsh", "ah", "ash", "bh", "ch"
			};
			int NumOfNotes = 0;
