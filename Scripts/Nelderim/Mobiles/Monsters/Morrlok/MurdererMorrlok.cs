	...
	public class MordercaMorrlok : BaseCreature
	{
				...
				if (map != null)
				{
					for (int i = 0; i < 10; ++i)
					{
						...
						Effects.SendLocationParticles(EffectItem.Create(from, map, EffectItem.DefaultDuration), 0x3728,
							10, 10, 2023);
						Effects.SendLocationParticles(EffectItem.Create(to, map, EffectItem.DefaultDuration), 0x3728,
							10, 10, 5023);
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
		{
			if (m.Alive && m is PlayerMobile)
			{
				...
				if (range >= 0 && InRange(m, range) && !InRange(oldLocation, range) && this.Hits == this.HitsMax &&
				    this.Hidden && IsEnemy(m))
				{
					this.Frozen = false;
					this.Hidden = false;
					this.Combatant = m;
