namespace Server.Items
{
    public class ImiennySmokKey : MasterKey
    {
        public ImiennySmokKey()
            : base(0xFF3)
        {
        }

        public ImiennySmokKey(Serial serial)
            : base(serial)
        {
        }
        public override int LabelNumber => 3070027;//klucz do jamy imiennego smoka
        public override int Lifespan => 600;
        public override void Serialize(GenericWriter writer)
        {
            base.Serialize(writer);

            writer.Write(0); // version
        }

        public override void Deserialize(GenericReader reader)
        {
            base.Deserialize(reader);

            int version = reader.ReadInt();
        }

        public override bool CanOfferConfirmation(Mobile from)
        {
            if (from.Region != null && from.Region.IsPartOf("Wulkan_LVL4_Boss"))
                return base.CanOfferConfirmation(from);

            return false;
        }
    }
}
