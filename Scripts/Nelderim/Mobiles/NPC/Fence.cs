...
namespace Server.Mobiles
{
    public class Fence : AnimalTrainer
    {
        ...
        private void StablingHandle(SpeechEventArgs e)
        {
            ...
            {
                e.Handled = true;
                if (e.Speech.ToLower() == "zmniejsz" || Regex.IsMatch(e.Speech, "^zmniejsz..?$", RegexOptions.IgnoreCase)) OnLazySpeech();
                ...
                else
                {
                    ...
                    else
                    {
                        if (!e.Mobile.Backpack.TryDropItem(e.Mobile, check, false))
                            Say(00505093, charge.ToString()); // Trzymaj ten czek! To bylo spore ryzyko... warte {0} sztuk zlota
                        else
                        {
        ...
        public override void OnSpeech(SpeechEventArgs e)
        {
                    ...
                    if (CheckVendorAccess(from))
                    {
                        ...
                        if (e.Speech.ToLower() == "saldo") OnLazySpeech();
                            ...
                            if (box != null && Banker.Withdraw(from, 100))
                                this.Say(00505104, (Banker.GetBalance(from) + 100).ToString()); // Masz w banku {0} zlota... Minus 100 za moja fatyge
                            else
            ...
            else if (Type == FenceType.Barkeeper)
            {
                ...
                if ((Regex.IsMatch(e.Speech, "nowa", RegexOptions.IgnoreCase) || Regex.IsMatch(e.Speech, "zmieni", RegexOptions.IgnoreCase)) && Regex.IsMatch(e.Speech, "tozsamosc", RegexOptions.IgnoreCase))
                {
                    //e.Handled = true;
