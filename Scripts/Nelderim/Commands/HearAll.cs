...
namespace Server.Commands
{
	...
	{
		private static readonly ArrayList m_HearAll = new ArrayList();

		public static void Initialize()
		{
			CommandSystem.Register("HearAll", AccessLevel.Counselor, HearAll_OnCommand);
			...
			if (heard != null)
			{
				if (m_HearAll.Count > 0)
				{
					...
					for (int i = 0; i < m_HearAll.Count; ++i)
					{
						...
						if (hearing != null && heard.AccessLevel <= hearing.AccessLevel ||
						    (heard.AccessLevel >= AccessLevel.Seer && hearing.AccessLevel >= AccessLevel.Seer))
