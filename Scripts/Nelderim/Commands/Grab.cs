...
namespace Server.Commands
{
	public class Grab
	{
		...
		private static readonly Type[] m_Arrows = { typeof(Arrow), typeof(Bolt) };

		private static readonly Type[] m_JewelStone =
		{
		...
		private static readonly Type[] m_Reagents =
		{
			typeof(SpidersSilk), typeof(SulfurousAsh), typeof(Nightshade), typeof(MandrakeRoot), typeof(Ginseng),
			typeof(Garlic), typeof(Bloodmoss), typeof(BlackPearl)
		};

		private static readonly Type[] m_NecroReagents =
		{
			typeof(BatWing), typeof(GraveDust), typeof(DaemonBlood), typeof(NoxCrystal), typeof(PigIron)
		};

		private static readonly Type[] m_RareIngredients =
		{
			typeof(Brimstone), typeof(DaemonBone), typeof(Bloodspawn), typeof(DragonsBlood), typeof(WyrmsHeart),
			typeof(WyrmsHeart), typeof(WyrmsHeart), typeof(Blackmoor), typeof(EyeOfNewt), typeof(Pumice),
			typeof(VolcanicAsh), typeof(BlueDragonsHeart), typeof(RedDragonsHeart), typeof(DragonsHeart)
		};

		private static readonly Type[] m_meat =
		{
			typeof(RawBird), typeof(RawChickenLeg), typeof(RawFishSteak), typeof(RawLambLeg), typeof(RawRibs)
		};

		private static readonly Type[] m_wool = { typeof(Wool), typeof(TaintedWool) };

		#endregion

		public static void Initialize()
		{
			CommandSystem.Register("Grab", AccessLevel.Player, Grab_OnCommand);
			CommandSystem.Register("GrabPojemnik", AccessLevel.Player, GrabPojemnik_OnCommand);
		}
		...
		[Usage("Grab")]
		[Description(
			"Grabi okolice skladujac przedmioty w plecaku. Dostepne opcje: gem, reg, necro, arrow, meat, wool, gold, bone, weapon, armor, jewel, scroll, feather, leather, spined, horned, barbed, leatherall, aids, cc, unsafe, gut, zoogie, egg")]
					...
					else if (item is Corpse)
					{
						...
						if (m_CheckCriminal && corpse.IsCriminalAction(e.Mobile))
						{
							if (corpse.Owner == null || !corpse.Owner.Player)
								e.Mobile.SendLocalizedMessage(505571,
									corpse.Owner == null
										? ""
										: corpse.Owner
											.Name); // Spladrowanie ciala stworzenia ~1_NAME~bedzie przestepstwem! Pomijam.
							else
								e.Mobile.SendLocalizedMessage(505570,
									corpse.Owner
										.Name); // Spladrowanie zwlok gracz ~1_NAME~ bedzie przestepstwem! Pomijam.
		...
		private class GrabTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				{
					if (!m_From.GrabContainer.Deleted && (m_From.GrabContainer.IsChildOf(m_From.Backpack) ||
					                                      m_From.GrabContainer == m_From.Backpack))
					{
					...
					{
						m_From.SendMessage(
							"W plecaku twojej postaci nie ma pojemnika wybranego do przechowywania grabionych przedmiotow. Uzyj komendy [GrabPojemnik aby wskazac nowy pojemnik.");
						Stop();
						m_From.EndAction("Grab");
						return;
					}
