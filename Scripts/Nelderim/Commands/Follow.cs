...
namespace Server.Commands
{
	public class Follow
	{
		...
		{
			CommandSystem.Register("Follow", AccessLevel.Counselor, Follow_OnCommand);
			...
			if (m_From != null && !m_From.Deleted)
			{
				...
				if (m_Target != null && !m_Target.Deleted)
				{
					if (m_Target is PlayerMobile)
					{
						...
						if (m_From.AccessLevel == m_Target.AccessLevel && m_Target.Hidden &&
						    !m_pmTarget.VisibilityList.Contains(m_From))
						{
	...
	public class FollowGump : Gump
	{
		...
		public FollowGump(Mobile from, Mobile following, Timer timer) : base(20, 30)
		{
			...
			AddHtml(0, 0, 150, 50, Resize(Color(Center("Currently Following"), 0xFFFFFF), 100), false, false);
			AddHtml(0, 14, 150, 50, Resize(Color(Center(String.Format("{0}", following.Name)), 0x0000CC), 100), false,
				false);
