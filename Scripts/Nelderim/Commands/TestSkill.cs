...
{ 
    ...
    { 
        ...
        { 
            CommandSystem.Register( "testSkill", AccessLevel.GameMaster, new CommandEventHandler( TestSkilll_OnCommand ) ); 
            CommandSystem.Register( "testStat", AccessLevel.GameMaster, new CommandEventHandler( TestStat_OnCommand ) ); 
            CommandSystem.Register( "testTemplate", AccessLevel.GameMaster, new CommandEventHandler( TestTemplate_OnCommand ) ); 
        }

        private static void testStat(Mobile from, SkillName skillName, int targetStr, int targetDex, int targetInt, double targetSkillValue)
        {
                    ...
                    for ( uses = 0; true; ++uses )
                    {
                        ...
                        if( uses % 6 == 0 && false)
                            logger.WriteLine( "\t" + skill.Name + "\t" + skill.Base + "\t" + ( uses + 1 ) + "\t" + delay + "\t" + ( delay * ( uses + 1 ) / 3600 ) );

                        // to avoid infinite loop:
                        if( uses % 20100 == 0 ) {
                            if( skill.Base == previous && from.RawStr == previousStr && from.RawDex == previousDex && from.RawInt == previousInt  ) {
				    ...
				    SkillDebug( from, "{0}/{1}/{2} ({3}) | {4}: {5}% ({6}% cap)", startStr, startDex, startInt, startStr + startDex + startInt, skillName, startSkill, from.Skills[skillName].Cap );
                    SkillDebug( from, "Wartosci aktualne: " );
				    SkillDebug( from, "{0}/{1}/{2} ({3}) | {4}: {5}% ({6}% cap)", from.RawStr, from.RawDex, from.RawInt, from.RawStatTotal, skillName, from.Skills[skillName].Base, from.Skills[skillName].Cap );
				    SkillDebug( from, "Przyrosty: " );
				    SkillDebug( from, "STR: +{0} | DEX: +{1} INT: +{2} | {3}: +{4}%", from.RawStr - startStr, from.RawDex - startDex, from.RawInt - startInt, skillName, from.Skills[skillName].Base - startSkill );
        ...
        private static void TestStat_OnCommand( CommandEventArgs arg )
        {
            ...
            {
                if ( arg.Length < 1 )
                    arg.Mobile.SendMessage( 38, "TestSkill2 <Skill> [<TargetStr>] [<TargetDex>] [<TargetInt>] [<TargetSkillValue>]" );
                else
                {
                    ...
                    double targetSkillValue = (arg.Length > 4) ? arg.GetDouble( 4 ) : -1.0; // from.Skills[ skillName ].Base;
        ...
        private static void TestSkilll_OnCommand( CommandEventArgs arg )
        {
            ...
            {
                if ( arg.Length < 7 )
                    from.SendMessage( 38, "TestSkill <SkillName> <SkillDelay> <SkillStartValue> <SuccessChance> <Hours> <Minutes> <Seconds>" );
                else
                {
                    PlayerMobile pm = (PlayerMobile)from;
                    bool debugEnabled = pm.GainsDebugEnabled;
                    ...
                    for ( int i = 0; i < uses; i++ )
                    {
                        ...
                        if( i % 6 == 0 )
                            logger.WriteLine( "\t" + skill.Name + "\t" + skill.Base + "\t" + ( i + 1 ) + "\t" + skillDelay + "\t" + ( skillDelay * ( i + 1 ) / 3600 ) );
                    ...
                    SkillDebug( from, "----------------------------" );
                    SkillDebug( from, "Czas trwania: {0}h {1}m {2}s, lacznie {3} sekund", hours, minutes, seconds, totalSeconds );
