...
namespace Server.Commands
{
	public class SBInfosCommand
    {
        public static void Initialize()
       	{
          	CommandSystem.Register( "SBInfos", AccessLevel.Administrator, new CommandEventHandler( SBInfos_OnCommand ) ); 
            CommandSystem.Register( "CraftInfos", AccessLevel.Administrator, new CommandEventHandler( CraftInfos_OnCommand ) ); 
       	} 
        ...
        public static void CraftInfos_OnCommand(CommandEventArgs e)
        {
            ...
            string fileName = String.Format("craft {0}-{1}-{2} {3}-{4}-{5}", now.Year, now.Month, now.Day, now.Hour, now.Minute, now.Second);
            ...
            foreach (CraftSystem sys in systems)
            {
                ...
                foreach (CraftItem item in sys.CraftItems)
                {
                    ...
                    foreach (SBInfo sb in SBInfosAnalyzer.SBInfos)
                    {
                        ...
                        if (sellInfo != null && sellInfo.IsInList(product))
                        {
                            ...
                            double unitPrice = (double)price / (double)resourceAmount;
                    ...
                    if (maxPrice > 0)
                    {
                        if (!typeUnitPrice.ContainsKey(resType) || maxUnitPrice > ((SBCraftDeal)typeUnitPrice[resType]).UnitPrice)
                ...
                foreach (DictionaryEntry en in typeUnitPrice)
                {
                    ...
                    if (resType != typeof(IronIngot) && resType != typeof(Log) && resType != typeof(Leather) && resType != typeof(Cloth))
                        continue;
    ...
    public class SBCraftDeal
    {
        ...
        private Type m_Product;
        private SBInfo m_SBInfo;

        public double UnitPrice { get { return m_UnitPrice; } }
        public CraftSystem System { get { return m_System; } }
        public Type Product { get { return m_Product; } }
        public SBInfo SBInfo { get { return m_SBInfo; } }
    ...
    public class SBInfosAnalyzer
    {
        ...
        private Dictionary<Type, SBInfoItem> m_SBInfoItems;
        ...
        public SBInfosAnalyzer( Mobile m )
        {
            ...
            IEnumerable<Type> types = Assembly.GetAssembly(typeof(SBInfo)).GetTypes().Where(myType => myType.IsClass && !myType.IsAbstract && myType.IsSubclassOf(typeof(SBInfo)));
        ...
        public string FileName
		{
            ...
            { 
                DateTime now = DateTime.Now;
			    return String.Format( "{0}_{1}-{2}-{3} {4}-{5}-{6}", m_Mobile.Account, now.Year, now.Month, now.Day, now.Hour, now.Minute, now.Second );
            }
		}
        ...
        public void LogPrices()
        {
            ...
            foreach ( Type type in m_SBInfoItems.Keys )
            {
                ...
                foreach ( SBInfoRecord r in buyInfos )
                    m_Logger.WriteLine( String.Format( "{0} - {1}", r.Price, r.Npc) );
        ...
        public void Load()
        {
            ...
            m_Logger.WriteLine( "{0}: {1} items loaded ({2} BuyInfos, {3} SellInfos)", m_Mobile.Name, m_SBInfoItems.Count, buyCnt, sellCnt ); 
        }
    ...
    {
        private List<SBInfoRecord> m_BuyInfos;
        private List<SBInfoRecord> m_SellInfos;

        public SBInfoItem()
        {
            m_BuyInfos  = new List<SBInfoRecord>();
            m_SellInfos = new List<SBInfoRecord>();
        }

        public List<SBInfoRecord> BuyInfos
        {
            get{ return m_BuyInfos; }
            set{ m_BuyInfos = value; }
        }

        public List<SBInfoRecord> SellInfos
        {
            get{ return m_SellInfos; }
            set{ m_SellInfos = value; }
        }
    }
