...
namespace Server.Mobiles
{
	...
	public abstract partial class BaseAI
	{
		...
		private class InternalEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				if (!m_Mobile.Deleted && m_Mobile.Controlled && m_From.CheckAlive())
				{
					...
					if (isFriend && m_Order != OrderType.Follow && m_Order != OrderType.Stay &&
			...
			if (from.Alive && m_Mobile.Controlled && from.InRange(m_Mobile, 14))
			{
				if (from == m_Mobile.ControlMaster)
				{
					...
					if (!m_Mobile.Summoned && !(m_Mobile is GrizzledMare))
					{
						...
						list.Add(new InternalEntry(from, 6099, 14, m_Mobile, this,
							OrderType.Unfriend)); // Remove Friend
		...
		{
			SkillName.Parry, SkillName.Healing, SkillName.Hiding, SkillName.Stealing, SkillName.Alchemy,
			SkillName.AnimalLore, SkillName.ItemID, SkillName.ArmsLore, SkillName.Begging, SkillName.Blacksmith,
			SkillName.Fletching, SkillName.Peacemaking, SkillName.Camping, SkillName.Carpentry, SkillName.Cartography,
			SkillName.Cooking, SkillName.DetectHidden, SkillName.Discordance, //??
			...
			SkillName.Tracking, SkillName.Stealth, SkillName.Inscribe, SkillName.Swords, SkillName.Macing,
			SkillName.Fencing, SkillName.Wrestling, SkillName.Lumberjacking, SkillName.Mining, SkillName.Meditation
		};
		...
		public virtual bool DoActionWander()
		{
				...
				if (m_Mobile.CurrentNavPoint >= 0 && m_Mobile.CurrentNavPoint < m_Mobile.NavPoints[map].Count)
				{
					...
					else if (OnAtWayPoint())
					{
						...
						else
						{
							m_Mobile.CurrentNavPoint++;
							m_Mobile.DebugSay(
								string.Format("I will go to the next navpoint: {0}",
									m_Mobile.NavPoints[map][m_Mobile.CurrentNavPoint]));
						}
					}
				}
			}
			...
			if (!DirectionLocked && m_Mobile.Combatant != null && !m_Mobile.Combatant.Deleted &&
		...
		public virtual bool DoOrderFollow()
		{
			...
			else if (m_Mobile.ControlTarget != null && !m_Mobile.ControlTarget.Deleted &&
		...
		public virtual bool DoOrderGuard()
		{
			...
			double closestDist =
				combatant == null ? m_Mobile.RangePerception : combatant.GetDistanceToSqrt(controlMaster);

			foreach (Mobile aggressor in controlMaster.Aggressors.Select(x => x.Attacker)
				         .Where(m => ValidGuardTarget(m)))
			{
		...
		{
			return combatant != null && combatant != m_Mobile && combatant != m_Mobile.ControlMaster &&
			       !combatant.Deleted && m_Mobile.CanSee(combatant) &&
			       ...
			       combatant.GetDistanceToSqrt(m_Mobile) <= m_Mobile.RangePerception;
		}
			    ...
			    (m_Mobile.ControlTarget is Mobile && ((Mobile)m_Mobile.ControlTarget).IsDeadBondedPet))
			{
				...
				if (m_Mobile.FightMode == FightMode.Closest || m_Mobile.FightMode == FightMode.Aggressor)
				{
					...
					foreach (Mobile aggr in eable)
					{
						...
						if ((newCombatant == null || aggrScore > newScore ||
						     (aggrScore == newScore && !aggr.Player && newCombatant.Player)) && m_Mobile.InLOS(aggr))
						{
							newCombatant = aggr;
							newScore = aggrScore;
						}
		...
		{
			if (DateTime.UtcNow >= m_Mobile.BardEndTime &&
			    (m_Mobile.BardMaster == null || m_Mobile.BardMaster.Deleted ||
		...
		public virtual void WalkRandom(int iChanceToNotMove, int iChanceToDir, int iSteps)
		{
			...
			if (m_Mobile.CanFly && Utility.Random(8 * iChanceToNotMove) <= 8)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					...
					if (m_Mobile.Home != Point3D.Zero && m_Mobile.RangeHome > 0 &&
					    !Utility.InRange(p, m_Mobile.Home, m_Mobile.RangeHome))
					{
						continue;
					}
		...
		public virtual MoveResult DoMoveImpl(Direction d)
		{
			if (m_Mobile.Deleted || m_Mobile.Frozen || m_Mobile.Paralyzed || !m_Mobile.CanMove ||
			    (m_Mobile.Spell != null && m_Mobile.Spell.IsCasting && m_Mobile.FreezeOnCast) ||
		...
		public virtual bool MoveTo(IPoint3D p, bool run, int range)
		{
			if (m_Mobile.Deleted || m_Mobile.DisallowAllMoves || p == null ||
			    (p is IDamageable && ((IDamageable)p).Deleted))
			{
				return false;
			}
			...
			{
				m_Path = new PathFollower(m_Mobile, p) { Mover = DoMoveImpl };
		...
		*/
		public virtual bool AcquireFocusMob(int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend,
			bool bFacFoe)
		{
		...
		public virtual bool AcquireFocusMob(int iRange, FightMode acqType, bool bPlayerOnly)
		{
			...
			{
				if (m_Mobile.ControlTarget == null || m_Mobile.ControlTarget.Deleted ||
				    (m_Mobile.ControlTarget is Mobile && ((Mobile)m_Mobile.ControlTarget).Hidden) ||
				    !m_Mobile.ControlTarget.Alive ||
				    (m_Mobile.ControlTarget is Mobile && ((Mobile)m_Mobile.ControlTarget).IsDeadBondedPet) ||
			...
			if (map != null)
			{
				...
				foreach (Mobile m in eable)
				{
					...
					if (m_Mobile.Summoned && m_Mobile.SummonMaster != null)
					{
						...
						if (m_Mobile.SummonMaster is PlayerMobile &&
					...
					if (!(m_Mobile is BaseNelderimGuard) && m is PlayerMobile pm && !(m_Mobile.Combatant == m))
					{
						...
						if (pm.HonorActive &&
						    !(m_Mobile.IgnoreHonor && (m.Aggressed.Count != 0 || m.Aggressors.Count != 0)))
							continue;
					    ...
					    IsSpidersFriend(m))
					{
						...
						{
							if (aggr.Defender.Equals(m_Mobile))
								shouldFocus = true;
						}
