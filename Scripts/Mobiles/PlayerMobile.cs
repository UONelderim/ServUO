...
namespace Server.Mobiles
{
	...
	public partial class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
		...
		{
			get { return BaseSweet.GetToothAche(this); }
			set { BaseSweet.SetToothAche(this, value, true); }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public bool HasValiantStatReward
		{
			get { return GetFlag(PlayerFlag.HasValiantStatReward); }
			set { SetFlag(PlayerFlag.HasValiantStatReward, value); }
		}
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public bool ToggleCutTopiaries
		{
			get { return GetFlag(PlayerFlag.ToggleCutTopiaries); }
			set { SetFlag(PlayerFlag.ToggleCutTopiaries, value); }
		}
				...
				foreach (KeyValuePair<Type, int> kvp in m_RecoverableAmmo)
				{
					if (kvp.Value > 0)
					{
						...
						if (ammo != null)
						{
							...
							SendLocalizedMessage(1073504,
								string.Format("{0}\t{1}", ammo.Amount, name)); // You recover ~1_NUM~ ~2_AMMO~.
						}
					}
				}
		...
		public override void ComputeResistances()
		{
			...
			for (int i = 0; i < Items.Count; ++i)
			{
				...
				Resistances[0] += setItem != null && setItem.SetEquipped
					? setItem.SetResistBonus(ResistanceType.Physical)
					: item.PhysicalResistance;
				Resistances[1] += setItem != null && setItem.SetEquipped
					? setItem.SetResistBonus(ResistanceType.Fire)
					: item.FireResistance;
				Resistances[2] += setItem != null && setItem.SetEquipped
					? setItem.SetResistBonus(ResistanceType.Cold)
					: item.ColdResistance;
				Resistances[3] += setItem != null && setItem.SetEquipped
					? setItem.SetResistBonus(ResistanceType.Poison)
					: item.PoisonResistance;
				Resistances[4] += setItem != null && setItem.SetEquipped
					? setItem.SetResistBonus(ResistanceType.Energy)
		...
		public override void AggressiveAction(Mobile aggressor, bool criminal)
		{
			...
			if (aggressor is BaseCreature creature && creature.ControlMaster != null && creature.ControlMaster != this)
			{
				...
				{
					BuffInfo.AddBuff(this,
						new BuffInfo(BuffIcon.HeatOfBattleStatus, 1153801, 1153827, Aggression.CombatHeatDelay, this,
							true));
					BuffInfo.AddBuff(aggressiveMaster,
						new BuffInfo(BuffIcon.HeatOfBattleStatus, 1153801, 1153827, Aggression.CombatHeatDelay,
							aggressiveMaster, true));
				}
			}
		...
		public override void DoHarmful(IDamageable damageable, bool indirect)
		{
			...
			if (ViceVsVirtueSystem.Enabled && (ViceVsVirtueSystem.EnhancedRules || Map == ViceVsVirtueSystem.Facet) &&
			    damageable is Mobile mobile)
			{
		...
		public override int ManaMax => base.ManaMax + AosAttributes.GetValue(this, AosAttribute.BonusMana) +
		                               (Race == Race.Elf ? 20 : 0) + MasteryInfo.IntuitionBonus(this) +
		...
		public override void OnHitsChange(int oldValue)
		{
			if (Race == Race.Gargoyle)
			{
				...
				{
					BuffInfo.AddBuff(this,
						new BuffInfo(BuffIcon.Berserk, 1080449, 1115021,
							string.Format("{0}\t{1}", GetRacialBerserkBuff(false), GetRacialBerserkBuff(true)), false));
		...
		public override void OnHeal(ref int amount, Mobile from)
		{
			...
			if (amount > 0 && from != this)
			{
				for (int i = Aggressed.Count - 1; i >= 0; i--)
				{
					...
					if (info.Defender.InRange(Location, Core.GlobalMaxUpdateRange) &&
					    info.Defender.DamageEntries.Any(de => de.Damager == this))
					{
				...
				for (int i = Aggressors.Count - 1; i >= 0; i--)
				{
					...
					if (info.Attacker.InRange(Location, Core.GlobalMaxUpdateRange) &&
					    info.Attacker.DamageEntries.Any(de => de.Damager == this))
					{
		...
		public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
		{
			...
			if (from == this)
			{
				...
				{
					VoidPoolController controller = Map == Map.Felucca
		...
		public override bool Drop(Point3D loc)
		{
			...
			if (item != null && drop && item.Parent == null && _BlessedItem != null && _BlessedItem == item)
			{
				...
				SendLocalizedMessage(1075292,
					item.Name != null ? item.Name : "#" + item.LabelNumber.ToString()); // ~1_NAME~ has been unblessed.
			}
		...
		private void OpenItemInsuranceMenu()
		{
			...
			if (items.Count == 0)
				SendLocalizedMessage(1114915, "",
					0x35); // None of your current items meet the requirements for insurance.
			else
		...
		private class ItemInsuranceMenuGump : Gump
		{
				...
				: base(25, 50)
			{
				...
				AddHtmlLocalized(10, 14, 150, 20, 1114121, 0x7FFF, false,
					false); // <CENTER>ITEM INSURANCE MENU</CENTER>
		...
		public override bool CheckTrade(Mobile to, Item item, SecureTradeContainer cont, bool message, bool checkItems,
			bool checkWeight, int plusItems, int plusWeight)
		{
			int msgNum = 0;
			...
			if (msgNum == 0 && item != null)
			{
				...
				if (Backpack == null ||
				    !Backpack.CheckHold(this, item, false, checkItems, checkWeight, plusItems, plusWeight))
				{
					msgNum = 1004040; // You would not be able to hold this if the trade failed.
				}
				else if (to.Backpack == null ||
				         !to.Backpack.CheckHold(to, item, false, checkItems, checkWeight, plusItems, plusWeight))
				{
		...
		protected override void OnMapChange(Map oldMap)
		{
			...
			if ((Map != ViceVsVirtueSystem.Facet && oldMap == ViceVsVirtueSystem.Facet) ||
			    (Map == ViceVsVirtueSystem.Facet && oldMap != ViceVsVirtueSystem.Facet))
			{
				InvalidateProperties();
			}
		...
		public override void Resurrect()
		{
			...
			if (Alive && !wasAlive)
			{
				...
				{
					BuffInfo.AddBuff(this,
						new BuffInfo(BuffIcon.ArcaneEmpowerment, 1078511, 1078512, AcceleratedSkill.ToString()));
				}
				...
				if (mob != null && mob.AccessLevel >= AccessLevel.GameMaster && mob.AccessLevel > from.AccessLevel)
				{
					if (p == null)
					{
						p =
							Packet.Acquire(
								new UnicodeMessage(
									from.Serial, from.Body, MessageType.Regular, from.SpeechHue, 3, from.Language,
									from.Name, text));
					}
		...
		public override ApplyPoisonResult ApplyPoison(Mobile from, Poison poison)
		{
			...
			if ((Poison == null || Poison.Level < poison.Level) && ToleranceSpell.OnPoisonApplied(this))
			{
				...
				{
					PrivateOverheadMessage(MessageType.Regular, 0x3F, 1053092,
						NetState); // * You feel yourself resisting the effects of the poison *
					return ApplyPoisonResult.Immune;
				}
			}
		...
		public override int Luck => AosAttributes.GetValue(this, AosAttribute.Luck) +
		                            TenthAnniversarySculpture.GetLuckBonus(this);
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (m_RewardTitles != null && m_SelectedTitle > -1)
			{
				if (m_SelectedTitle < m_RewardTitles.Count)
				{
					if (m_RewardTitles[m_SelectedTitle] is int)
					{
						...
						if ((int)m_RewardTitles[m_SelectedTitle] == 1154017 &&
						    CityLoyaltySystem.HasCustomTitle(this, out cust))
						{
			...
			if (TestCenter.Enabled)
			{
				...
				list.Add(string.Format("Kills: {0} / Deaths: {1} / Assists: {2}", // no cliloc for this!
					entry == null ? "0" : entry.Kills.ToString(), entry == null ? "0" : entry.Deaths.ToString(),
					entry == null ? "0" : entry.Assists.ToString()));

				list.Add(1060415,
					AosAttributes.GetValue(this, AosAttribute.AttackChance).ToString()); // hit chance increase ~1_val~%
				list.Add(1060408,
					AosAttributes.GetValue(this, AosAttribute.DefendChance)
						.ToString()); // defense chance increase ~1_val~%
				list.Add(1060486,
					AosAttributes.GetValue(this, AosAttribute.WeaponSpeed).ToString()); // swing speed increase ~1_val~%
				list.Add(1060401,
					AosAttributes.GetValue(this, AosAttribute.WeaponDamage).ToString()); // damage increase ~1_val~%
				list.Add(1060483,
					AosAttributes.GetValue(this, AosAttribute.SpellDamage)
						.ToString()); // spell damage increase ~1_val~%
				list.Add(1060433,
					AosAttributes.GetValue(this, AosAttribute.LowerManaCost).ToString()); // lower mana cost
			}
		...
		public void SelectRewardTitle(int num, bool silent = false)
		{
			...
			else if (num < m_RewardTitles.Count && num >= -1)
			{
				if (m_SelectedTitle != num)
				{
					...
					{
						SendLocalizedMessage(1074008,
							(string)m_RewardTitles[num]); // You change your Reward Title to "~1_TITLE~".
					}
				}
				else if (!silent)
				{
		...
		public override void AddNameProperties(ObjectPropertyList list)
		{
			...
			bool vvv = ViceVsVirtueSystem.IsVvV(this) &&
			           (ViceVsVirtueSystem.EnhancedRules || Map == ViceVsVirtueSystem.Facet);
		...
		public override void OnSkillChange(SkillName skill, double oldBase)
		{
			if (skill != SkillName.Alchemy && Skills.CurrentMastery == skill &&
			    Skills[skill].Value < MasteryInfo.MinSkillRequirement)
			{
		...
		{
			new Point3D(1481, 1612, 20), new Point3D(2708, 2153, 0), new Point3D(2249, 1230, 0),
			new Point3D(5197, 3994, 37), new Point3D(1412, 3793, 0), new Point3D(3688, 2232, 20),
			new Point3D(2578, 604, 0), new Point3D(4397, 1089, 0), new Point3D(5741, 3218, -2),
			new Point3D(2996, 3441, 15), new Point3D(624, 2225, 0), new Point3D(1916, 2814, 0),
		...
		{
			new Point3D(1216, 468, -13), new Point3D(723, 1367, -60), new Point3D(745, 725, -28),
			new Point3D(281, 1017, 0), new Point3D(986, 1011, -32), new Point3D(1175, 1287, -30),
			new Point3D(1533, 1341, -3), new Point3D(529, 217, -44), new Point3D(1722, 219, 96)
		};
		...
		public class ChampionTitleInfo
		{
			...
			public int Glade
			{
				get { return GetValue(ChampionSpawnType.Glade); }
			...
			public int Corrupt
			{
				get { return GetValue(ChampionSpawnType.Corrupt); }
