...
namespace Server.Items
{
	...
	public partial class Teleporter : Item, ITeleporter
	{
		...
		public virtual bool CanTeleport(Mobile m)
		{
			...
			if (!AllowGhosts && m.Player && !m.Alive)
				return false;
		...
		private void DelayedTeleport(Mobile m)
		{
			...
			Timer.DelayCall(m_Delay > TeleportRegion.Delay ? m_Delay : TeleportRegion.Delay, DelayedTeleportCallback,
				m);
		}
		...
		public override bool CanTeleport(Mobile m)
		{
			...
			if (sk == null || sk.Base < m_Required)
			{
				if (m.BeginAction(this))
				{
					...
					{
						m.Send(new MessageLocalized(Serial, ItemID, MessageType.Regular, 0x3B2, 3, m_MessageNumber,
							null, ""));
					}
