...
namespace Server.Multis
{
	...
	{
		public static int AccountHouseLimit
		{
			get => Config.Get("Housing.AccountHouseLimit", 1);
			set => Config.Set("Housing.AccountHouseLimit", value);
		}
		...
		public virtual DecayLevel DecayLevel
		{
			get
			{
				...
				else if (DynamicDecay.Enabled)
				{
					...
					if (stage == DecayLevel.Ageless ||
					    (DynamicDecay.Decays(stage) && NextDecayStage <= DateTime.UtcNow))
		...
		{
			typeof(Engines.Plants.SeedBox), typeof(GardenShedAddon), typeof(GardenShedBarrel),
			typeof(BaseSpecialScrollBook), typeof(JewelryBox)
		};

		private readonly Type[] _NoDecayItems = new Type[]
		{
			typeof(BaseBoard), typeof(Aquarium), typeof(FishBowl), typeof(BaseSpecialScrollBook),
			typeof(Engines.Plants.SeedBox), typeof(JewelryBox), typeof(FermentationBarrel)
		};
		...
		public bool HasActiveAuction
		{
			get
			{
				foreach (Item item in Addons.Keys)
				{
					if (item is AuctionSafe && ((AuctionSafe)item).Auction != null &&
					    ((AuctionSafe)item).Auction.OnGoing)
						return true;
				}
				...
				if (!addon.Deleted)
				{
					...
					if (addon is IAddon)
					{
						...
						if (addon is BaseAddon &&
						    ((BaseAddon)addon).RetainDeedHue) //There are things that are IAddon which aren't BaseAddon
						{
						...
						else
						{
							...
							if (item is IAddon)
							{
								...
								if (item is BaseAddon &&
								    ((BaseAddon)item)
								    .RetainDeedHue) //There are things that are IAddon which aren't BaseAddon
								{
		...
		public virtual bool CheckAosStorage(int need)
		{
			...
			return (GetAosCurSecures(out fromSecures, out fromVendors, out fromLockdowns, out fromMovingCrate) +
			        need) <= GetAosMaxSecures();
		}
		...
		{
			typeof(TenthAnniversarySculpture), typeof(RewardBrazier), typeof(VendorRentalContract), typeof(Dyes),
			typeof(DyeTub), typeof(BaseInstrument), typeof(Clock), typeof(TreasureMap), typeof(RecallRune),
			typeof(Dices), typeof(BaseBoard), typeof(Runebook)
		};

		private readonly Type[] _AccessibleToFriends = { typeof(PotionKeg) };
		...
		{
			BaseDoor northDoor = MakeDoor(wood,
				invert ? DoorFacing.SouthCCW : altopen ? DoorFacing.NorthCCW : DoorFacing.NorthCW);
			BaseDoor southDoor = MakeDoor(wood,
				invert ? DoorFacing.NorthCW : altopen ? DoorFacing.SouthCW : DoorFacing.SouthCCW);
		...
		public void SetLockdown(Mobile m, Item i, bool locked)
		{
			...
			Timer.DelayCall(() =>
				i.PrivateOverheadMessage(MessageType.Regular, 0, locked ? 501721 : 501657,
					m.NetState)); // locked down! : [no longer locked down]
		...
		private class TransferItem : Item
		{
			...
			public override void GetProperties(ObjectPropertyList list)
			{
				...
				bool valid = m_House != null && Sextant.Format(m_House.Location, m_House.Map, ref xLong, ref yLat,
					ref xMins, ref yMins, ref xEast, ref ySouth);

				if (valid)
					location = string.Format("{0}° {1}'{2}, {3}° {4}'{5}", yLat, yMins, ySouth ? "S" : "N", xLong,
						xMins, xEast ? "E" : "W");
				else
					location = "unknown";
			...
			public override bool AllowSecureTrade(Mobile from, Mobile to, Mobile newOwner, bool accepted)
			{
				...
				if (AtAccountHouseLimit(to))
				{
					from.SendLocalizedMessage(
						501388); // You cannot transfer ownership to another house owner or co-owner!
					return false;
				}
			...
			public override void OnSecureTrade(Mobile from, Mobile to, Mobile newOwner, bool accepted)
			{
				...
				if (m_House == null || m_House.Deleted || !m_House.IsOwner(from) || !from.CheckAlive() ||
				    !to.CheckAlive())
					return;
			...
			else if (to.Player)
			{
				...
				else if (CheckTransferPosition(from, to))
				{
					...
					if (HasRentedVendors)
					{
						...
						*/
						to.SendGump(new WarningGump(1060635, 30720, 1062487, 32512, 420, 280, ConfirmTransfer_Callback,
							from));
					}
					else
					{
		...
		public bool Release(Mobile m, Item item)
		{
			...
			if (IsLockedDown(item))
			{
				...
				{
					m.LocalOverheadMessage(MessageType.Regular, 0x3E9,
						1010418); // You did not lock this down, and you are not able to release this.
				}
		...
		public virtual bool IsCombatRestricted(Mobile m)
		{
			...
			for (int i = 0; i < m.Aggressed.Count; ++i)
			{
				...
				if (info.Defender.Player && info.Defender.Alive &&
				    (DateTime.UtcNow - info.LastCombatTime) < HouseRegion.CombatHeatDelay && (attackerGuild == null ||
					    defenderGuild == null ||
		...
		{
			if (item is Container && item.Items.Any(i =>
				    i is CraftableHouseItem || i is CraftableMetalHouseDoor || i is CraftableStoneHouseDoor))
			{
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			foreach (RelocatedEntity relEntity in RelocatedEntities)
			{
				...
				if ((relEntity.Entity is Item && ((Item)relEntity.Entity).Deleted) ||
		...
		{
			List<Item> list =
				new List<Item>(VendorRentalContracts.Where(c => c.RootParent != null || FindHouseAt(c) != this));
		...
		private void CheckUnregisteredAddons()
		{
			...
			foreach (Item item in Region.AllItems)
			{
				if (item is IAddon)
				{
					if ((!Addons.TryGetValue(item, out Mobile owner) || owner == null || owner.Deleted ||
					     !owner.Player))
						Addons[item] = Owner;
				}
		...
		public static bool CheckAccountHouseLimit(Mobile m, bool message = true)
		{
			if (AtAccountHouseLimit(m))
			{
				if (message)
				{
					...
					{
						m.SendMessage("You already own {0} houses, you may not place any more!",
							AccountHouseLimit.ToString());
					}
				}
		...
		public override void OnClick()
		{
			if (Mobile.Alive && BaseHouse.FindHouseAt(Mobile) == House && House.IsOwner(Mobile))
			{
				...
				Mobile.SendLocalizedMessage(
					1159160); // Target the location that you wish to relocate this container. Once selected the container will no longer be secured.
			}
			else
			{
