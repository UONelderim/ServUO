...
namespace Server.Items
{
	...
	public class RefinementItem : Item
	{
		...
		{
			// First check if player is in Tasandora
			if (!IsInTasandora(from))
			{
				// Send message that these services are only available in Tasandora
				from.SendLocalizedMessage(3070080); // "Usługi oczyszczania zbroi są dostępne tylko w mieście Tasandora."
				return false;
			}

			// Look for matching refiner within 12 tiles
			...
			// No matching refiner found - send random "vendor not found" message
			bool useFirstMessage = Utility.RandomBool();
			switch (item.CraftType)
			{
				case RefinementCraftType.Blacksmith:
					from.SendLocalizedMessage(useFirstMessage ? 1154012 : 1154009); 
					// 1154012: "Musisz znaleźć oczyszczacza zbroi kowalskich. Spróbuj poszukać w dzielnicy rzemieślniczej Tasandory."
					// 1154009: "Potrzebujesz pomocy oczyszczacza zbroi kowalskich. Znajdziesz go w dzielnicy rzemieślniczej Tasandory."
					break;
				case RefinementCraftType.Tailor:
					from.SendLocalizedMessage(useFirstMessage ? 1154011 : 1154008);
					// 1154011: "Musisz znaleźć oczyszczacza zbroi krawieckich. Spróbuj poszukać w dzielnicy rzemieślniczej Tasandory."
					// 1154008: "Potrzebujesz pomocy oczyszczacza zbroi krawieckich. Znajdziesz go w dzielnicy rzemieślniczej Tasandory."
					break;
				case RefinementCraftType.Carpenter:
					from.SendLocalizedMessage(useFirstMessage ? 1154013 : 1154010);
					// 1154013: "Musisz znaleźć oczyszczacza zbroi stolarskich. Spróbuj poszukać w dzielnicy rzemieślniczej Tasandory."
					// 1154010: "Potrzebujesz pomocy oczyszczacza zbroi stolarskich. Znajdziesz go w dzielnicy rzemieślniczej Tasandory."
					break;
			}
		...
		// Helper method to check if mobile is in Tasandora
		private static bool IsInTasandora(Mobile from)
		{
			// Define the boundaries of Tasandora
			Rectangle2D tasandoraBounds = new Rectangle2D(
				// Replace these with actual coordinates of Tasandora
				1374, 
				1657,
				1000,
				1000
			...
			return tasandoraBounds.Contains(from.Location);
		}
