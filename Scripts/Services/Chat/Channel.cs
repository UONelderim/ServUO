...
namespace Server.Engines.Chat
{
	public class Channel
	{
		public static void Initialize()
		{
			...
			AddStaticChannel("Pomoc");
			AddStaticChannel("Ogolny");
			AddStaticChannel("Handle");
			AddStaticChannel("Szukam grupy");
		}
		...
		public void SendUsersTo(ChatUser to)
		{
			foreach (ChatUser user in m_Users)
			{
				ChatSystem.SendCommandTo(to.Mobile, ChatCommand.AddUserToChannel,
					user.GetColorCharacter() + user.Username, string.Format("{{{0}}}", m_Name));
			}
		}
