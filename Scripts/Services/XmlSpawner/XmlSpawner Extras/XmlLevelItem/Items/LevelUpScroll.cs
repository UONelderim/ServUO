	...
	public class LevelUpScroll : Item
	{
		...
		public override void AddNameProperties(ObjectPropertyList list)
		{
			...
			if (IsBlacksmithOnly)
			{
				if (!m_BlacksmithValidated)
					BlacksmithMsg = "(Must be validated by player with " + LevelItems.BlacksmithSkillRequired +
					                "+ Blacksmithy skill)";
				else
		...
		public override void OnDoubleClick(Mobile from)
		{
			...
			else
			{
				...
				{
					if (m_BlacksmithValidated ||
					    (from.Skills[SkillName.Blacksmith].Value >= LevelItems.BlacksmithSkillRequired))
					{
					...
					{
						from.SendMessage("Please target another player with a base Blacksmith skill of " +
						                 LevelItems.BlacksmithSkillRequired + " or higher.");
			...
			protected override void OnTarget(Mobile from, object target)
			{
				...
				else if (target is Item)
				{
					...
					if (item.RootParent != from ||
					    !item.IsChildOf(from.Backpack)) // Make sure its in their pack or they are wearing it
					{
					...
					else
					{
						...
						{
							from.SendMessage(
								"Invalid Target Type.  Levelable Weapons, Armor, Jewelery and Clothing only!");
						}
					}
				}
				else
		...
		private class BlacksmithTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object target)
			{
				if (target is PlayerMobile)
				{
					...
					{
						from.SendMessage(
							"This players blacksmith skill is not high enough to increase levels on items.");
					}
					else
					{
