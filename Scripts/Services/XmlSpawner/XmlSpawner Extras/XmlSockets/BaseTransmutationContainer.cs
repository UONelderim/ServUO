...
namespace Server.Items
{
	public class BaseTransmutationContainer : Container
	{
			...
			if (s.Skills != null && s.MinSkillLevel != null)
			{
				...
				for (int i = 0; i < s.Skills.Length; i++)
				{
					...
					if (i < s.MinSkillLevel.Length)
					{
						...
						{
							from.SendMessage("Need {0} {1} to transmute this", s.MinSkillLevel[i],
								s.Skills[i].ToString());
							return false;
						}
					}
		...
		public void Transmute(Mobile from)
		{
			...
			{
				if (r.Ingredients == null || r.Ingredients.Length == 0 || r.Quantity == null ||
				    r.Ingredients.Length != r.Quantity.Length) continue;
				...
				foreach (Item i in Items)
				{
					...
					for (int j = 0; j < r.Ingredients.Length; j++)
					{
						                                 ...
						                                 (rt.IsInterface && ContainsInterface(it.GetInterfaces(), rt))))
						{
							...
							if (r.PropertyTests != null && r.PropertyTests[j] != null &&
							    !BaseXmlSpawner.CheckPropertyString(null, i, r.PropertyTests[j], out status_str))
							{
