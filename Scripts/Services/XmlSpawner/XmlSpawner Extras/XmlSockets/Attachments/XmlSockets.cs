...
namespace Server.Engines.XmlSpawner2
{
	public class XmlSockets : XmlAttachment
	{
		...
		public static int
			MaxAugmentDistance =
				2; // if socketed object isnt in pack, then this is the max distance away from it you can be and still augment

		public static int
			MaxSocketDistance =
				2; // if socketable object isnt in pack, then this is the max distance away from it you can be and still socket

		private ArrayList m_SocketOccupants;

		private bool
			m_MustAugmentInPack =
				true; // determines whether the socketed object must be in the players pack in order to augment
		...
		public class LimitSocketsCommand : BaseCommand
		{
			...
			public LimitSocketsCommand()
			{
				...
				Description =
					"Finds XmlSocket attachments with more than maxsockets, recovers their augmentations and reduces the socket number";
				ListOptimized = true;
			}
		...
		public class UpgradeAugmentCommand : BaseCommand
		{
			...
			public UpgradeAugmentCommand()
			{
				...
				Description =
					"Upgrades the specified augmentation on objects from the oldversion number to the newversion number.";
				ListOptimized = true;
			}
			...
			public override void ExecuteList(CommandEventArgs e, ArrayList list)
			{
				...
				for (int i = 0; i < list.Count; ++i)
				{
					...
					if (s != null && s.SocketOccupants != null)
					{
						...
						while (found && nchange <= maxsock)
						{
							...
							{
								SocketOccupant so = s.SocketOccupants[j] as SocketOccupant;
								if (so != null && so.OccupantType == m_augmenttype &&
								    so.AugmentationVersion == m_oldversion)
								{
									// recover the old version of the augment
									BaseSocketAugmentation augment = s.RecoverAugmentation(null, targetobject, j);
				...
				AddResponse(String.Format("{0} {1} augmentations upgraded on {2} objects. {3} failures.", naugments,
					m_augmenttype.Name, nobjects, nfailures));
			}
		...
		public static void ConfigureRandom(object target, double dropchance, double chance5, double chance4,
			double chance3, double chance2, double chance1)
		{
		...
		public static void ConfigureRandom(object target, double dropchance, double chance5, double chance4,
			double chance3,
			double chance2, double chance1, SkillName skillname, double minskill, SkillName skillname2,
			double minskill2, Type resource, int quantity)
		{
			if (Utility.Random(1000) < (int)(dropchance * 10))
			{
				// compute chance of having sockets
				...
				if (socklevel < (int)(chance5 * 10))
					XmlAttach.AttachTo(target,
						new XmlSocketable(5, skillname, minskill, skillname2, minskill2, resource, quantity));
				else if (socklevel < (int)(chance4 + chance5) * 10)
					XmlAttach.AttachTo(target,
						new XmlSocketable(4, skillname, minskill, skillname2, minskill2, resource, quantity));
				else if (socklevel < (int)(chance3 + chance4) * 10)
					XmlAttach.AttachTo(target,
						new XmlSocketable(3, skillname, minskill, skillname2, minskill2, resource, quantity));
				else if (socklevel < (int)(chance2 + chance3) * 10)
					XmlAttach.AttachTo(target,
						new XmlSocketable(2, skillname, minskill, skillname2, minskill2, resource, quantity));
				else
					XmlAttach.AttachTo(target,
						new XmlSocketable(1, skillname, minskill, skillname2, minskill2, resource, quantity));
			}
		}
		...
		public static int ComputeSuccessChance(Mobile from, int nsockets, double difficulty, SkillName requiredSkill,
			double difficulty2, SkillName requiredSkill2)
		{
			if (from == null) return 0;
		...
		public static bool AddSocket(Mobile from, object target, int maxSockets, double difficulty,
			SkillName requiredSkill, Type resource, int quantity)
		{
			...
			double difficulty2, SkillName requiredSkill2, Type resource, int quantity)
		{
			...
			int successchance =
				ComputeSuccessChance(from, nSockets, difficulty, requiredSkill, difficulty2, requiredSkill2);
		...
		public static bool Augment(Mobile from, object parent, XmlSockets sock, int socketnum, IXmlSocketAugmentation a)
		{
			...
			if (sock.SocketOccupants != null && sock.NFree >= a.SocketsRequired &&
			    sock.SocketOccupants.Count > socketnum && a.OnAugment(from, parent))
			{
				SocketOccupant occ = new SocketOccupant(a.GetType(), a.Version, a.Icon, a.IconXOffset, a.IconYOffset,
					a.IconHue, a.UseGumpArt,
		...
		public BaseSocketAugmentation RecoverAugmentation(Mobile from, object o, int index)
		{
			...
			if (augment != null)
			{
				if (augment.CanRecover(from, o, s.AugmentationVersion) &&
				    augment.RecoverableSockets(s.AugmentationVersion) > 0)
				{
					// recover the augment
					...
					{
						if (SocketOccupants[i] != null && ((SocketOccupant)SocketOccupants[i]).OccupantType == t &&
						    ((SocketOccupant)SocketOccupants[i]).AugmentationVersion == s.AugmentationVersion)
						{
		...
		private class AddAugmentationToSocket : Target
		{
			...
			public AddAugmentationToSocket(object parent, XmlSockets s, int socketnum) : base(30, false,
				TargetFlags.None)
			{
				m_parent = parent;
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is IXmlSocketAugmentation)
				{
					...
					// check the parent
					if (m_parent is Item && !((Item)m_parent).IsChildOf(from.Backpack) && m_s != null &&
					    m_s.MustAugmentInPack)
					{
		...
		public class AddSocketToTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				AddSocket(from, targeted, maxSockets, difficulty, skillname, difficulty2, skillname2, resource,
					quantity);
			}
		}
		...
		{
			public XmlSockets m_attachment;
			public const int vertspacing = 65;
			...
			public SocketsGump(Mobile from, XmlSockets a) : base(0, 0)
			{
				...
				else if (m_attachment.AttachedTo is Mobile)
				{
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				if (m_attachment == null || state == null || state.Mobile == null || info == null ||
				    m_attachment.SocketOccupants == null) return;

				// make sure the parent of the socket is still valid
				if (m_attachment.AttachedTo == null ||
				    (m_attachment.AttachedTo is Item && ((Item)m_attachment.AttachedTo).Deleted) ||
				...
				for (int i = 0; i < m_attachment.SocketOccupants.Count; i++)
				{
					...
					if (info.ButtonID == i + 2000)
					{
						if (s != null)
						{
							...
							if (augment != null)
							{
								canrecover = augment.CanRecover(state.Mobile, m_attachment.AttachedTo,
									s.AugmentationVersion);
								augment.Delete();
							}
