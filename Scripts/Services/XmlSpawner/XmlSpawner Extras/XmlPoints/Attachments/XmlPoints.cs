...
namespace Server.Engines.XmlSpawner2
{
	public class XmlPoints : XmlAttachment
	{
		...
		private bool
			m_Broadcast =
				true; // flag that determines whether pvp results will be broadcast.  Broadcast is determined by the killer's flag.
		...
		public bool HasChallenge
		{
			get
			{
				return ((m_Challenger != null && !m_Challenger.Deleted) ||
		...
		public static bool
			TeleportOnDuel = true; // are players automatically teleported to and from the specified dueling location

		private static TimeSpan
			m_DeathDelay = TimeSpan.FromSeconds(60); // 60 seconds default min time between deaths for point loss

		private static TimeSpan
			m_KillDelay =
				TimeSpan.FromHours(6); // 6 hour default min interval between kills of the same player for point gain
		...
		private static double
			m_WinScale = 0.05; // set to zero for no scaling for points gained for killing (fixed 1 point per kill)

		private static double
			m_LoseScale = 0.05; // set to zero for no scaling for points lost when killed (fixed 1 point per death)
		...
		private static TimeSpan
			m_LeaderboardSaveInterval = TimeSpan.FromMinutes(15); // default time interval between leaderboard saves

		private static string
			m_LeaderboardSaveDirectory = "Leaderboard"; // default directory for saving leaderboard xml information

		private static int
			m_LeaderboardSaveRanks =
				20; // number of ranked players to save to the leaderboard.  0 means save all players.

		private static TimeSpan
			m_PointsDecayTime =
				TimeSpan.FromDays(15); // default time interval for automatic point loss for no pvp activity
		...
		public static bool
			UnrestrictedChallenges =
				false; // allow the normal waiting time between kills for points to be overridden for challenges
		...
		public string Text(int index)
		{
			...
			if (h == null || !h.Contains(index))
			{
				...
				if (h == null || !h.Contains(index))
				{
					Console.WriteLine("XmlPoints: Also missing entry for {0} in system language {1}", index,
						m_SystemLanguage);
					...
					if (h == null || !h.Contains(index))
					{
						Console.WriteLine("XmlPoints: And finally missing entry for {0} in default language {1}", index,
		...
		public static new void Initialize()
		{
			...
			CommandSystem.Register("TeamLMSChallenge", AccessLevel.Player,
			...
			CommandSystem.Register("TeamDeathmatch", AccessLevel.Player,
			...
			CommandSystem.Register("KingOfTheHill", AccessLevel.Player,
				new CommandEventHandler(KingOfTheHill_OnCommand));
			CommandSystem.Register("TeamDeathBall", AccessLevel.Player,
			...
			CommandSystem.Register("SystemBroadcastKills", AccessLevel.GameMaster,
			...
			CommandSystem.Register("BroadcastKills", AccessLevel.Player,
			...
			CommandSystem.Register("AddAllPoints", AccessLevel.Administrator,
				new CommandEventHandler(AddAllPoints_OnCommand));
			CommandSystem.Register("RemoveAllPoints", AccessLevel.Administrator,
				new CommandEventHandler(RemoveAllPoints_OnCommand));
			CommandSystem.Register("LeaderboardSave", AccessLevel.Administrator,
...
#else
			sw.WriteLine(
				"<TR><TH><TH>Player Name<TH>Guild<TH>Points<TH>Kills<TH>Deaths<TH>Rank<TH>Change<TH>Time at current rank");
#endif
			// go through the sorted list and display the top ranked players
		...
		public static void LeaderboardSave_OnCommand(CommandEventArgs e)
		{
			if (e.Arguments.Length > 0)
			{
				...
				if (e.Arguments[0].ToLower() != "off")
				{
					...
					m_LeaderboardTimer = new LeaderboardTimer(m_LeaderboardFile, m_LeaderboardSaveInterval,
						m_LeaderboardSaveRanks);
		...
		public static bool AreChallengers(Mobile from, Mobile target)
		{
			...
			if (afrom.m_ChallengeGame != null && !afrom.m_ChallengeGame.Deleted &&
		...
		public static bool AreInAnyGame(Mobile target)
		{
			...
			if (atarget != null && !atarget.Deleted && atarget.m_ChallengeGame != null &&
				...
				if (targeted is Mobile && ((Mobile)targeted).Player)
				{
					...
					if (a != null && !a.Deleted)
					{
						...
						if (a.Challenger != null)
						{
							...
							{
								a.SendText(100209,
									XmlPoints.CancelTimeout
										.TotalMinutes); // "Canceling current challenge.  Please wait {0} minutes"

								SendText(a.Challenger, 100210, from.Name,
									XmlPoints.CancelTimeout
										.TotalMinutes); // "{0} is canceling the current challenge. {1} minutes remain"
						...
						if (xa != null && !xa.Deleted)
						{
							...
							{
								from.SendMessage(String.Format(a.Text(100211),
									pm.Name)); // "{0} is already being challenged."
								return;
							}
						}
			...
			protected override void OnTick()
			{
				...
				if (xa != null && !xa.Deleted)
				{
					...
					if (xa.Challenger == from)
					{
						...
						{
							m_attachment.Challenger.SendMessage(String.Format(xa.Text(100214),
								from.Name)); // "Challenge with {0} has been cancelled"
						}
		...
		public override void Serialize(GenericWriter writer)
		{
			...
			if (m_Kills > 0 && m_PointsDecay > 0 && m_Points > DefaultStartingPoints &&
			    ...
			    (DateTime.UtcNow - m_LastKill) > m_PointsDecayTime &&
			    (DateTime.UtcNow - m_LastDeath) > m_PointsDecayTime)
			{
		...
		public override void OnKill(Mobile killed, Mobile killer)
		{
			...
			{
				BroadcastMessage(AccessLevel.Player, 0x482,
		...
		public override string OnIdentify(Mobile from)
		{
			...
			msg.AppendFormat("\n");
			msg.AppendFormat(GetText(from, 100222), Kills,
				Deaths); // "Total Kills = {0}\nTotal Deaths = {1}\nRecent Kill List"
		...
		*/
		...
		public class PointsGump : Gump
		{
			...
			public PointsGump(XmlPoints a, Mobile from, Mobile target, string text) : base(0, 0)
			{
				...
				if (a.Challenger != null)
				{
					...
					if (ca != null && !ca.Deleted)
					{
						if ((ca.m_CancelTimer != null && ca.m_CancelTimer.Running) ||
						    (ca.ChallengeGame != null && ca.ChallengeGame.ChallengeBeingCancelled))
							challengehue = 33;
					}


					AddLabel(20, 143, challengehue,
						String.Format(GetText(from, 200225), a.Challenger.Name)); // "Currently challenging {0}"
				}
				else
				...
				if (from == target)
				{
					...
					AddButton(x2, 165, (a.Broadcast ? 0xD3 : 0xD2), (a.Broadcast ? 0xD2 : 0xD3), 200,
						GumpButtonType.Reply, 0);
		...
		private class IssueChallengeGump : Gump
		{
			...
			public IssueChallengeGump(Mobile from, Mobile target) : base(0, 0)
			{
				...
				if (TeleportOnDuel && SpellHelper.CheckTravel(from.Map, from.Location, TravelCheckType.RecallFrom) &&
				    SpellHelper.CheckTravel(target.Map, target.Location, TravelCheckType.RecallFrom))
				{
			...
			public override void OnResponse(NetState state, RelayInfo info)
			{
				...
				switch (info.ButtonID)
				{
					default:
					{
						...
						{
							// issue the challenge
							m_Target.SendGump(new ConfirmChallengeGump(m_From, m_Target,
								XmlPoints.DuelLocations[radiostate - 2]));
							m_From.SendMessage(GetText(m_From, 100257),
								m_Target.Name); // "You have issued a challenge to {0}."
						}
						...
						{
							if (m_From != null)
								m_From.SendMessage(GetText(m_From, 100258),
									m_Target.Name); // "You decided against challenging {0}."
						}
		...
		private class ConfirmChallengeGump : Gump
		{
			...
			public ConfirmChallengeGump(Mobile from, Mobile target, DuelLocationEntry duelloc) : base(0, 0)
			{
				...
				{
					AddLabel(20, y, 0,
						String.Format(GetText(target, 200649), GetText(target, 200661))); // "Location: Duel Here"
				}
