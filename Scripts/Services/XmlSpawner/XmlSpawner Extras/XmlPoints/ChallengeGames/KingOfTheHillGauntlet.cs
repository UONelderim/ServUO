	...
	public class KingOfTheHillGauntlet : BaseChallengeGame
	{
		...
		public static bool
			OnlyInChallengeGameRegion =
				true; // if this is true, then the game can only be set up in a challenge game region
		...
		public void CheckForDisqualification()
		{
			...
			foreach (IChallengeEntry entry in Participants)
			{
				if (entry.Participant == null || entry.Status == ChallengeStatus.Forfeit ||
				    entry.Status == ChallengeStatus.Disqualified) continue;
		...
		public KingOfTheHillGauntlet(Mobile challenger) : base(0x1414)
		{
			...
			{
				Name = XmlPoints.SystemText(100410) + " " +
