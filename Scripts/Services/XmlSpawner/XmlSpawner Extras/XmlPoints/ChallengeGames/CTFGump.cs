			...
			: base(20, 30)
		{
			...
			if (gauntlet.Participants != null)
			{
				...
				if (gauntlet.MatchLength > TimeSpan.Zero)
					AddLabel(150, 110, 0,
						String.Format(XmlPoints.GetText(from, 200563), gauntlet.MatchLength)); // "Match Length: {0}"
				else
				...
				AddLabel(220, 130, 0,
					String.Format("Page: {0}/{1}", viewpage + 1, (int)(m_WorkingList.Count / PlayersPerPage) + 1));

				if (gauntlet.GameInProgress && gauntlet.MatchLength > TimeSpan.Zero)
				{
					AddLabelCropped(20, 130, 180, 21, 0, String.Format(
						XmlPoints.GetText(from, 200565), // "Time left {0}"
						TimeSpan.FromSeconds(
							(double)((int)((gauntlet.MatchStart + gauntlet.MatchLength - DateTime.UtcNow)
								.TotalSeconds)))));
				}
				else if (gauntlet.GameCompleted && gauntlet.MatchLength > TimeSpan.Zero)
				{
					AddLabelCropped(20, 130, 180, 21, 0, String.Format(
						XmlPoints.GetText(from, 200565), // "Time left {0}"
						TimeSpan.FromSeconds(
							(double)((int)((gauntlet.MatchStart + gauntlet.MatchLength - gauntlet.MatchEnd)
								.TotalSeconds)))));
				}
						...
						if (selection < m_WorkingList.Count)
						{
							...
							if (m_gauntlet.Participants != null)
							{
								...
								if (kickentry != null)
								{
									...
									{
										XmlPoints.SendText(kickentry.Participant, 100545,
											m_gauntlet.ChallengeName); // "You have been kicked from {0}"
