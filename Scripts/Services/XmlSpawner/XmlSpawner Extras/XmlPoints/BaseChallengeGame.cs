	...
	public abstract class BaseChallengeGame : Item, IChallengeGame
	{
		...
		public virtual bool CollectEntryFee(Mobile m, int amount)
		{
			...
			if (m.BankBox != null)
			{
				...
				{
					XmlPoints.SendText(m, 100541,
						amount); // "Could not withdraw the Entry fee of {0} gold from your bank."
					return false;
				}
			}
			else
		...
		public static void DoSetupChallenge(Mobile from, int nameindex, Type gametype)
		{
			if (from != null && gametype != null)
			{
				...
				if (r is ChallengeGameRegion)
				{
					...
					{
						from.SendMessage(String.Format(XmlPoints.GetText(from, 100303),
							XmlPoints.GetText(from,
								nameindex))); //"Unable to set up a {0} Challenge: Another Challenge Game is already in progress in this Challenge Game region.", "Last Man Standing"
						return;
					}
				}
				else if (onlyinchallenge)
				{
					from.SendMessage(String.Format(XmlPoints.GetText(from, 100304),
						XmlPoints.GetText(from,
							nameindex))); // "Unable to set up a {0} Challenge: Must be in a Challenge Game region.", "Last Man Standing"
					return;
				}
				...
				if (g == null || g.Deleted)
				{
					from.SendMessage(String.Format(XmlPoints.GetText(from, 100305),
						XmlPoints.GetText(from,
							nameindex))); // "Unable to set up a {0} Challenge.", "Last Man Standing"
					return;
				}


				g.MoveToWorld(from.Location, from.Map);
				from.SendMessage(String.Format(XmlPoints.GetText(from, 100306),
					XmlPoints.GetText(from, nameindex))); // "Setting up a {0} Challenge.", "Last Man Standing"
				...
				if (r is ChallengeGameRegion)
				{
					...
					XmlPoints.BroadcastMessage(AccessLevel.Player, 0x482,
						String.Format(XmlPoints.SystemText(100307), XmlPoints.SystemText(nameindex), r.Name,
							from.Name)); // "{0} Challenge being prepared in '{1}' by {2}", "Last Man Standing"
				}
					...
					foreach (IChallengeEntry entry in Participants)
					{
						...
						if (from != null && from.BankBox != null && EntryFee > 0)
						{
							...
							if (!from.BankBox.TryDropItem(from, gold, false))
							{
								...
								XmlPoints.SendText(from, 100000,
									EntryFee); // "Entry fee of {0} gold has been returned to you."
							}
							else
							{
								XmlPoints.SendText(from, 100001,
									EntryFee); // "Entry fee of {0} gold has been returned to your bank account."
							}
						}
				...
				foreach (IChallengeEntry entry in Participants)
				{
					if (entry.Participant != from)
					{
						from.Send(new MobileMoving(entry.Participant, Notoriety.Compute(from, entry.Participant),
							entry.Participant.GetPacketFlags()));
					}
				}
			}
		}
