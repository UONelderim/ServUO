...
namespace Server.Engines.XmlSpawner2
{
	public class MobFactionRegionStone : Item
	{
		...
		public void RefreshRegions()
		{
			...
			m_MobFactionRegion = new MobFactionRegion(m_MobFactionRegionName, m_MobFactionMap, m_MobFactionPriority,
				m_MobFactionArea);
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				case 1:
				{
					...
					{
						m_FactionType =
							(XmlMobFactions.GroupTypes)Enum.Parse(typeof(XmlMobFactions.GroupTypes), factiontype);
					}
					catch { }
		...
		private static void MobFactionRegionArea_Callback(Mobile from, Map map, Point3D start, Point3D end,
			object state)
		{
			...
			if (s != null && from != null)
			{
				...
				s.m_MobFactionArea[0] =
					Region.ConvertTo3D(new Rectangle2D(start.X, start.Y, end.X - start.X + 1, end.Y - start.Y + 1));
