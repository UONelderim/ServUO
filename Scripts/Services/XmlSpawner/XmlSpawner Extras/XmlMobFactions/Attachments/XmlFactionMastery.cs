...
namespace Server.Engines.XmlSpawner2
{
	public class XmlFactionMastery : XmlAttachment
	{
		...
		private double
			m_FactionScaling =
				0.002; // faction multiplier for damage bonus. By default, 0.002% increase in damage per total faction level
		...
		[Attachable]
		public XmlFactionMastery(string enemyfaction, int hitchance, int basepercentincrease, int percentcap,
			double expiresin)
		{
			this.m_Chance = hitchance;
			this.m_PercentIncrease = basepercentincrease;
			...
			set
			{
				...
				try
				{
					if (value == "?")
					{
						// randomly pick one
						this.m_EnemyType =
							(XmlMobFactions.GroupTypes)Utility.Random((int)XmlMobFactions.GroupTypes.End_Unused);
					}
					else
						this.m_EnemyType =
							(XmlMobFactions.GroupTypes)Enum.Parse(typeof(XmlMobFactions.GroupTypes), value, true);
				}
				catch
				{
				}
		...
		public override void OnWeaponHit(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven)
		{
			...
			if (defender != null && attacker != null && this.m_EnemyType != XmlMobFactions.GroupTypes.End_Unused)
			{
				...
				var list = XmlAttach.FindAttachments(XmlAttach.MobileAttachments, attacker, typeof(XmlMobFactions),
					"Standard");
							...
							if (targetgroup == this.m_EnemyType)
							{
								...
								if (g.Opponents != null)
								{
									...
									for (int i = 0; i < g.Opponents.Length; i++)
									{
										...
										try
										{
											totalfac += (int)(x.GetFactionLevel(g.Opponents[i].GroupType) *
											                  g.OpponentGain[i]);
										}
										catch
										{
		...
		public override string OnIdentify(Mobile from)
		{
			if (from == null)
			{
				if (this.Expiration > TimeSpan.Zero)
				{
					return String.Format(
						"Faction Mastery : increased damage vs {0}, {1}% hitchance, expires in {2} mins",
						this.m_Enemy, this.Chance, this.Expiration.TotalMinutes);
				}
				else
				{
					return String.Format("Faction Mastery : increased damage vs {0}, {1}% hitchance", this.m_Enemy,
						this.Chance);
				}
			}
			...
			{
				msg = String.Format(
					"Faction Mastery : +{3}% damage vs {0} ({4}% max), {1}% hitchance, expires in {2} mins",
					this.m_Enemy, this.Chance, this.Expiration.TotalMinutes, percentincrease, this.PercentCap);
			}
			else
			{
				msg = String.Format("Faction Mastery : +{2}% damage vs {0} ({3}% max), {1}% hitchance", this.m_Enemy,
					this.Chance, percentincrease, this.PercentCap);
			}
