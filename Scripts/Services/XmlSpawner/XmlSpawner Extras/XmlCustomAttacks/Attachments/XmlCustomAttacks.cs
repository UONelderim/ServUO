...
namespace Server.Engines.XmlSpawner2
{
	public class XmlCustomAttacks : XmlAttachment
	{
		...
		public static new void Initialize()
		{
			...
			AddSpecialAttack("Triple Slash",
				"Deals triple the max damage of the weapon", // attack name, and description
				SpecialAttacks.TripleSlash, 0x520C,
				TimeSpan.FromSeconds(3.5), // attack id, id of gump icon, and chaining time
			...
			AddSpecialAttack("Mind Drain", "Drains mana from the target",
				SpecialAttacks.MindDrain, 0x5007, IconTypes.GumpID,
				TimeSpan.FromSeconds(3.5), // explicitly specifying the gump icon as a gumpid
				10, 5, 5, 0,
				0, 0, 40,
			...
			AddSpecialAttack("Vortex Strike", "Calls an energy vortex to aid you in battle",
				SpecialAttacks.VortexStrike, 0x20b9, IconTypes.ItemID,
				TimeSpan.FromSeconds(3.5), // example of using an itemid for the gump icon
				40, 20, 0, 0,
				0, 0, 30,
				new SkillName[] { SkillName.Magery },
				new int[] { 60 },
			...
			AddSpecialAttack("Stam Drain", "Drains stamina from the target",
				SpecialAttacks.StamDrain, 0x500e,
				TimeSpan.FromSeconds(3.5), // if the icon type is not specified, gump icons use gumpids
				30, 5, 0, 0,
				40, 40, 0,
				null,
				null,
			...
			AddComboAttack("Thunder Strike", ComboAttacks.ThunderStrike, // combo name, and id
				...
				{
					SpecialAttacks.TripleSlash, SpecialAttacks.MindDrain, SpecialAttacks.ParalyzingFear,
					SpecialAttacks.TripleSlash, SpecialAttacks.StamDrain
				}
			);
			...
			AddComboAttack("Lightning Rain", ComboAttacks.LightningRain,
				new SpecialAttacks[]
				{
					SpecialAttacks.TripleSlash, SpecialAttacks.MindDrain, SpecialAttacks.MindDrain,
					SpecialAttacks.StamDrain
				}
			);

			AddComboAttack("Squeezing Fist", ComboAttacks.SqueezingFist,
				new SpecialAttacks[] { SpecialAttacks.MindDrain, SpecialAttacks.StamDrain }
			);
		...
		//
		public void DoSpecialAttack(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven,
			SpecialAttack special)
		{
			...
			switch (special.AttackID)
			{
				...
				case SpecialAttacks.VortexStrike:
				{
					...
					BaseCreature.Summon(new EnergyVortex(true), false, attacker, defender.Location, 0x212,
		...
		//
		public void DoComboAttack(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven,
			ComboAttack combo)
		{
		...
		public class SpecialAttack
		{
			...
			public SpecialAttack(string name, string description, SpecialAttacks id, int icon, IconTypes itype,
				TimeSpan duration,
		...
		private static void AddSpecialAttack(string name, string description, SpecialAttacks id, int icon,
			TimeSpan duration,
			int mana, int stam, int hits, int karma, int minstr, int mindex, int minint,
			SkillName[] skills, int[] minlevel, Type[] reagents, int[] quantity)
		{
		...
		private static void AddSpecialAttack(string name, string description, SpecialAttacks id, int icon,
			IconTypes itype, TimeSpan duration,
			int mana, int stam, int hits, int karma, int minstr, int mindex, int minint,
			SkillName[] skills, int[] minlevel, Type[] reagents, int[] quantity)
		{
		...
		private void CheckCombos(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven, SpecialAttack s)
		{
			...
			foreach (ActiveCombo c in Combos)
			{
				if (c != null && c.Combo != null && c.Combo.AttackSequence != null &&
				    c.PositionInSequence < c.Combo.AttackSequence.Length)
				{
			...
			if (s.Skills != null && s.MinSkillLevel != null)
			{
				...
				for (int i = 0; i < s.Skills.Length; i++)
				{
					...
					if (i < s.MinSkillLevel.Length)
					{
						...
						{
							from.SendMessage("Need {0} {1} to perform {2}", s.MinSkillLevel[i], s.Skills[i].ToString(),
								s.Name);
							return false;
						}
					}
			...
			public CustomAttacksGump(Mobile from, XmlCustomAttacks a) : base(0, 0)
			{
				...
				for (int i = 0; i < specialcount; i++)
				{
					...
					*/
					...
					if (m_attachment != null && m_attachment.m_SelectedAttack != null &&
