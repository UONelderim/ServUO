	...
	public class XmlCustomDefenses : XmlAttachment
	{
		...
		public static new void Initialize()
		{
			...
			AddSpecialDefense("Shield of Spikes", "Returns damage to an attacker",
				SpecialDefenses.SpikeShield, 0x2086, IconTypes.ItemID,
				TimeSpan.FromSeconds(10.5), // example of using an itemid for the gump icon
				0, 20, 0, 0,
				30, 30, 0,
				null,
				null,
			...
			AddSpecialDefense("Mind Drain", "Drains mana from the target",
				SpecialDefenses.MindDrain, 0x5007, IconTypes.GumpID,
				TimeSpan.FromSeconds(10.5), // explicitly specifying the gump icon as a gumpid
				10, 5, 5, 0,
				0, 0, 40,
			...
			AddSpecialDefense("Stam Drain", "Drains stamina from the target",
				SpecialDefenses.StamDrain, 0x500e,
				TimeSpan.FromSeconds(10.5), // if the icon type is not specified, gump icons use gumpids
				30, 5, 0, 0,
				40, 40, 0,
				null,
				null,
			...
			AddComboDefense("Cold Wind", ComboDefenses.ColdWind,
				new SpecialDefenses[]
				{
					SpecialDefenses.SpikeShield, SpecialDefenses.SpikeShield, SpecialDefenses.StamDrain
				}
			);
		...
		//
		public int DoSpecialDefense(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven,
		...
		//
		public void DoComboDefense(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven,
			ComboDefense combo)
		{
		...
		public class SpecialDefense
		{
			...
			public SpecialDefense(string name, string description, SpecialDefenses id, int icon, IconTypes itype,
				TimeSpan duration,
		...
		private static void AddSpecialDefense(string name, string description, SpecialDefenses id, int icon,
			TimeSpan duration,
			int mana, int stam, int hits, int karma, int minstr, int mindex, int minint,
			SkillName[] skills, int[] minlevel, Type[] reagents, int[] quantity)
		{
		...
		private void CheckCombos(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven, SpecialDefense s)
		{
			...
			foreach (ActiveCombo c in Combos)
			{
				if (c != null && c.Combo != null && c.Combo.DefenseSequence != null &&
				    c.PositionInSequence < c.Combo.DefenseSequence.Length)
				{
		...
		{
			if (attacker == null || defender == null || weapon == null || armor == null ||
			    m_SelectedDefense == null) return 0;
			...
			public CustomDefenseGump(Mobile from, XmlCustomDefenses a) : base(0, 0)
			{
				...
				for (int i = 0; i < specialcount; i++)
				{
					...
					*/
					...
					if (m_attachment != null && m_attachment.m_SelectedDefense != null &&
