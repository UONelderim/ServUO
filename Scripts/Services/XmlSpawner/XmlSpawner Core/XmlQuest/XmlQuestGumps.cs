...
namespace Server.Gumps
{
	...
	public class XmlSimpleGump : Gump
	{
		...
		public XmlSimpleGump(object invoker, string gumptext, string gumptitle, int gumptype,
			BaseXmlSpawner.KeywordTag tag, Mobile from) : this(invoker, gumptext, gumptitle, gumptype, tag, from, null)
		{
		}

		public XmlSimpleGump(object invoker, string gumptext, string gumptitle, int gumptype,
			BaseXmlSpawner.KeywordTag tag, Mobile from, XmlGumpCallback gumpcallback) : base(0, 0)
		{
			...
			else if (gumptype == 4)
			{
				...
				for (int i = 0; i < gumpSelections.Count; i++)
				{
					...
					AddHtml(137, y + 4, 250, 40,
						XmlSimpleGump.Color(((GumpSelection)gumpSelections[i]).Selection, "FFFFFF"), false, false);
				}
