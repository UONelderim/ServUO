	...
	public abstract class XmlQuestHolder : Container, IXmlQuest
	{
		...
		public override void OnAdded(IEntity target)
		{
			...
			if ((target != null) && target is Container)
			{
				...
				else if ((parentOfTarget != null) && (parentOfTarget is PlayerMobile) && PlayerMade &&
				         (Owner != null) && ((Owner == Creator) || (Creator == null)))
				{
					// check the old owner
					CheckOwnerFlag();
				...
				else
				{
					...
					if (holder != null)
					{
						if (notify)
						{
							...
							holder.SendMessage(JournalNotifyColor,
								"Journal entry '{0}' of quest '{1}' has been modified.", entryID, Name);
						}
			...
			else
			{
				...
				if (entryText != null && entryText.Length != 0)
				{
					...
					if (holder != null)
					{
						...
						{
							// notify the player holding the questholder                       
							holder.SendMessage(JournalNotifyColor, "Journal entry '{0}' has been added to quest '{1}'.",
								entryID, Name);
						}
