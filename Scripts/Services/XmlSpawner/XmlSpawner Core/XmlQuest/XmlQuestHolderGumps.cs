...
namespace Server.Gumps
{
	public class XmlQuestStatusGump : Gump
	{
		...
		public void DisplayQuestStatus(int x, int y, string objectivestr, string statestr, bool status,
				...
				if (arglist.Length > 0)
					switch (arglist[0])
					{
						case "GIVE":
							...
							XmlQuest.CheckArgList(arglist, 3, null, out typestr, out targetcount, out checkprop,
								out status_str);
						...
						case "KILL":
							...
							XmlQuest.CheckArgList(arglist, 2, null, out typestr, out targetcount, out checkprop,
								out status_str);
				...
				if (foundkill)
				{
					...
					else
					{
						if (name != null)
						{
							...
							text = String.Format("Kill {0} {1}(s) named {2} ({3} left)", targetcount, type, name,
								remaining);
						}
						else
				...
				else if (foundcollect)
				{
					...
					else
					{
						if (name != null)
						{
							...
							text = String.Format("Collect {0} {1}(s) named {2} ({3} left)", targetcount, type, name,
								remaining);
						}
						else
				...
				else if (foundgive)
				{
					...
					else
					{
						if (name != null)
						{
							...
							text = String.Format("Give {0} {1}(s) named {2} to {3} ({4} left)", targetcount, type, name,
								mobname, remaining);
						}
						else
							text = String.Format("Give {0} {1}(s) to {2} ({3} left)", targetcount, type, mobname,
								remaining);
					}
				}
				else
			...
			: base(X, Y)
		{
			...
			else
			{
				...
				DisplayQuestStatus(130, 192, questitem.Objective1, questitem.State1, questitem.Completed1,
					questitem.Description1);
				DisplayQuestStatus(130, 224, questitem.Objective2, questitem.State2, questitem.Completed2,
					questitem.Description2);
				DisplayQuestStatus(130, 256, questitem.Objective3, questitem.State3, questitem.Completed3,
					questitem.Description3);
				DisplayQuestStatus(130, 288, questitem.Objective4, questitem.State4, questitem.Completed4,
					questitem.Description4);
				DisplayQuestStatus(130, 320, questitem.Objective5, questitem.State5, questitem.Completed5,
				...
				if ((questitem.RewardItem != null && !questitem.RewardItem.Deleted))
				{
					...
					if (questitem.RewardItem.Amount > 1)
					{
						...
						AddLabel(230, 373, 55,
							String.Format("Weight: {0}", questitem.RewardItem.Weight * questitem.RewardItem.Amount));
					}
					else if (questitem.RewardItem is Container)
					{
						AddLabel(230, 356, 55, String.Format("Reward: {0} ({1} items)",
							questitem.RewardItem.GetType().Name,
							questitem.RewardItem.TotalItems));
						AddLabel(230, 373, 55,
							String.Format("Weight: {0}",
				...
				else if (questitem.AlreadyDone)
				{
					...
					{
						ArrayList a = XmlAttach.FindAttachments(questitem.Owner, typeof(XmlQuestAttachment),
							questitem.Name);
						if (a != null && a.Count > 0)
						{
							AddLabel(100, 392, 33,
								String.Format("Repeatable in {0}", ((XmlQuestAttachment)a[0]).Expiration));
						}
						else
						{
				...
				{
					AddHtml(250, 40, 200, 30,
						XmlSimpleGump.Color(String.Format("Difficulty Level {0}", questitem.Difficulty), "00FF42"),
						false, false);
				}
				...
				if (questitem.PartyEnabled)
				{
					...
					{
						AddHtml(250, 70, 200, 30,
							XmlSimpleGump.Color(String.Format("Party Range {0}", questitem.PartyRange), "00FF42"),
							false, false);
					}
					else
					{
		...
		{
			if (info == null || state == null || state.Mobile == null || state.Mobile.Deleted || m_questitem == null ||
			    m_questitem.Deleted)
				return;
