...
namespace Server.Gumps
{
	public class QuestLogGump : Gump
	{
		...
		public QuestLogGump(Mobile from, int page, ArrayList list) : base(12, 24)
		{
			...
			if (list == null)
			{
				...
				if (from.Backpack != null)
				{
					...
					if (packquestitems != null)
					{
						...
						{
							if (packquestitems[i] != null && !packquestitems[i].Deleted &&
			...
			AddHtml(243 - xoffset, 45, 200, 32, XmlSimpleGump.Color("Available Credits:", "FFFFFF"), false,
				false); // Your Reward Points: 

			AddHtml(453 - xoffset, 45, 200, 32, XmlSimpleGump.Color("Rank:", "FFFFFF"), false, false); // Rank

			AddHtml(600 - xoffset, 45, 200, 32, XmlSimpleGump.Color("Quests Completed:", "FFFFFF"), false,
				false); // Quests completed
			...
			AddHtml(610 - xoffset, 64, 200, 32, XmlSimpleGump.Color("Next Available", "FFFFFF"), false,
				false); // Next Available
			...
			AddHtml(150 - xoffset, 416, 200, 32, XmlSimpleGump.Color("Top Players", "FFFFFF"), false,
				false); // Top players gump
			...
			for (int i = index; i < (index + count) && i >= 0 && i < list.Count; ++i)
			{
				...
				else if (obj is XmlQuestPoints.QuestEntry)
				{
					...
					XmlQuestAttachment qa =
						(XmlQuestAttachment)XmlAttach.FindAttachment(from, typeof(XmlQuestAttachment), e.Name);
					if (qa != null)
					{
