...
namespace Server.Gumps
{
	public class XmlSetGump : Gump
	{
...
#if (NEWTIMERS)
		public XmlSetGump(PropertyInfo prop, Mobile mobile, object o, Stack<PropertiesGump.StackEntry> stack, int page,
...
#endif
		{
			...
			AddBackground(0, 0, BackWidth + xextend,
				BackHeight + (canNull ? (EntryHeight + OffsetSize) : 0) + (canDye ? (EntryHeight + OffsetSize) : 0) +
				(isBody ? (EntryHeight + OffsetSize) : 0), BackGumpID);
			AddImageTiled(BorderSize, BorderSize, TotalWidth + xextend - (OldStyle ? SetWidth + OffsetSize : 0),
			...
			if (canDye)
			{
				...
				AddImageTiled(x, y, EntryWidth + xextend, EntryHeight, EntryGumpID);
				AddLabelCropped(x + TextOffsetX, y, EntryWidth + xextend - TextOffsetX, EntryHeight, TextHue,
					"Hue Picker");
				x += EntryWidth + xextend + OffsetSize;
...
#if (NEWTIMERS)
			public InternalPicker(PropertyInfo prop, Mobile mobile, object o, Stack<PropertiesGump.StackEntry> stack,
				int page, ArrayList list) : base(((IHued)o).HuedItemID)
#else
		...
		public override void OnResponse(NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 4: // Body Picker
				{
					...
					m_Mobile.SendGump(new SetBodyGump(m_Property, m_Mobile, m_Object, new Stack(m_Stack), m_Page,
						m_List));
