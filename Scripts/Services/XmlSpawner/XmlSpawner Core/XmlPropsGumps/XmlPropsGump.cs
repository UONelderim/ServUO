...
namespace Server.Gumps
{
	public class XmlPropertiesGump : Gump
	{
				...
				default:
				{
					...
					if (index >= 0 && index < m_List.Count)
					{
						...
						if (IsType(type, typeofMobile) || IsType(type, typeofItem))
							...
							from.SendGump(new XmlSetCustomEnumGump(prop, from, m_Object, m_Stack, m_Page, m_List,
								GetCustomEnumNames(type)));
						else if (IsType(type, typeofEnum))
							from.SendGump(new XmlSetListOptionGump(prop, from, m_Object, m_Stack, m_Page, m_List,
								Enum.GetNames(type), GetObjects(Enum.GetValues(type))));
						else if (IsType(type, typeofBool))
							from.SendGump(new XmlSetListOptionGump(prop, from, m_Object, m_Stack, m_Page, m_List,
								m_BoolNames, m_BoolValues));
		...
		private static readonly Type[] typeofReal = { typeof(float), typeof(double) };
		...
		public static string ValueToString(object o)
		{
			...
			if (o is byte || o is sbyte || o is short || o is ushort || o is int || o is uint || o is long ||
			    o is ulong)
			{
