...
namespace Server.Mobiles
{
	...
	public class BaseXmlSpawner
	{
		...
		private static readonly Dictionary<string, typemodKeyword> typemodKeywordHash =
			new Dictionary<string, typemodKeyword>();

		private static readonly Dictionary<string, valueKeyword> valueKeywordHash =
			new Dictionary<string, valueKeyword>();

		private static readonly Dictionary<string, valuemodKeyword> valuemodKeywordHash =
						...
						if (IsValuemodKeyword(value_keywordargs[0]))
						{
							...
							if (kw == valuemodKeyword.INC)
							{
								...
								if (value_keywordargs.Length > 1)
								{
									...
									{
										int min, max;
										if (int.TryParse(value_keywordargs[1], out min) &&
			...
			if (spawner != null)
			{
				...
				if (!spawner.Deleted)
				{
					...
					else
					{
						...
						Point3D loc = spawner.GetSpawnPosition(requiresurface, theSpawn.PackRange, packcoord,
							spawnpositioning);
		...
		public static bool SpawnTypeKeyword(object invoker, XmlSpawner.SpawnObject TheSpawn, string typeName,
			string substitutedtypeName, Mobile triggermob, Map map, out string status_str, byte loops)
		{
			status_str = null;
