...
namespace Server.Engines.XmlSpawner2
{
	public class XmlEnemyMastery : XmlAttachment
	{
		...
		public override string OnIdentify(Mobile from)
		{
			...
			{
				msg = String.Format("Enemy Mastery : +{3}% damage vs {0}, {1}%, hitchance expires in {2} mins", m_Enemy,
					Chance, Expiration.TotalMinutes, PercentIncrease);
			}
			else
			{
				msg = String.Format("Enemy Mastery : +{2}% damage vs {0}, {1}% hitchance", m_Enemy, Chance,
					PercentIncrease);
			}
