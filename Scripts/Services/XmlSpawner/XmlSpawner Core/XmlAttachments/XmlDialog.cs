...
namespace Server.Engines.XmlSpawner2
{
	public class XmlDialog : XmlAttachment
	{
		...
		public class SpeechEntry
		{
			...
			public string
				Keywords; // comma separated list of keywords that can be matched to activate the entry.  If no keywords are present then it is automatically activated
			...
			public int
				PrePause = -1; // pause in seconds before saying the speech for this entry.  -1 indicates the use of auto pause calculation based on triggering speech length.
			...
			public bool
				IgnoreCarried =
					false; // ignore the TriggerOnCarried/NoTriggerOnCarried settings for the dialog when activating this entry
		...
		private int
			m_CurrentEntryNumber =
				-1; // used to determine which entry will be subject to modification by various entry editing calls
		...
		private Mobile
			m_ActivePlayer; // keep track of the player that is currently engaged in conversation so that other players speech can be ignored.

		private int
			m_SpeechPace =
				defSpeechPace; // used for automatic prepause delay calculation.  delayinsecs = speechlength/speechpace + 1
		...
		public bool LoadConfig
		{
			...
			set
			{
				if (value == true) DoLoadNPC(null, ConfigFile);
			}
		}
			...
			foreach (SpeechEntry s in m_SpeechEntries)
			{
				    ...
				    (CheckDependsOn(s, currententryid) && (IsActive || currententryid == 0)))
				{
					...
					if (s.Condition != null)
					{
						...
						if (!BaseXmlSpawner.CheckPropertyString(null, this, s.Condition, out status_str))
						{
							continue;
						}
					}
		...
		private void ExecuteGump(Mobile mob, string gumpstring)
		{
			...
			if (typeName == "GUMP")
			{
				BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName, substitutedtypeName, mob, map,
					out status_str);
		...
		private void ExecuteAction(Mobile mob, string action)
		{
			...
			{
				BaseXmlSpawner.SpawnTypeKeyword(AttachedTo, TheSpawn, typeName, substitutedtypeName, mob, map,
					out status_str);
			}
			else
			{
				...
				try
				{
					...
					else if (o is Mobile)
					{
						...
						BaseXmlSpawner.ApplyObjectStringProperties(null, substitutedtypeName, m, mob, AttachedTo,
							out status_str);
					}
					else if (o is Item)
					{
						Item item = (Item)o;
						BaseXmlSpawner.AddSpawnItem(null, AttachedTo, TheSpawn, item, loc, map, mob, false,
		...
		{
			if (status_str != null && mob != null && !mob.Deleted && mob is PlayerMobile &&
		...
		public override void OnSpeech(SpeechEventArgs e)
		{
			...
			if (!e.Handled && m_Running && m_ProximityRange >= 0 && ValidSpeechTrig(e.Mobile) &&
		...
		public bool ProcessSpeech(Mobile m, string speech)
		{
			...
			if ((speech != null || CheckDependsOn(matchentry, -2)) && TriggerOnCarried != null &&
			...
			if ((speech != null || CheckDependsOn(matchentry, -2)) && NoTriggerOnCarried != null &&
			...
			m_HoldProcessing = true;
			Timer.DelayCall(TimeSpan.FromSeconds(prepause), new TimerStateCallback(DelayedSpeech),
