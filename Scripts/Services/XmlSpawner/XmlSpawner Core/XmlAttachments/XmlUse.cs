...
namespace Server.Engines.XmlSpawner2
{
	public class XmlUse : XmlAttachment
	{
		...
		private string
			m_TargetCondition; // condition test applied when target is selected to determine whether it is appropriate
		...
		private string
			m_FailureAction; // action performed if the player cannot use the object for reasons other than range, refractory, or maxuses

		private string
			m_RefractoryAction; // action performed if the object is used before the refractory interval expires
		...
		private void ExecuteAction(Mobile mob, object target, string action)
		{
			...
			{
				BaseXmlSpawner.SpawnTypeKeyword(target, TheSpawn, typeName, substitutedtypeName, mob, map,
					out status_str);
			}
			else
			{
				...
				try
				{
					...
					else if (o is Mobile)
					{
						...
						BaseXmlSpawner.ApplyObjectStringProperties(null, substitutedtypeName, m, mob, target,
							out status_str);
					}
					else if (o is Item)
					{
						Item item = (Item)o;
						BaseXmlSpawner.AddSpawnItem(null, target, TheSpawn, item, loc, map, mob, false,
		...
		{
			return (BlockDefaultUse || !(CheckRange(from, target) && CheckCondition(from, target) && CheckMaxUses &&
			                             CheckRefractory));
		}
