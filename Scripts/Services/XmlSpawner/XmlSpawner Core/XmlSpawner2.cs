 ...
 AccessLevel.GameMaster; // only allow spawning of objects that have Constructable access restrictions at this level or lower. Must define RESTRICTCONSTRUCTABLE to enable this.
		...
		public TimeSpan GameTOD
		{
			get
			{
				...
				return (new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, DateTime.UtcNow.Day, hours, minutes,
					0).TimeOfDay);
			}
		}
		...
		{
			get { return m_DisableGlobalAutoReset; }
					...
					for (int x = 0; x <= m_Width; x++)
					{
						...
						for (int y = 0; y <= m_Height; y++)
						{
							    ...
							    NewY == ValidX1 || NewY == ValidX2 || NewY == ValidY1 || NewY == ValidY2)
							{
								...
								Static s = new Static(ShowBoundsItemId) { Visible = false };
		...
		public TimeSpan TOD
		{
			get
			{
				if (m_TODMode == TODModeType.Gametime)
				{
					...
					int minutes;
					Clock.GetTime(Map, Location.X, Location.Y, out hours, out minutes);
					return (new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, DateTime.UtcNow.Day, hours,
						minutes, 0).TimeOfDay);
				}
		...
		public bool TODInRange
		{
			get
			{
				...
				if (m_TODMode == TODModeType.Gametime)
				{
					...
					int minutes;
					Clock.GetTime(Map, Location.X, Location.Y, out hours, out minutes);
					now = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, DateTime.UtcNow.Day, hours, minutes,
						0);
				}
				else
				{
		...
		public bool LoadConfig
		{
			...
			set
			{
				if (value) LoadXmlConfig(ConfigFile);
			}
		}
		...
		public static void ExecuteAction(object attachedto, Mobile trigmob, string action)
		{
			...
			SpawnObject TheSpawn = new SpawnObject(null, 0) { TypeName = action };
			...
			{
				BaseXmlSpawner.SpawnTypeKeyword(attachedto, TheSpawn, typeName, substitutedtypeName, trigmob, map,
					out status_str);
			}
			else
			{
		...
		{
			if (AllowTriggering &&
			    !m_proximityActivated) // only proximity trigger when no spawns have already been triggered
			{
		...
		{
			if (m_Running && m_ProximityRange >= 0 && ValidPlayerTrig(e.Mobile) && CanSpawn && !m_refractActivated &&
			    TODInRange)
			{
				m_speechTriggerActivated = false;
									...
									if (b.Commands != null)
									{
										for (int j = 0; j < b.Commands.Length; j++)
										{
											...
											if (commandname.ToLower() == oldname)
											{
												...
												List<BaseCommandImplementor>
													impls = BaseCommandImplementor.Implementors;
		...
		public static void ShowSpawnPoints_OnCommand(CommandEventArgs e)
		{
			...
			foreach (XmlSpawner xml_item in ToShow)
			{
				    ...
				    xml_item.RootParent is Container)
				{
					...
					Static s = new Static(ShowItemId) { Visible = false };
			...
			out int processedmaps, out int processedspawners)
		{
			...
			if (from != null)
				from.SendMessage(string.Format("UnLoading {0} objects{1} from file {2}.",
					"XmlSpawner",
					(!string.IsNullOrEmpty(SpawnerPrefix) ? " beginning with " + SpawnerPrefix : string.Empty),
					filename));
			...
			ref double overridemaxtime)
		{
			...
			if (args.Length > 0 && (args[0] == "*"))
			{
				...
				if (!badspawn && totalspawns > 0)
				{
					...
					string spawnername = string.Format("{0}#{1}", Path.GetFileNameWithoutExtension(filename),
						spawnercount);
					...
					if (map == 0)
					{
						...
						spawner = new XmlSpawner(SpawnId, x, y, 0, 0, spawnername, totalmaxcount,
							...
							TimeSpan.FromHours(0), null, false, null)
						{
							SpawnRange = spawnrange, m_PlayerCreated = true
						};
		...
		private static void ImportMegaSpawner(Mobile from, XmlElement node)
		{
					...
					foreach (XmlElement entrynode in listnode.GetElementsByTagName("EntryList"))
					{
						...
						if (entrynode != null)
						{
							...
							else
							{
								...
								if (minDelay !=
								    TimeSpan.FromSeconds(int.Parse(GetText(entrynode["MinDelay"], "05:00"))))
								{
									diff = true;
									// log it
									try
								...
								if (maxDelay !=
								    TimeSpan.FromSeconds(int.Parse(GetText(entrynode["MaxDelay"], "10:00"))))
								{
									diff = true;
									// log it
									try
			...
			XmlSpawner spawner = new XmlSpawner(SpawnId, location.X, location.Y, 0, 0, name, maxcount,
				...
				TimeSpan.FromMinutes(0), null, null, null, null, null,
				null, null, null, null, 1, null, group, defTODMode, defKillReset, false, -1, null, false, false, false,
				null, defDespawnTime, null, false, null) { SpawnRange = spawnRange, m_PlayerCreated = true };
		...
		{
			XmlLoadFromStream(fs, filename, SpawnerPrefix, from, fromloc, frommap, loadrelative, maxrange, loadnew,
				out processedmaps, out processedspawners, false);
		}
				...
				if (ds.Tables[SpawnTablePointName] != null && ds.Tables[SpawnTablePointName].Rows.Count > 0)
					foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
					{
						...
						if ((SpawnerPrefix == null) || (SpawnerPrefix.Length == 0) ||
						    SpawnName.StartsWith(SpawnerPrefix))
						{
							// Try load the GUID (might not work so create a new GUID)
							Guid SpawnId = Guid.NewGuid();
							if (!loadnew)
							...
							if (!bad_spawner)
							{
								...
								XmlSpawner TheSpawn = new XmlSpawner(SpawnId, SpawnX, SpawnY, SpawnWidth, SpawnHeight,
									...
									SpawnWaypoint) { m_DisableGlobalAutoReset = TickReset };
		...
		public static void Load_OnCommand(CommandEventArgs e)
		{
			...
			if (m == null || m.AccessLevel >= DiskAccessLevel)
			{
				if (e.Arguments.Length >= 1)
				{
					...
					XmlLoadFromFile(filename, SpawnerPrefix, m, false, 0, false, out processedmaps,
						out processedspawners);
				}
				else if (m != null)
				{
		...
		public bool Spawn(bool smartspawn, byte loops)
		{
			    ...
			    TODInRange)
			{
				...
				SpawnIndex = m_SequentialSpawning >= 0
					? GetCurrentAvailableSequentialSpawnIndex(m_SequentialSpawning)
					: RandomAvailableSpawnIndex();
			...
			byte loops)
		{
			...
			if (TheSpawn != null)
			{
				...
				if (BaseXmlSpawner.IsTypeOrItemKeyword(typeName))
				{
					...
					bool completedtypespawn = BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName,
						substitutedtypeName,
			...
			if (sgroup >= 0)
			{
				...
				for (int j = 0; j < m_SpawnObjects.Count; j++)
				{
					...
					if (so != null && so.SubGroup == sgroup)
					{
						...
						bool success = Spawn(j, smartspawn, so.SpawnsPerTick, so.PackRange, packcoord,
			...
			Mobile mob)
		{
			...
			for (int i = 0; i < staticTiles.Length; ++i)
			{
				...
				if ((surface || impassable) && (staticTiles[i].Z + id.CalcHeight) > z &&
						...
						if (includetile && !excludetile && ((sflags & tileflag) == tileflag))
						{
							...
							if (p == Point3D.Zero)
								...
								p = new Point3D(x, y, stile.Z + stile.Height);
							         ...
							         Math.Abs(stile.Z -
							                  spawnerZ)) //maggiore distanza rispetto allo statico dallo spawner
								p = new Point3D(x, y, stile.Z + stile.Height);
							allok = true;
			...
			List<SpawnPositionInfo> spawnpositioning, Mobile mob)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				var fit = requiresurface
					? CanSpawnMobile(x, y, defaultZ, mob)
					: Map.CanFit(x, y, defaultZ, SpawnFitSize, true, false, false);
				...
				fit = requiresurface
					? CanSpawnMobile(x, y, z, mob)
					: Map.CanFit(x, y, z, SpawnFitSize, true, false, false);
		...
		private class SpawnerTimer : Timer
		{
				...
				: base(delay)
			{
				if (spawner.IsInactivated ||
				    ...
				    spawner.MaxCount) // reduce timer priority if spawner is inactivated or spawner is maxed
				{
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
				...
				case 0:
				{
					...
					if (m_Width == m_Height && (m_Width % 2) == 0 && (m_X + m_Width / 2) == X &&
					    (m_Y + m_Height / 2) == Y)
						m_SpawnRange = m_Width / 2;
					else
						m_SpawnRange = -1;
					...
					for (int i = 0; i < SpawnListSize; ++i)
					{
						...
						if (typeName == null || ((SpawnerType.GetType(typeName) == null) &&
						                         (!BaseXmlSpawner.IsTypeOrItemKeyword(typeName) &&
						                          typeName.IndexOf('{') == -1 && !typeName.StartsWith("*") &&
						                          !typeName.StartsWith("#"))))
						{
							if (m_WarnTimer == null)
