...
namespace Server.Mobiles
{
	public class XmlGetAttGump : Gump
	{
			...
			bool selectall, bool[] selectionlist, int X, int Y) : base(X, Y)
		{
			...
			AddButton(415 + 25 * (int)(MaxEntries / MaxEntriesPerPage), 450, 0x15E1, 0x15E5, 201, GumpButtonType.Reply,
				0); // block forward
			...
			for (int i = 0; i < MaxEntries; i++)
			{
				...
				AddButton(620, 22 * (i % MaxEntriesPerPage) + 32, (sel ? 0xD3 : 0xD2), (sel ? 0xD2 : 0xD3), 4000 + i,
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 156: // Delete selected items
				{
					...
					state.Mobile.SendGump(new XmlConfirmDeleteGump(state.Mobile, m_TargetObject, m_SearchList,
						m_SelectionList, DisplayFrom, SelectAll, allcount));
					return;
				}
		...
		public class XmlConfirmDeleteGump : Gump
		{
			...
			public XmlConfirmDeleteGump(Mobile from, object target, ArrayList searchlist, bool[] selectedlist,
