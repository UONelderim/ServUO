	...
	public class XmlEditDialogGump : Gump
	{
				...
				switch (info.ButtonID)
				{
					default:
					{
						if (radiostate == 1)
						{
							...
							state.Mobile.SendGump(new XmlEditDialogGump(state.Mobile, true, xd, -1, 0, null, false,
								null, 0, 0));
						}
			...
			bool selectall, bool[] selectionlist, int X, int Y) : base(X, Y)
		{
			...
			for (int i = 0; i < MaxEntries; i++)
			{
				...
				AddImageTiled(x, y + 22 * (i % MaxEntriesPerPage) + 31, w, 21, 0x23F4);
				AddLabelCropped(x, y + 22 * (i % MaxEntriesPerPage) + 31, w - 5, 21, texthue,
				...
				AddButton(730, y + 22 * (i % MaxEntriesPerPage) + 32, (sel ? 0xD3 : 0xD2), (sel ? 0xD2 : 0xD3),
					4000 + i, GumpButtonType.Reply, 0);
			}
			...
			if (m_SearchList != null)
			{
				...
				int last = DisplayFrom + MaxEntries < m_SearchList.Count
			...
			AddButton(510 + 25 * (int)(MaxEntries / MaxEntriesPerPage), y, 0x15E1, 0x15E5, 201, GumpButtonType.Reply,
				0); // block forward
		...
		private void SaveList(Mobile from, string filename)
		{
			...
			string dirname;
			if (System.IO.Directory.Exists(XmlDialog.DefsDir) && filename != null && !filename.StartsWith("/") &&
			    !filename.StartsWith("\\"))
			{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			if (m_SearchList != null && Selected >= 0 && Selected + DisplayFrom >= 0 &&
			...
			switch (info.ButtonID)
			{
				...
				case 156: // Delete selected entries
				{
					...
					state.Mobile.SendGump(new XmlConfirmDeleteGump(state.Mobile, g, m_SearchList, m_SelectionList,
						DisplayFrom, SelectAll, allcount));
					return;
				}
		...
		public class XmlConfirmDeleteGump : Gump
		{
			...
			public XmlConfirmDeleteGump(Mobile from, XmlEditDialogGump gump, ArrayList searchlist, bool[] selectedlist,
				int displayfrom, bool selectall, int allcount) : base(0, 0)
			{
				...
				switch (info.ButtonID)
				{
					default:
					{
						if (radiostate == 1 && SearchList != null && SelectedList != null)
						{
							...
							{
								int index = i - DisplayFrom;
								if ((index >= 0 && index < SelectedList.Length && SelectedList[index] == true) ||
								    selectAll)
								{
