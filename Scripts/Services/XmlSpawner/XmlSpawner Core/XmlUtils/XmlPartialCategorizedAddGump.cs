...
namespace Server.Gumps
{
	public class XmlPartialCategorizedAddGump : Gump
	{
		...
		public XmlPartialCategorizedAddGump(Mobile from, string searchString, int page, ArrayList searchResults,
			bool explicitSearch, int entryindex, Gump gump) : base(50, 50)
		{
			...
			for (int i = 0; i < types.Count; ++i)
			{
				    ...
				    t.Name.ToLower().IndexOf(match) >= 0 && !results.Contains(t))
				{
					...
					for (int j = 0; j < ctors.Length; ++j)
					{
						...
						{
							SearchEntry s = new SearchEntry { EntryType = t, Parameters = ctors[j].GetParameters() };
							//results.Add( t );
							results.Add(s);
							//break;
						}
		...
		public override void OnResponse(Network.NetState sender, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				case 1: // Search
				{
					...
					{
						from.SendGump(new XmlPartialCategorizedAddGump(from, match, 0, Match(match), true, m_EntryIndex,
							m_Gump));
					}
				...
				default:
				{
					...
					if (index >= 0 && index < m_SearchResults.Count)
					{
						...
						{
							if (mXmlAddGump.defs?.NameList != null && m_EntryIndex >= 0 &&
							    m_EntryIndex < mXmlAddGump.defs.NameList.Length)
							{
						...
						else if (m_Spawner != null && type != null)
						{
							...
							if (xg != null)
							{
								...
								{
									Typename = type.Name, Index = m_EntryIndex, Color = 0x1436
								};
