...
namespace Server.Mobiles
{
	public class XmlSpawnerDefaults
	{
		...
		public static DefaultEntry GetDefaults(string account, string name)
		{
			...
			DefaultEntry newentry = new DefaultEntry { PlayerName = name, AccountName = account };
			if (DefaultEntryList == null)
	...
	public class XmlAddGump : Gump
	{
		...
		private void DoSaveDefs(Mobile from, string filename)
		{
			...
			var dirname = Directory.Exists(DefsDir)
				? string.Format("{0}/{1}.defs", DefsDir, filename)
				: string.Format("{0}.defs", filename);
				...
				if (ds.Tables.Count > 0)
				{
					...
					if (ds.Tables[DefsTablePointName] != null && ds.Tables[DefsTablePointName].Rows.Count > 0)
					{
						...
						if (todmode != null)
						{
							...
							else if (todmode == "Gametime") defs.TODMode = XmlSpawner.TODModeType.Gametime;
						}
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				for (int i = 0; i < MaxEntries; i++)
				{
					if (defs.SelectionList != null && i < defs.SelectionList.Length && defs.SelectionList[i] &&
					    defs.NameList != null && i < defs.NameList.Length && defs.NameList[i] != null &&
				...
				XmlSpawner spawner = new XmlSpawner(SpawnId, from.Location.X, from.Location.Y, 0, 0, sname, maxcount,
					...
					defs.Team, defs.HomeRange, defs.HomeRangeIsRelative, new XmlSpawner.SpawnObject[0], defs.RefractMin,
					defs.RefractMax,
					defs.TODStart, defs.TODEnd, null, defs.TriggerObjectProp, defs.ProximityMsg, defs.TriggerOnCarried,
					defs.NoTriggerOnCarried,
					defs.SpeechTrigger, null, null, defs.PlayerTriggerProp, defs.TriggerProbability, null, defs.Group,
					defs.TODMode, defs.KillReset, defs.ExternalTriggering,
					defs.SequentialSpawn, null, defs.AllowGhostTrig, defs.AllowNPCTrig, defs.SpawnOnTrigger, null,
					defs.DespawnTime, defs.SkillTrigger, defs.SmartSpawning, null) { PlayerCreated = true };
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			// read the text entries for default values
			XmlSpawnerDefaults.DefaultEntry defaults =
				XmlSpawnerDefaults.GetDefaults(state.Account.ToString(), state.Mobile.Name);
			...
			switch (info.ButtonID)
			{
				...
				case 115: // SaveDefs
				{
					...
					{
						filename = string.Format("{0}-{1}-{2}", defaults.AccountName, defaults.PlayerName,
							defaults.DefsExt);
					}
					else
					{
				...
				case 306: // TOD mode
				{
					defaults.TODMode = defaults.TODMode == XmlSpawner.TODModeType.Realtime
						? XmlSpawner.TODModeType.Gametime
						: XmlSpawner.TODModeType.Realtime;
					break;
				}
				case 1000: // GoTo
				{
				...
				case 3999: // clear selections
				{
					...
					if (defaults.SelectionList != null)
						Array.Clear(defaults.SelectionList, 0, defaults.SelectionList.Length);
					break;
				}
				...
				default:
				{
					...
					if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxEntries)
					{
						...
						XmlAddGump newg = new XmlAddGump(state.Mobile, defaults.StartingLoc, defaults.StartingMap,
							false, defaults.ShowExtension, 0, 0);
						...
						if (defaults.NameList[i] == null || defaults.NameList[i].Length == 0)
						{
							...
							state.Mobile.SendGump(new XmlCategorizedAddGump(state.Mobile, defaults.CurrentCategory,
								defaults.CurrentCategoryPage, i, newg));
						}
						else
						{
							...
							state.Mobile.SendGump(new XmlPartialCategorizedAddGump(state.Mobile, defaults.NameList[i],
								0, types, true, i, newg));
						}
