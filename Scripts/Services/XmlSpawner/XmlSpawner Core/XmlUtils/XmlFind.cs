...
using Rudder = Server.Items.Rudder;
	...
	public class XmlFindGump : Gump
	{
		public class XmlFindThread
		{
			...
			public void XmlFindThreadMain()
			{
				...
				Timer.DelayCall(TimeSpan.Zero, new TimerStateCallback(GumpDisplayCallback),
		...
		public class SearchCriteria
		{
			...
			public SearchCriteria(bool dotype, bool doname, bool dorange, bool doregion, bool doentry, bool doentrytype,
				bool docondition, bool dofel, bool dotram,
				bool domal, bool doilsh, bool dotok, bool doter, bool doint, bool donull, bool doerr, bool doage,
				bool dohidevalid,
				bool agedirection, double age, int range, string region, string condition, string type, string name,
				string entry
			)
			{
				Dosearchtype = dotype;
		...
		private static bool TestRegion(object o, string regionname)
		{
			...
			if (o is Item item)
			{
				...
				Region r = Region.Regions.FirstOrDefault(reg =>
					reg.Map == item.Map && !string.IsNullOrEmpty(reg.Name) && string.Equals(reg.Name, regionname,
			...
			{
				Region r = Region.Regions.FirstOrDefault(reg =>
					reg.Map == mob.Map && !string.IsNullOrEmpty(reg.Name) && string.Equals(reg.Name, regionname,
						StringComparison.CurrentCultureIgnoreCase));
		...
		private static bool TestValidInternal(object o)
		{
			...
			else if (o is Item i)
			{
				...
				if (i.Map != Map.Internal || i.Parent != null || i is Fists || i is MountItem || i is EffectItem ||
				    i.HeldBy != null ||
				...
				if (i is Static && i.Name != null && (i.Name.ToLower() == "weapon pad" || i.Name.ToLower() == "deck"))
					return true;
				if (i is GalleonHold || i is MooringLine || i is HoldItem || i is BaseDockedBoat || i is Rudder ||
				    i is RudderHandle || i is ShipWheel || i is BaseBoat || i is Plank || i is TillerMan || i is Hold ||
				    i is IShipCannon || i is DeckItem || i is WeaponPad)
					return true;
				...
				if (i is AddonComponent component)
				{
					...
					if (addon != null && (addon is ArmoryAddon || addon is BarAddon || addon is BelfryAddon ||
					                      addon is ShadowguardFountainAddon || addon is OrchardAddon
					                      || addon is CastleAddon))
						return true;
				}

				if (i is BaseAddon && (i is ArmoryAddon || i is BarAddon || i is BelfryAddon ||
				                       i is ShadowguardFountainAddon || i is OrchardAddon
				                       || i is CastleAddon))
					return true;
		...
		public static ArrayList Search(SearchCriteria criteria, out string status_str)
		{
			...
			if (itemarray != null)
			{
				foreach (Item i in itemarray)
				{
					...
					// check for name
					if (criteria.Dosearchname && (i.Name != null) && (criteria.Searchname != null) &&
					...
					if (criteria.Dosearchrange &&
					    TestRange(i, criteria.Searchrange, criteria.Currentmap, criteria.Currentloc))
					{
						hasrange = true;
					}
					...
					if (criteria.Dosearchcondition && (criteria.Searchcondition != null))
					{
						...
						hascondition =
							BaseXmlSpawner.CheckPropertyString(null, i, criteria.Searchcondition, out status_str);
					}
					...
					if (criteria.Dosearchspawnentry)
					{
						...
						if (criteria.Searchspawnentry == null ||
						...
						else
						{
							...
							if (i is XmlSpawner spawner)
							{
								...
								foreach (XmlSpawner.SpawnObject so in spawner.m_SpawnObjects)
								{
									if (criteria.Dosearchspawntype)
									{
										...
										if (type != null && (type == targetentrytype ||
									...
									else
									{
										...
										if (so.TypeName != null && so.TypeName.ToLower()
											    .IndexOf(criteria.Searchspawnentry.ToLower()) >= 0)
										{
											hasentry = true;
											break;
										}
							...
							else if (i is Spawner spawner1)
							{
								...
								foreach (SpawnObject obj in spawner1.SpawnObjects)
								{
									...
									{
										if (so != null && so.ToLower().IndexOf(criteria.Searchspawnentry.ToLower()) >=
										    0)
										{
											hasentry = true;
											break;
			...
			if (!criteria.Dosearcherr)
			{
				...
				if (mobilearray != null)
				{
					foreach (Mobile i in mobilearray)
					{
						...
						// check for range
						if (criteria.Dosearchrange && TestRange(i, criteria.Searchrange, criteria.Currentmap,
							    criteria.Currentloc))
						{
							hasrange = true;
						}
						...
						if (criteria.Dosearchtype &&
						    (i.GetType().IsSubclassOf(targetType) || i.GetType() == targetType))
						{
							hastype = true;
						}
		...
		public static void XmlFind_OnCommand(CommandEventArgs e)
		{
			...
			e.Mobile.SendGump(
				new XmlFindGump(e.Mobile, e.Mobile.Location, e.Mobile.Map, typename, range, dorange, x, y));
		}
		...
		public XmlFindGump(Mobile from, Point3D startloc, Map startmap, string type, int range, bool dorange, int x,
			int y)
		...
		public XmlFindGump(Mobile from, Point3D startloc, Map startmap, bool firststart, bool extension, bool descend,
			SearchCriteria criteria, ArrayList searchlist, int selected, int displayfrom, string savefilename,
			string commandstring, bool sorttype, bool sortname, bool sortrange, bool sortmap, bool sortselect,
			bool selectall, int X, int Y)
			: base(X, Y)
		{
			...
			if (m_ShowExtension)
			{
				...
				AddButton(415 + 25 * (MaxEntries / MaxEntriesPerPage), height - 25, 0x15E1, 0x15E5, 201,
					GumpButtonType.Reply, 0); // block forward
				...
				for (int i = 0; i < MaxEntries; i++)
				{
					...
					{
						AddButton(145, 22 * (i % MaxEntriesPerPage) + 30, 0xFBD, 0xFBE, 2000 + i, GumpButtonType.Reply,
							0);
					}
					...
					AddButton(730, 22 * (i % MaxEntriesPerPage) + 32, (e.Selected ? 0xD3 : 0xD2),
						(e.Selected ? 0xD2 : 0xD3), 4000 + i, GumpButtonType.Reply, 0);
				}
			}
		}
		...
		{
			state.Mobile.SendGump(new XmlFindGump(m_From, StartingLoc, StartingMap, false, m_ShowExtension,
				Descendingsort, m_SearchCriteria, m_SearchList, Selected, DisplayFrom, SaveFilename,
		...
		private void SaveList(Mobile from, string filename)
		{
			...
			string dirname;
			if (System.IO.Directory.Exists(XmlSpawner.XmlSpawnDir) && filename != null && !filename.StartsWith("/") &&
			    !filename.StartsWith("\\"))
			{
		...
		public override void OnResponse(NetState state, RelayInfo info)
		{
			...
			switch (info.ButtonID)
			{
				...
				case 3: // Search
				{
					...
					Thread find = new Thread(tobj.XmlFindThreadMain) { Name = "XmlFind Thread" };
					find.Start();
