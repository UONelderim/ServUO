	...
	public class HarvestTarget : Target
	{
		...
		protected override void OnTarget(Mobile from, object targeted)
		{
			if (m_System is Mining)
			{
				if (targeted is StaticTarget staticTarget)
				{
					    ...
					    itemID == 0xEE8)
					{
						...
						if (player != null)
						{
							...
							//////Massapequa's grave digging starts here.//////
							GraveNode grave = null;

							foreach (Item item in player.Map.GetItemsInRange(staticTarget.Location, 1))
							{
								if (item is GraveNode graveNode)
								{
									grave = graveNode;
									break;
								}
							...
							{
								grave = new GraveNode();
								grave.MoveToWorld(staticTarget.Location, player.Map);
							}

							grave.OnMine(player, m_Tool);
				...
				else if (targeted is LandTarget && ((LandTarget)targeted).TileID >= 113 &&
				         ((LandTarget)targeted).TileID <= 120)
				{
